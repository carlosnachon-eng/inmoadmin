...
export async function paymentContext(db, token, now = new Date()) {
  ...
  if (!externalCase || !['awaiting_payment', 'proof_received', 'payment_validated', 'payment_rejected'].includes(externalCase.status)) return null
  const payment = checked(await db.from('blindaje_investigation_payments').select('id,status,proof_storage_path,rejection_reason').eq('case_id', access.case_id).maybeSingle())
  if (!payment || !['pending', 'proof_received', 'validated', 'rejected'].includes(payment.status)) return null
...
export const publicPayment = (context, bank) => ({
  ...
  ...(context.payment.status === 'rejected' ? { rejection_reason: context.payment.rejection_reason } : {}),
})
