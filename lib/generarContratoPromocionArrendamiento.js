...
export async function generarContratoPromocionArrendamiento(data) {
  const {
  ...
  const d = {
    nombre_propietario:    data.nombre_arrendador    || '',
    domicilio_propietario: data.domicilio_arrendador || '',
    telefono_propietario:  data.telefono_arrendador  || '',
    direccion_inmueble:    data.direccion_inmueble   || '',
    renta_mensual:         fmt(data.renta_mensual),
    renta_mensual_letra:   data.renta_mensual_letra  || '',
    ...
    sections: [{
      ...
      children: [
        ...
        para([
          ...
          bold(d.renta_mensual),
          d.renta_mensual_letra ? bold(` (${d.renta_mensual_letra})`) : normal(''),
          normal(', '),
          bold('"LA INMOBILIARIA"'),
        ...
        para([
          bold('SÉPTIMA.-'),
          normal(' La duración del presente contrato será de 60 días naturales, forzosos para ambas partes iniciando el día '),
        ...
        para([
          normal('Las partes manifiestan haber leído en su integridad este contrato, estar conformes con su contenido y tener plena capacidad jurídica para comprometerse.'),
        ]),
  ...
  a.href = url
  a.download = `Contrato_Promocion_Arrendamiento_${d.nombre_propietario.replace(/\s+/g, '_')}.docx`
  a.click()
