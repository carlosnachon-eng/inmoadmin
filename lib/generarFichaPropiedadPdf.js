...
export async function generarFichaPropiedadPdf(propiedad) {
  ...
  doc.setFontSize(15);
  doc.setTextColor(GRIS_OSCURO);
  const tituloLineas = doc.splitTextToSize(propiedad.titulo || "Propiedad", tituloAncho);
  ...
  doc.setFontSize(18);
  doc.setTextColor(ROJO);
  doc.text(`${fmt(propiedad.precio)} MXN`, pageWidth - margin, y, { align: "right" });
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(GRIS_CLARO);
  doc.text(operacionLabel, pageWidth - margin, y + 13, { align: "right" });
  ...
  if (amenidades.length > 0) {
    ...
    for (const a of amenidades) {
      const lineas = doc.splitTextToSize(`•  ${a}`, colAmenW);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(9.5);
