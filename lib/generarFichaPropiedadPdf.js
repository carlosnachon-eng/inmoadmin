...
export async function generarFichaPropiedadPdf(propiedad) {
  ...
  if (fotosPortada.length > 0) {
    ...
    for (const url of fotosPortada) {
      // Fondo de respaldo, por si la foto no llena el recuadro completo o
      // la descarga falla — nunca debe verse blanco "roto".
      doc.setFillColor(GRIS_BG);
      doc.rect(x, y, fotoW, fotoH, "F");

      const img = await descargarImagen(url);
      if (img) {
        // Modo "contain": la foto completa siempre cabe dentro del recuadro,
        // sin recortarse ni desbordarse (a diferencia de "cover", que puede
        // sobresalir del rect si el clip no se aplica correctamente).
  ...
  if (fotosUrls.length > 0) {
    ...
    for (const url of fotosUrls) {
      ...
      if (img) {
        ...
        const w = img.width * ratio, h = img.height * ratio;
        const offsetX = cx + (galW - w) / 2;
        const offsetY = rowY + (galH - h) / 2;
        doc.addImage(img.dataUrl, img.formato, offsetX, offsetY, w, h);
      }
      col++;
      if (col === galCols) {
