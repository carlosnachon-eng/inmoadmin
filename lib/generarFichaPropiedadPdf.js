...
// Las fichas se muestran en hoja carta, así que incrustar la fotografía
// original (a menudo de varios megapíxeles) solo infla el PDF sin aportar
// calidad visible. Para imágenes públicas de Supabase pedimos una versión
// optimizada; la foto original permanece intacta en Storage.
function urlOptimizadaParaPdf(url) {
  try {
...
async function descargarImagen(url) {
  const urlPdf = urlOptimizadaParaPdf(url);
  if (cacheImagenes.has(urlPdf)) return cacheImagenes.get(urlPdf);

  const descarga = (async () => {
  try {
    let resp = await fetch(urlPdf, { headers: { "User-Agent": "Mozilla/5.0 (compatible; EmporioBot/1.0)" } });
    // Si el servicio de transformación no está disponible para una imagen,
    // conservamos compatibilidad descargando el archivo original.
    if (!resp.ok && urlPdf !== url) {
      resp = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0 (compatible; EmporioBot/1.0)" } });
    }
  ...
  cacheImagenes.set(urlPdf, descarga);
  return descarga;
}
