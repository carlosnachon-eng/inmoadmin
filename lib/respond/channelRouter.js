...
const EMP_PROPERTY_ID_PATTERN = /(?:^|[^A-Z0-9-])(EMP-[A-Z0-9]{8})(?=$|[^A-Z0-9])/gi;
...
export function extractCanonicalEmpPropertyIds(messageText) {
  if (typeof messageText !== "string") return [];
  return [...new Set(
    [...messageText.matchAll(EMP_PROPERTY_ID_PATTERN)]
      .map((match) => match[1].toUpperCase())
  )];
}

export async function resolveRespondTerritorialRoute(event, {
  admin,
  targetPlazaCode = process.env.RESPOND_TERRITORY_VERACRUZ_PLAZA_CODE || "VERACRUZ",
  targetSalesTeamId = process.env.RESPOND_TERRITORY_VERACRUZ_SALES_TEAM_ID,
  targetRespondTeamId = process.env.RESPOND_TERRITORY_VERACRUZ_TEAM_ID,
  territorialWorkflowUrl = process.env.RESPOND_TERRITORY_VERACRUZ_WORKFLOW_URL,
} = {}) {
  const propertyIds = extractCanonicalEmpPropertyIds(event?.messageText);
  if (propertyIds.length !== 1) {
    return { route: false, reason: propertyIds.length ? "ambiguous_property_id" : "missing_property_id" };
  }
  if (!admin) return { route: false, reason: "territorial_lookup_unavailable" };

  const propertyPublicId = propertyIds[0];
  const { data: properties, error: propertyError } = await admin
    .from("propiedades")
    .select("id,public_id,plaza_id")
    .eq("public_id", propertyPublicId)
    .limit(2);
  if (propertyError) return { route: false, reason: "property_lookup_error" };
  if (properties?.length !== 1) {
    return { route: false, reason: properties?.length ? "ambiguous_property_match" : "unknown_property_id", propertyPublicId };
  }
  ...
  const { data: plaza, error: plazaError } = await admin
    ...
    .select("id,code")
    .eq("id", property.plaza_id)
    .maybeSingle();
  if (plazaError || !plaza) return { route: false, reason: "property_plaza_unresolved", propertyPublicId };
  if (plaza.code !== targetPlazaCode) {
    return { route: false, reason: "non_target_plaza", propertyPublicId, plazaCode: plaza.code };
  }

  const url = workflowUrl(territorialWorkflowUrl);
  if (!targetSalesTeamId || !targetRespondTeamId || !url) {
    return { route: false, reason: "territorial_configuration_incomplete", propertyPublicId, plazaCode: plaza.code };
  }
  const { data: team, error: teamError } = await admin
    .from("sales_teams")
    .select("id,plaza_id")
    .eq("id", targetSalesTeamId)
    .eq("plaza_id", plaza.id)
    .maybeSingle();
  if (teamError || !team) {
    return { route: false, reason: "territorial_team_mapping_invalid", propertyPublicId, plazaCode: plaza.code };
  }

  return {
    route: true,
    decision: "commercial_property_territory",
    target: "veracruz_sales_team",
    workflowUrl: url,
    eventId: clean(event?.eventId),
    messageId: clean(event?.messageId),
    contactId: clean(event?.respondContactId),
    channelId: clean(event?.channelId, MAX_CHANNEL_ID_LENGTH),
    propertyPublicId,
    plazaCode: plaza.code,
    respondTeamId: clean(targetRespondTeamId, 40),
    salesTeamId: team.id,
  };
}
  ...
  try {
    const response = await fetchImpl(decision.workflowUrl, {
      ...
      body: JSON.stringify({
        ...
        propertyPublicId: decision.propertyPublicId || null,
        plazaCode: decision.plazaCode || null,
        respondTeamId: decision.respondTeamId || null,
        salesTeamId: decision.salesTeamId || null,
      }),
...
export async function routeRespondMessageIsolated(event, options = {}) {
  ...
  const baseDecision = decideRespondMessageRoute(event, config);
  let decision = baseDecision;
  if (baseDecision.route && baseDecision.target === "ivonne_recepcion_v2") {
    const territorial = await resolveRespondTerritorialRoute(event, options);
    if (territorial.route) decision = territorial;
  }
  if (!decision.route) {
