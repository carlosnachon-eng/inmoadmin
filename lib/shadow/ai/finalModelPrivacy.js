...
const block = (reason) => { throw new FinalModelPrivacyError(reason); };
...
export function verifyFinalModelPayload(payload, { scope = null, transport = false } = {}) {
  ...
  const visit = (value, path = [], key = "") => {
    ...
    for (const [field, descriptor] of Object.entries(Object.getOwnPropertyDescriptors(value))) {
      ...
      inspectText(field, scope, reasons);
      visit(descriptor.value, [...path, field], Array.isArray(value) && ID_KEY.test(key) ? key : field);
    }
    seen.delete(value);
  };
  visit(payload);
  return { allowed: reasons.size === 0, reasons: [...reasons].sort() };
}
const assertVerified = (value, scope, transport = false) => {
  const checked = verifyFinalModelPayload(value, { scope, transport });
  if (!checked.allowed) block(checked.reasons[0]);
};
const freeze = (value) => {
...
export function serializeVerifiedAnthropicBody(body, messages) {
...
function assertNoEphemeralAliases(value) {
  if (typeof value === "string") {
    // No word boundaries: concatenating text must not hide an issued alias.
    if (/ref_[A-Za-z]+_\d+/u.test(value)) block("model_alias_in_free_text");
  } else if (value && typeof value === "object") {
    for (const [key, child] of Object.entries(value)) {
      assertNoEphemeralAliases(key);
      assertNoEphemeralAliases(child);
    }
  }
}
...
function decodeReferences(decision, result) {
  ...
  if (!scope) block("missing_model_privacy_scope");
  // Detect raw identifiers anywhere, before resolving even one argument/tool.
  assertVerified(decision, scope);
  const decoded = structuredClone(decision);
  for (const entity of decoded.resolvedEntities || []) entity.internalId = scope.resolve(entity.internalId, modelReferenceType("internalId", entity));
  for (const call of decoded.proposedToolCalls || []) {
    for (const [key, value] of Object.entries(call.arguments || {})) {
      const type = modelReferenceType(key, call.arguments, call.tool);
      if (type) call.arguments[key] = scope.resolve(value, type);
      else if (typeof value === "string" && [...value.matchAll(ALIAS)].length) block("model_reference_wrong_position");
    }
  }
  for (const claim of decoded.factualClaims || []) claim.evidenceIds = (claim.evidenceIds || []).map((id) => scope.resolve(id, "evidence"));
  if (decoded.conversationalResponseParts) decoded.conversationalResponseParts.verifiedFactReferences = (decoded.conversationalResponseParts.verifiedFactReferences || []).map((id) => scope.resolve(id, "evidence"));
