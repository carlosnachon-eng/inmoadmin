...
import { continueShadowAiStateMachine, startShadowAiStateMachine, terminateAwaitingShadowAiRun } from "./stateMachine.js";
import { SHADOW_AI_LIMITS } from "./guards.js";
import { supersedeConversationActionsForHumanResponses } from "./conversationAction.js";
import { reconcilePendingRespondOutgoingOrigins } from "../outboundOrigin.js";

export const AUTO_REAL_MAX_TURNS_PER_INVOCATION = 1;
export const AUTO_REAL_LOOKBACK_DAYS_MAX = 5;
export const AUTO_REAL_MAX_IMMEDIATE_CONTINUATIONS = 2;
...
const runOptions = (env, turn, inputMode) => ({
  ...
  idempotencyIdentity: `turn:${turn.turnKey}`, turnMetadata: { turnKey: turn.turnKey, messageIds: turn.messageIds, humanResponseId: turn.humanResponseId, closedReason: turn.closedReason, mediaSettlement: turn.mediaSettlement || null },
});

export function immediateContinuationGate({ turn, expectedTurnKey, deadlineAt, now = Date.now(), minimumBudgetMs = SHADOW_AI_LIMITS.minimumAnthropicRetryBudgetMs } = {}) {
  if (!turn) return { allowed: false, reason: "turn_no_longer_current" };
  if (turn.turnKey !== expectedTurnKey) return { allowed: false, reason: "new_inbound_before_next_round" };
  if (turn.humanResponseId || turn.closedReason === "human_response") return { allowed: false, reason: "human_response_before_next_round" };
  if (turn.mediaSettlement?.status === "waiting") return { allowed: false, reason: "media_settlement_changed" };
  const deadlineMs = Date.parse(String(deadlineAt || ""));
  if (!Number.isFinite(deadlineMs) || deadlineMs - now < minimumBudgetMs) return { allowed: false, reason: "insufficient_round_budget", timeout: true };
  return { allowed: true, remainingBudgetMs: deadlineMs - now };
}

async function continueAutoRealImmediately(admin, initial, { env, lookbackDays, inputMode, turnKey }) {
  let result = initial;
  for (let index = 0; index < AUTO_REAL_MAX_IMMEDIATE_CONTINUATIONS && result?.status === "awaiting_model_round"; index += 1) {
    const refreshed = await loadAutoRealTurns(admin, { lookbackDays, env, inputMode, now: Date.now() });
    await supersedeConversationActionsForHumanResponses(admin, refreshed.messages, env);
    const turn = refreshed.turns.find((item) => item.runId === result.runId);
    const gate = immediateContinuationGate({ turn, expectedTurnKey: turnKey, deadlineAt: result.telemetry?.deadline_at, now: Date.now(), minimumBudgetMs: Number(env.SHADOW_AI_MINIMUM_RETRY_BUDGET_MS || SHADOW_AI_LIMITS.minimumAnthropicRetryBudgetMs) });
    if (!gate.allowed) return terminateAwaitingShadowAiRun(admin, result.runId, { reason: gate.reason, timeout: gate.timeout });
    result = await continueShadowAiStateMachine(admin, result.runId, runOptions(env, turn, inputMode));
...
export async function loadAutoRealTurns(admin, { lookbackDays = AUTO_REAL_LOOKBACK_DAYS_MAX, env = process.env, now = Date.now(), inputMode = "auto_real_shadow" } = {}) {
  ...
  const [{ data: messages, error: messageError }, { data: conversations, error: conversationError }, { data: runs, error: runError }, { data: mediaInterpretations, error: mediaError }, { data: mediaRetrievals, error: retrievalError }] = await Promise.all([
    ...
    admin.from("shadow_media_interpretations").select("external_message_id,status,result_safe,interpreted_at").gte("created_at",since).limit(1000),
    admin.from("shadow_media_retrieval_queue").select("external_message_id,status,completed_at,created_at").gte("created_at",since).limit(1000),
  ]);
  if (messageError || conversationError || runError || mediaError || retrievalError) throw messageError || conversationError || runError || mediaError || retrievalError;
  const model = env.SHADOW_AI_MODEL || DEFAULT_SHADOW_AI_MODEL;
  const turns = buildRealShadowConversationTurns({ messages: messages || [], conversations: conversations || [], mediaInterpretations:mediaInterpretations||[], mediaRetrievals:mediaRetrievals||[], env, now });
  const mapped = turns.map((turn) => {
    ...
    const disposition = !latest && turn.mediaSettlement?.status === "waiting" ? "wait_media" : autoRealRunDisposition(latest);
...
export async function processNextAutoRealTurn(admin, { env = process.env, lookbackDays = AUTO_REAL_LOOKBACK_DAYS_MAX, inputMode = "auto_real_shadow", reconcileOrigins = reconcilePendingRespondOutgoingOrigins } = {}) {
  assertAutoRealEnvironment(env, { mode: inputMode === "backfill_real_shadow" ? "backfill" : "auto" });
  await reconcileOrigins(admin);
  const loaded = await loadAutoRealTurns(admin, { lookbackDays, env, inputMode });
  await supersedeConversationActionsForHumanResponses(admin, loaded.messages, env);
  const awaiting = loaded.turns.find((turn) => turn.disposition === "block_running" && turn.runState === "awaiting_model_round");
  if (awaiting) {
    const first = await continueShadowAiStateMachine(admin, awaiting.runId, runOptions(env, awaiting, inputMode));
    return { ...(await continueAutoRealImmediately(admin, first, { env, lookbackDays, inputMode, turnKey: awaiting.turnKey })), observability: loaded.observability };
  }
  ...
  const started = await startAutoRealTurn(admin, pending, conversation, env, inputMode);
  const result = await continueAutoRealImmediately(admin, started, { env, lookbackDays, inputMode, turnKey: pending.turnKey });
