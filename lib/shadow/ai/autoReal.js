...
export async function loadAutoRealTurns(admin, { lookbackDays = AUTO_REAL_LOOKBACK_DAYS_MAX, env = process.env, now = Date.now() } = {}) {
  ...
  const [{ data: messages, error: messageError }, { data: conversations, error: conversationError }, { data: runs, error: runError }] = await Promise.all([
    ...
    admin.from("shadow_ai_runs").select("id,message_id,status,execution_state,current_round,model,prompt_version,telemetry_json,created_at").eq("prompt_version", REAL_SHADOW_AI_PROMPT_VERSION).order("created_at", { ascending: false }).limit(1000),
  ]);
  ...
  const eligible = turns.map((turn) => {
    ...
    return { ...turn, disposition, runId: latest?.id || null, runState: latest?.execution_state || null, currentRound: Number(latest?.current_round || 0) };
  });
