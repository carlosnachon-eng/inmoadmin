...
export const priorConversationActor = (direction) => direction === "inbound" ? "contact" : direction === "outbound_human" ? "emporio_human" : null;
...
export function plannableShadowToolCalls(decision) {
  return (decision?.proposedToolCalls || []).filter((call) => !(call?.tool === "find_properties" && (
    (Object.hasOwn(call?.arguments || {}, "propertyReference") && !String(call.arguments.propertyReference || "").trim())
    || (Object.hasOwn(call?.arguments || {}, "propertyId") && !String(call.arguments.propertyId || "").trim())
  )));
}
...
export function applyActorRoleGuards(decision, envelope = {}, tools = []) {
  ...
  if (invalidPropertyPlan) {
    next.proposedToolCalls = plannableShadowToolCalls(next);
    next.safetyFlags.push("invalid_property_reference_plan_blocked");
  }
