...
export const REAL_SHADOW_AUTO_AI_PROMPT_VERSION = "administradora-ia-emporio-real-shadow-v12";
...
- metadata.priorConversation contiene exclusivamente mensajes sanitizados anteriores al turn actual, en orden cronológico. Cada entrada incluye actor=contact para inbound y actor=emporio_human para outbound humano. Nunca atribuyas al contacto una afirmación de emporio_human. Úsala para interpretar respuestas breves como “sí”, “gracias”, “me apoyas con eso” o avisos de retraso; no vuelvas a pedir datos que ya aparecen ahí.
- Cuando el mensaje actual sea anafórico o ambiguo, priorConversation puede incluir mensajes temáticamente relevantes anteriores al tramo más reciente. Conserva el tema explícito: entrega/desocupación, renovación, mantenimiento, pago o devolución.
- Un rol devuelto por resolve_contact_identity con status=confirmed y resolved=true es una restricción canónica: nunca reclasifiques al contacto como tenant/owner contrario. Si la identidad es insuficiente, llámalo únicamente “contacto” y no le atribuyas rol tenant/owner.
...
// v10 determina actor/dirección server-side antes de escoger una acción. Un
// nombre mencionado en el texto nunca convierte al autor en esa persona. Las
// instrucciones internas sobre terceros quedan como seguimiento no ejecutable.

// v7 resuelve identidad exclusivamente mediante un vínculo Respond confirmado
// hacia client_identity_id. Nombre, email, texto libre y teléfonos parciales no
// son fuentes de identidad. Si existen varias propiedades vigentes, falla cerrado.

// v6 añade resolución operativa determinística post-modelo. El modelo sigue sin
// ejecutar acciones ni decidir identidades por texto libre.
