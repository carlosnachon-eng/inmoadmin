...
const MAINTENANCE = /\b(?:humedad|fuga|gotera|filtracion|agua|inundad[oa]|inundacion|moho|desperfecto|falla|dan[oa]|no funciona|cambi(?:e|o|ar).{0,30}llave)\b/;
...
export function deriveIdentityIndependentContext({ caseDomain, text, documentPresent = false, sensitiveComponent = false, missingInformation = [] } = {}) {
  ...
  if (caseDomain === "maintenance" && MAINTENANCE.test(value) && documentPresent && sensitiveComponent) return result({
    allowed: true, action: "acknowledge_received_information", kind: "maintenance_evidence_received",
    confidence: 0.9, blocker: null, sensitive_internal_handoff: true,
  });
...
export function validateIdentityIndependentMessage(message, context) {
  ...
  if (context.kind === "maintenance_evidence_received") {
    if (!/\b(?:reporte|imagen|comprobante|fuga|inundacion|pieza|llave|flujo|agua)\b/.test(text)) return { allowed: false, reason: "maintenance_evidence_not_acknowledged" };
    if (/\b(?:reembolso|pagar|paga|pagara|responsabilidad|autoriza|aprobado|procedente)\b/.test(text)) return { allowed: false, reason: "sensitive_maintenance_conclusion" };
  }
