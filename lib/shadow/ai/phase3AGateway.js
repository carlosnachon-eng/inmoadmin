...
const allowlistedToolRow = (row = {}) => {
  ...
  // Extend only the new source; legacy 7/7 model context remains byte-for-byte unchanged.
  ...
  if (condominiumIdentity || condominiumUnit) {
    ...
    if (condominiumUnit) {
      const condominiumId = boundedReference(row.condominiumId);
...
export async function invokeShadowPhase3A({ admin, envelope, deterministic, toolResults = [], round = 0, systemPrompt, toolGuide, modelCall = createAnthropicShadowResponse, modelOptions = {} }) {
