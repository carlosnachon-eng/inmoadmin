...
export async function createAnthropicShadowResponse(messages, { signal, fetchImpl = fetch, env = process.env, onPhase = () => {}, outputMode = shadowAiOutputMode(env) } = {}) {
  ...
  onPhase("complete");
  return recordModelPrivacyReceipt({ id: json.id, text: (json.content || []).find((block) => block.type === "text")?.text, usage: json.usage || {}, model: json.model || env.SHADOW_AI_MODEL || DEFAULT_SHADOW_AI_MODEL, outputMode }, privacyReceipt());
  } catch (error) {
