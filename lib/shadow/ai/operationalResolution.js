...
import { isConversationalClose } from "./conversationClosure.js";
...
export function buildShadowOperationalResolution({ decision, envelope, tools = [] } = {}) {
  ...
  if (isConversationalClose(envelope)) assessment = { status: "no_message", missing: [], conflict: false };
  else if (caseDomain !== "outside_phase_3a" && !hasIdentity) assessment = { status: "insufficient_identity_context", missing: ["trusted_identity_link"], conflict: false };
