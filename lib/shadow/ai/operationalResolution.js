...
export const SHADOW_OPERATIONAL_RESOLUTION_VERSION = "shadow-operational-resolution-v4";
...
export function buildShadowOperationalResolution({ decision, envelope, tools = [] } = {}) {
  ...
  const requiresHuman = caseDomain === "outside_phase_3a" || !hasIdentity || failed || assessment.conflict || insufficient || actorRequiresHuman
    || caseDomain === "payment" || caseDomain === "property_handover" || assessment.status === "possible_duplicate" || Boolean(decision?.responseBlocked);
