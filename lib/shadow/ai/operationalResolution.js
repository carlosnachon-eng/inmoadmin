...
const hasSensitiveMaintenanceEvidence = (envelope) => completedInterpretations(envelope).some((item) =>
  item?.category === "possible_payment_receipt" || (
    item?.extractedFields?.amount !== null &&
    item?.extractedFields?.amount !== undefined &&
    item?.extractedFields?.amount !== "" &&
    Number.isFinite(Number(item.extractedFields.amount))
  )
);
...
const serviceTypeFromText = (text) => {
  const value = String(text || "");
  if (/\b(?:cfe|luz|electricidad|el[eé]ctrico)\b/i.test(value)) return "electricity";
  if (/\bagua\b/i.test(value)) return "water";
  if (/\bgas\b/i.test(value)) return "gas";
  if (/\binternet\b/i.test(value)) return "internet";
  return null;
};

function conversationObjectContext({ decision, envelope, rows }) {
  const text = String(envelope?.sanitizedText || "");
  const serviceType = serviceTypeFromText(text) || envelope?.providerMetadata?.serviceType || envelope?.providerMetadata?.service || null;
  const asksWhetherReceived = /\b(?:ya|si|s[ií])\b[\s\S]{0,50}\b(?:lleg[oó]|recibieron|recibiste|recibido|tienen|habr[aá]\s+llegado)\b/i.test(text)
    || /\b(?:lleg[oó]|recibieron|recibiste|recibido|tienen|habr[aá]\s+llegado)\b[\s\S]{0,50}\brecibo\b/i.test(text);
  const mentionsServiceReceipt = /\brecibo\b/i.test(text) && Boolean(serviceType);
  const customerProvidesDocument = paymentDocumentMentioned(text);
  const objectType = mentionsServiceReceipt ? "service_receipt" : customerProvidesDocument ? "customer_provided_document" : null;
  return {
    customer_intent: mentionsServiceReceipt && asksWhetherReceived ? "verify_service_receipt_received" : null,
    object_type: objectType,
    service_type: serviceType,
    customer_is_providing_document: customerProvidesDocument,
    customer_is_being_asked_for_document: false,
    verified_service_evidence_available: rows.some((row) => row.entityType === "service" && row.evidenceId),
  };
}
...
export function buildShadowOperationalResolution({ decision, envelope, tools = [] } = {}) {
  ...
  const identityIndependentContext = !hasIdentity && interactionDirection === "inbound_customer_action"
    ? deriveIdentityIndependentContext({ caseDomain, text: envelope?.sanitizedText, documentPresent: documents.equivalent_document_present, sensitiveComponent: hasSensitiveMaintenanceEvidence(envelope), missingInformation: effectiveMissing })
  ...
  if (resolvedIdentity?.relationshipCurrent === false && assessment.status === "insufficient_property_context") {
    assessment.status = resolvedIdentity.failClosedReason || assessment.status;
    assessment.missing = [assessment.status];
  }
  ...
  return {
    ...
    identity_context: { status: hasIdentity ? (hasOperationalEntity ? "trusted_link_available" : (resolvedIdentity?.failClosedReason || "insufficient_property_context")) : "insufficient_identity_context", client_identity_id: resolvedIdentity?.canonicalContactId || context.clientIdentityId || null, canonical_contact_id: resolvedIdentity?.canonicalContactId || context.clientIdentityId || null, identity_confirmed: resolvedIdentity?.identityConfirmed === true, roles: resolvedIdentity?.roles || [], relationship_current: resolvedIdentity?.relationshipCurrent === true, evidence_level: resolvedIdentity?.evidenceLevel || null, evidence_sources: resolvedIdentity?.evidenceSources || [], fail_closed_reason: resolvedIdentity?.failClosedReason || null, identifier_keys: (identityKeys[caseDomain] || []).filter((key) => Boolean(context[key])) },
    conflict_detected: assessment.conflict,
    technical_error: failed,
    document_context: documents,
    conversation_object_context: conversationObjectContext({ decision, envelope, rows }),
    ...
    } : identityIndependentContext?.sensitive_internal_handoff === true ? {
