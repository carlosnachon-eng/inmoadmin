...
export function adminOutboundCapabilities(env = process.env) {
  ...
  return { enabled: env.SHADOW_ADMIN_OUTBOUND_ENABLED === "true", globalOutboundEnabled: env.SHADOW_OUTBOUND_ENABLED === "true", channelId: clean(env.SHADOW_RESPOND_ADMIN_CHANNEL_ID || SHADOW_ADMIN_CHANNEL_ID, 40), notBefore: Number.isFinite(notBeforeMs) ? new Date(notBeforeMs).toISOString() : null };
}
...
export function validateAdminOutboundClaim(claim, { now = Date.now(), notBefore = null } = {}) {
...
async function defaultClaim(admin, workerId, notBefore) {
  const { data, error } = await admin.rpc("claim_shadow_admin_outbound", { p_worker_id: workerId, p_not_before: notBefore });
...
export async function processOneAdminOutbound(admin, { env = process.env, workerId = `admin-outbound-${Date.now()}`, claim = defaultClaim, send = sendRespondAdminText, recheck = recheckNoNewMessage, reconcileOrigin = reconcilePendingRespondOutgoingOrigins } = {}) {
  ...
  const item = await claim(admin, workerId, capability.notBefore);
  if (!item) return { status: "no_work" };
  const validation = validateAdminOutboundClaim(item, { notBefore: capability.notBefore });
