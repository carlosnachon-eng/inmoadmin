...
export const SHADOW_ADMIN_CANARY_ACTION = "acknowledge_received_information";
export const SHADOW_ADMIN_CANARY_MESSAGE = "Gracias, recibí la información que compartiste.";
...
export function adminOutboundCapabilities(env = process.env) {
  const notBeforeRaw = clean(env.SHADOW_ADMIN_OUTBOUND_NOT_BEFORE, 80);
  const notBeforeMs = Date.parse(notBeforeRaw);
  return {
    enabled: env.SHADOW_ADMIN_OUTBOUND_ENABLED === "true",
    globalOutboundEnabled: env.SHADOW_OUTBOUND_ENABLED === "true",
    r1Enabled: env.SHADOW_ADMIN_WORK_R1_ENABLED === "true",
    channelId: clean(env.SHADOW_RESPOND_ADMIN_CHANNEL_ID || SHADOW_ADMIN_CHANNEL_ID, 40),
    notBefore: Number.isFinite(notBeforeMs) ? new Date(notBeforeMs).toISOString() : null,
    canaryEnabled: env.SHADOW_ADMIN_OUTBOUND_CANARY_ENABLED === "true",
    canaryId: clean(env.SHADOW_ADMIN_OUTBOUND_CANARY_ID, 80) || null,
  };
}
...
export function assertAdminOutboundEnvironment(env = process.env) {
  ...
  if (!state.notBefore) throw Object.assign(new Error("admin_outbound_not_before_invalid"), { statusCode: 409 });
  if (state.canaryEnabled && state.r1Enabled) throw Object.assign(new Error("admin_canary_r1_must_remain_disabled"), { statusCode: 409 });
  if (state.canaryEnabled && !UUID.test(state.canaryId || "")) throw Object.assign(new Error("admin_canary_id_invalid"), { statusCode: 409 });
  if (!state.canaryEnabled && state.canaryId) throw Object.assign(new Error("admin_canary_mode_mismatch"), { statusCode: 409 });
...
export function validateAdminOutboundClaim(claim, { now = Date.now(), notBefore = null, canaryEnabled = false, canaryId = null } = {}) {
  ...
  if (!notBefore || !claim.action_created_at || Date.parse(claim.action_created_at) < Date.parse(notBefore)) return { allowed: false, reason: "action_before_activation" };
  if (canaryEnabled) {
    if (!canaryId || claim.canary_id !== canaryId) return { allowed: false, reason: "canary_claim_mismatch" };
    if (claim.canary_content_eligible !== true) return { allowed: false, reason: "canary_content_not_eligible" };
    if (!claim.anchor_occurred_at || Date.parse(claim.anchor_occurred_at) < Date.parse(notBefore)) return { allowed: false, reason: "canary_inbound_before_activation" };
    if (claim.conversation_action !== SHADOW_ADMIN_CANARY_ACTION) return { allowed: false, reason: "canary_action_not_allowlisted" };
    if (claim.case_domain !== "administrative_pending") return { allowed: false, reason: "canary_domain_not_allowlisted" };
    if (message !== SHADOW_ADMIN_CANARY_MESSAGE) return { allowed: false, reason: "canary_message_not_neutral_acknowledgement" };
  }
...
async function defaultClaim(admin, workerId, notBefore, canaryId = null) {
  const rpc = canaryId ? "claim_shadow_admin_outbound_canary" : "claim_shadow_admin_outbound";
  const params = canaryId
    ? { p_worker_id: workerId, p_canary_id: canaryId, p_not_before: notBefore }
    : { p_worker_id: workerId, p_not_before: notBefore };
...
export async function processOneAdminOutbound(admin, { env = process.env, workerId = `admin-outbound-${Date.now()}`, claim = defaultClaim, send = sendRespondAdminText, recheck = recheckNoNewMessage, reconcileOrigin = reconcilePendingRespondOutgoingOrigins } = {}) {
  const capability = assertAdminOutboundEnvironment(env);
  const item = await claim(admin, workerId, capability.notBefore, capability.canaryEnabled ? capability.canaryId : null);
  if (!item) return { status: "no_work" };
  const validation = validateAdminOutboundClaim(item, {
    notBefore: capability.notBefore,
    canaryEnabled: capability.canaryEnabled,
    canaryId: capability.canaryId,
  });
  ...
  try {
    ...
    let originSyncError = false;
    try { await reconcileOrigin(admin, { providerMessageId: result.providerMessageId }); }
    catch { originSyncError = true; }
    ...
    return { id: item.outbound_id, status: "sent", providerMessageId: result.providerMessageId, actionSyncError: Boolean(error), originSyncError };
  } catch (error) {
