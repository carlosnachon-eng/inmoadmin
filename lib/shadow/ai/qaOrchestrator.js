...
export function validateQaCampaignId(value) {
  ...
  if (!/^p3-[a-z0-9]+(?:-[a-z0-9]+)*$/.test(campaignId) || campaignId.length > 80) throw new Error("invalid_campaign_id");
  return campaignId;
}
...
export function aggregatePersistedShadowQa({ messages = [], runs = [], decisions = [] }, { model = DEFAULT_SHADOW_AI_MODEL, promptVersion = SHADOW_AI_PROMPT_VERSION, campaignId = null } = {}) {
  ...
  const metrics = evaluateShadowAiQa(SHADOW_AI_QA_DATASET, results);
  ...
  return { campaignId, model, promptVersion, results, missingFixtures, completed: results.filter((item) => item.status === "completed").length, metrics };
}
