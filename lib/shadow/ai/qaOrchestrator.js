...
import { SHADOW_AI_QA_DATASET, evaluateShadowAiQa } from "./qaDataset.js";

export const SHADOW_QA_MAX_MICRO_BATCH = 4;
export const SHADOW_QA_REQUEST_BUDGET_MS = 110000;
export const SHADOW_QA_RESERVE_MS = 8000;
export const SHADOW_QA_MIN_RUN_BUDGET_MS = 15000;
const fixtureSet = new Set(SHADOW_AI_QA_DATASET.map((item) => item.id));
...
export function executionDisposition(status) {
  if (status === "completed") return "skip_completed";
  if (status === "running") return "block_running";
  if (["error", "timeout"].includes(status)) return "report_failed_no_retry";
  return "execute";
}
