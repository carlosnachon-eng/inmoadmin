export const SHADOW_CONVERSATION_ACTION_VERSION = "shadow-conversation-action-v2";
...
function chooseAction(resolution, decision) {
  ...
  if (resolution?.identity_context?.status !== "trusted_link_available") {
    if (sensitiveReason(resolution, decision)) return "human_handoff";
    const actionableMissing = [...missing].filter((item) => item !== "trusted_identity_link" && !(documentPresent(resolution) && item === "attachment_interpretation"));
    if (documentPresent(resolution) && actionableMissing.some((item) => /document|attachment|payment_document/i.test(item))) return "acknowledge_received_information";
    if (actionableMissing.length === 1) return "ask_missing_information";
...
export function conversationActionFutureEligibility({ action, resolution, decision = {} } = {}) {
  ...
  if (resolution?.identity_context?.status !== "trusted_link_available") return { eligible: false, reason: "identity_not_confirmed" };
...
export function buildConversationAction({ resolution, decision = {}, turn = {}, now = Date.now() } = {}) {
  ...
  const sensitive = Boolean(resolution?.conflict_detected || resolution?.technical_error || sensitiveReason(resolution, decision)
    || Number(resolution?.action_confidence || 0) < 0.65);
  const candidate = validateConversationAction({
    ...
    missing_information: [...new Set(resolution?.missing_information || [])].slice(0, 10),
  });
