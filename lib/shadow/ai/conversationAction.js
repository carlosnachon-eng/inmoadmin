...
function chooseAction(resolution, decision) {
  ...
  if (domain === "payment") {
    ...
    if ((missing.has("payment_document") || resolution.case_status === "record_not_found") && !documentPresent(resolution)) return "request_document";
...
export function conversationActionFutureEligibility({ action, resolution, decision = {} } = {}) {
  ...
  if (action.conversation_action === "request_document" && documentPresent(resolution)) return { eligible: false, reason: "equivalent_document_already_present" };
