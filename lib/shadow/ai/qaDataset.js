...
export function evaluateShadowAiQa(dataset, results) {
  ...
  const expectedTools = completedRows.flatMap(({ scenario }) => scenario.golden.expectedTools);
  const predictedTools = completedRows.flatMap(({ result }) => (result?.tools || []).map((tool) => tool.name));
  const truePositiveTools = completedRows.reduce((count, { scenario, result }) => count + (result?.tools || []).filter((tool) => scenario.golden.expectedTools.includes(tool.name)).length, 0);
