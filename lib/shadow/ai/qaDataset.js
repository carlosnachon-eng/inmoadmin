...
export function evaluateShadowAiQa(dataset, results) {
  ...
  return {
    ...
    averageRoundsPerRun: rows.length ? rows.reduce((sum,{result})=>sum+Number(result?.rounds||0),0)/rows.length : 0,
    latencyMsP50: percentile(latencies, 0.5),
    latencyMsP95: percentile(latencies, 0.95),
    inputTokens,
    outputTokens,
    estimatedCostUsd,
    averageCostUsd: rows.length ? estimatedCostUsd / rows.length : 0,
  };
}
