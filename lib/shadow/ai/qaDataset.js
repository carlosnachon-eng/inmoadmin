const qa = (id, text, intent, expectedTools = [], requiresHuman = false, metadata = {}, toolSequence = [], toolTiming = {}) => {
  ...
  return {
    ...
    golden: { intent, expectedTools, toolSequence, requiredNowTools, expectedAfterClarificationTools,
      notApplicableTools: toolTiming.notApplicableTools || [], forbiddenTools: [], requiresHuman, requiredResponse: [], forbiddenResponse: ["ya quedó", "autorizado", "hemos pagado", "voy a revisar"] },
  };
};
export const SHADOW_AI_QA_DATASET = [
  ...
  qa(25,"Necesito que me presten las llaves","llaves",["get_key_custody_status"],true,{},[],{requiredNowTools:[],expectedAfterClarificationTools:["get_key_custody_status"]}),
...
export function evaluateShadowAiQa(dataset, results) {
  ...
  const promisePattern = /\b(?:(?:puedo|podr[eé]) (?:revisar|canalizar)|para proceder|vamos a gestionar|lo registrar[eé])(?=\s|[.,;:!?]|$)/i;
  const nonEscalationRows = completedRows.filter(({scenario})=>!scenario.golden.requiresHuman);
  const entityRows = completedRows.filter(({ scenario }) => Boolean(scenario.metadata?.propertyReference));
