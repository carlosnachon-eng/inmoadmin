...
export const SHADOW_AI_QA_DATASET = [
  qa(1,"Sigue la fuga en la casa de Montpellier","mantenimiento",["find_properties","get_maintenance_ticket_summary"],true,{propertyReference:"FASE2A-P3-QA Montpellier 101"},[["find_properties"],["get_maintenance_ticket_summary"],[]],resolved("property",IDS.property)),
  qa(2,"El técnico no llegó a la casa de Montpellier","mantenimiento",["find_properties","get_maintenance_ticket_summary"],false,{propertyReference:"FASE2A-P3-QA Montpellier 101"},[["find_properties"],["get_maintenance_ticket_summary"],[]],resolved("maintenance_ticket",IDS.ticket)),
  ...
  qa(11,"Ya te mandé lo del agua","servicio",["find_properties","get_service_period_status"],false,{propertyReference:"FASE2A-P3-QA Montpellier 101",serviceId:IDS.serviceWater},[["find_properties"],["get_service_period_status"],[]],resolved("service",IDS.serviceWater)),
  ...
  qa(21,"¿Cuándo vence mi contrato?","contrato",["find_properties","find_active_contracts"],false,{propertyReference:"FASE2A-P3-QA Montpellier 101"},[["find_properties"],["find_active_contracts"],[]],resolved("contract",IDS.contract)),
...
export function evaluateShadowAiQa(dataset, results) {
  ...
  const expectedTools = completedRows.flatMap(({ scenario }) => scenario.golden.requiredNowTools || scenario.golden.expectedTools);
  const predictedTools = completedRows.flatMap(({ result }) => (result?.tools || []).map((tool) => tool.name));
  const truePositiveTools = completedRows.reduce((count, { scenario, result }) => count + (result?.tools || []).filter((tool) => (scenario.golden.requiredNowTools || scenario.golden.expectedTools).includes(tool.name)).length, 0);
  ...
  const prematureTools = completedRows.reduce((count,{scenario,result})=>count+(result.tools||[]).filter((tool)=>(scenario.golden.expectedAfterClarificationTools||[]).includes(tool.name)).length,0);
  ...
  const entityMatches = ({ scenario, result }) => (result.decision.resolvedEntities || []).some((entity) =>
  ...
  return {
    ...
    prematureToolRate: predictedTools.length ? prematureTools / predictedTools.length : 0,
    executionPromiseRate: qualityRatio(({result})=>promisePattern.test(`${result.decision.proposedAction} ${result.decision.proposedResponse}`)),
    ...
    hallucinationRate: qualityRatio(({result}) => (result.decision.safetyFlags || []).some((flag) => ["hallucination","unsupported_erp_fact"].includes(flag))),
    ...
    averageToolCallsPerRun: completedRows.length ? predictedTools.length / completedRows.length : 0,
