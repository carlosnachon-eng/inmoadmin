...
export async function executeHistoricalReplayCase(admin, replayCase, { env = process.env, modelCall, executeTool = executeShadowReadOnlyTool, now = () => Date.now() } = {}) {
  ...
  return {
    ...
    error.historicalReplayTelemetry = {
      ...
      estimatedCostUsd: costUsd(inputTokens, outputTokens), latencyMs: now() - started,
      outputStage: privacyFailure ? "final_model_privacy" : error.outputStage || "post_provider_validation",
      diagnosticCode: privacyFailure || clean(error.diagnosticCode || error.message || "historical_replay_error", 80),
