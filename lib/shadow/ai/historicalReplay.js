...
import { decodeModelDecisionReferences, invokeShadowPhase3A } from "./phase3AGateway.js";
...
export async function executeHistoricalReplayCase(admin, replayCase, { env = process.env, modelCall, executeTool = executeShadowReadOnlyTool, now = () => Date.now() } = {}) {
  ...
  try { for (let round = 0; round < 2; round += 1) {
    ...
    let result;
    awaitingResult = true;
    try { result = await invokeShadowPhase3A({ admin, envelope: replayCase.envelope, deterministic, toolResults: tools, round,
      systemPrompt: REAL_SHADOW_AI_SYSTEM_PROMPT, toolGuide: REAL_SHADOW_AI_TOOL_GUIDE, modelCall, modelOptions: { env },
    }); } catch (error) {
    ...
    decision = decodeModelDecisionReferences(validateShadowAiDecision(normalized.value), result);
