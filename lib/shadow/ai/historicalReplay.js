...
import { classifyExplicitShadowAiIntent, finalizeShadowAiDecision, requestedShadowAiCalls, shadowAiDependencyError } from "./runner.js";
...
import { decodeModelDecisionReferences, invokeShadowPhase3A, invokeHistoricalReplayReducedPhase3A } from "./phase3AGateway.js";
import { decodeReducedShadowAiDecision } from "./reducedOutputSchema.js";
import { shadowAiOutputMode } from "./anthropic.js";
import { withHistoricalReplaySchemaContext } from "./historicalReplaySchemaContext.js";
import { storedHistoricalReplayConversationResult } from "./historicalReplayResult.js";
import { modelPrivacyReceipt, sanitizedModelPrivacyChecks } from "./modelPrivacyTelemetry.js";
import { projectOutputPrivacyFailure } from "./outputPrivacyDiagnostics.js";
import { projectProviderHttpError, replayProviderUsage, reportedTokenCount } from "./providerHttpDiagnostics.js";

export const HISTORICAL_REPLAY_RUNTIME = "administradora-ia-emporio-historical-replay-v3";
...
export function selectHistoricalReplayCohort({ messages = [], conversations = [], mediaInterpretations = [], env = process.env, now = Date.now(), selectedTurnKeys = null, occurredSince = null } = {}) {
  ...
  for (const turn of turns.slice().reverse()) {
    if (occurredSince && Date.parse(turn.lastInboundAt) < Date.parse(occurredSince)) continue;
...
function replayNoMessageForUnsupportedDomain(resolution, now) {
  return {
    ...
    interaction_direction: resolution?.interaction_direction || "ambiguous_actor", operational_follow_up: resolution?.operational_follow_up || null,
...
export async function executeHistoricalReplayCase(admin, replayCase, options = {}) {
  const { env = process.env, useReducedOutputSchema = false } = options;
  ...
  if (typeof useReducedOutputSchema !== "boolean") throw new Error("invalid_historical_replay_schema_option");
  if (useReducedOutputSchema && shadowAiOutputMode(env) !== "anthropic_json_schema") throw new Error("historical_replay_reduced_schema_requires_json_schema");
  return useReducedOutputSchema
    ? withHistoricalReplaySchemaContext((context) => executeReplayInContext(admin, replayCase, options, context))
    : executeReplayInContext(admin, replayCase, options, null);
}

async function executeReplayInContext(admin, replayCase, { env = process.env, modelCall, fetchImpl, useReducedOutputSchema = false, executeTool = executeShadowReadOnlyTool, now = () => Date.now() }, replaySchemaContext) {
  ...
  const privacyChecks = [];
  let receivedResult = false, awaitingResult = false, missingModel = false;
  const modelStatus = () => !providerModels.length ? "unaccredited" : missingModel ? "partial" : "reported";
  try { for (let round = 0; round < 2; round += 1) {
    const deterministic = { intent: classifyExplicitShadowAiIntent(replayCase.envelope.sanitizedText), interactionDirection: classifyInteractionDirection({ envelope: replayCase.envelope }), requiresHuman: false, reasonCodes: [] };
    let result;
    awaitingResult = true;
    const invoke = useReducedOutputSchema ? invokeHistoricalReplayReducedPhase3A : invokeShadowPhase3A;
    try { result = await invoke({ admin, replayCase, replaySchemaContext, envelope: replayCase.envelope, deterministic, toolResults: tools, round,
      systemPrompt: REAL_SHADOW_AI_SYSTEM_PROMPT, toolGuide: REAL_SHADOW_AI_TOOL_GUIDE, modelCall, modelOptions: { env, ...(fetchImpl ? { fetchImpl } : {}) },
    }); } catch (error) {
      const receipt = modelPrivacyReceipt(error);
      if (receipt) privacyChecks.push(receipt);
      else if (error.code === "pre_model_sanitization_blocked") privacyChecks.push({
        privacy_stage: "final_model_privacy", privacy_failure_code: "pre_transport_privacy_blocked", provider_invoked: false,
      });
      throw error;
    }
    const receipt = modelPrivacyReceipt(result);
    if (receipt) privacyChecks.push(receipt);
    receivedResult = true; awaitingResult = false;
    if (result.id) requestIds.push(opaque(clean(result.id, 80)));
    // Native transport explicitly distinguishes the returned model from its
    // legacy config fallback. Injected test adapters may report result.model.
    const reportedModel = clean(result.reportedModel !== undefined ? result.reportedModel : result.model, 80);
    if (reportedModel) providerModels.push(reportedModel); else missingModel = true;
    const input = reportedTokenCount(result.usage?.input_tokens), output = reportedTokenCount(result.usage?.output_tokens);
    inputTokens = inputTokens === null || input === null ? null : inputTokens + input;
    outputTokens = outputTokens === null || output === null ? null : outputTokens + output;
    let parsed;
    ...
    decision = useReducedOutputSchema ? decodeReducedShadowAiDecision(normalized.value, result)
      : decodeModelDecisionReferences(validateShadowAiDecision(normalized.value), result);
  ...
  return {
    ...
    evidence: resolution.evidence || [], providerRequestRefs: requestIds, providerModels: [...new Set(providerModels)], providerModelStatus: modelStatus(), inputTokens, outputTokens,
    providerUsage: replayProviderUsage(inputTokens, outputTokens),
    estimatedCostUsd: replayProviderUsage(inputTokens, outputTokens).estimated_cost_usd, latencyMs: now() - started,
    ...
    privacyChecks: sanitizedModelPrivacyChecks(privacyChecks),
  };
  } catch (error) {
    const outputPrivacy = projectOutputPrivacyFailure(error);
    const privacyFailure = privacyChecks.at(-1)?.privacy_failure_code;
    const providerHttp = projectProviderHttpError(error.providerError);
    if (providerHttp?.provider_request_ref) requestIds.push(providerHttp.provider_request_ref);
    // A failed HTTP round has no usage/model response. Earlier successful
    // rounds cannot make the total known or accredit the failed round's model.
    if (!receivedResult || (awaitingResult && !privacyFailure)) { inputTokens = null; outputTokens = null; missingModel = true; }
    error.historicalReplayTelemetry = {
      providerRequestRefs: [...new Set(requestIds)], providerModels: [...new Set(providerModels)], providerModelStatus: modelStatus(), inputTokens, outputTokens,
      providerUsage: replayProviderUsage(inputTokens, outputTokens), providerHttp,
      estimatedCostUsd: replayProviderUsage(inputTokens, outputTokens).estimated_cost_usd, latencyMs: now() - started,
      outputStage: outputPrivacy?.outputStage || (privacyFailure ? "final_model_privacy" : providerHttp ? "provider_http" : error.outputStage || "post_provider_validation"),
      ...(outputPrivacy?.outputPrivacy ? { outputPrivacy: outputPrivacy.outputPrivacy } : {}),
      diagnosticCode: outputPrivacy ? "pre_model_sanitization_blocked" : privacyFailure || (providerHttp ? `model_http_${providerHttp.provider_http_status}` : clean(error.diagnosticCode || error.message || "historical_replay_error", 80)),
      truncatedFields: outputPrivacy ? [] : [...new Set(truncatedFields)].slice(0, 20),
      privacyChecks: sanitizedModelPrivacyChecks(privacyChecks),
    };
    throw error;
  }
...
export function historicalReplayMetrics(cases = []) {
  ...
  const measured = completed.map(storedHistoricalReplayConversationResult);
  ...
  return {
    ...
    firstOutboundCandidates: measured.filter((row) => row.auto_send_eligible === true).length,
    autoSendEligible: measured.filter((row) => row.auto_send_eligible === true).length,
    requiresHuman: measured.filter((row) => row.requires_human === true).length,
    doesNotRequireHuman: measured.filter((row) => row.requires_human === false).length,
    eligibilityNotRecorded: measured.filter((row) => row.auto_send_eligible === null).length,
  };
}
