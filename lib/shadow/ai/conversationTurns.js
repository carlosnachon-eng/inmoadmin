...
const TOPIC_PATTERNS = Object.freeze({
  property_handover: /\b(?:entrega(?:r|do|da)?|desocup(?:ar|aci[oó]n)|devoluci[oó]n\s+de\s+(?:llaves|tarjetas)|llaves\s+y\s+tarjetas|inspecci[oó]n\s+de\s+salida|cierre\s+de\s+ocupaci[oó]n)\b/i,
  maintenance: /\b(?:aver[ií]a|fuga|humedad|da[ñn]o|reparaci[oó]n|mantenimiento|t[eé]cnico|cotizaci[oó]n)\b/i,
  payment: /\b(?:renta|pago|comprobante|transferencia|saldo|adeudo)\b/i,
  renewal: /\b(?:renovaci[oó]n|renovar|pr[oó]rroga)\b/i,
  refund: /\b(?:devoluci[oó]n|reembolso|dep[oó]sito)\b/i,
});
const ANAPHORIC = /\b(?:ya\s+lo|ya\s+la|eso|as[ií]|listo|qued[oó]|entregu[eé]|pendiente)\b/i;

export function selectRelevantPriorContext(history = [], currentText = "", { maxMessages = REAL_SHADOW_CONTEXT_MAX_MESSAGES, maxChars = REAL_SHADOW_CONTEXT_MAX_CHARS } = {}) {
  const recent = history.slice(-maxMessages);
  const currentTopics = Object.entries(TOPIC_PATTERNS).filter(([, pattern]) => pattern.test(String(currentText || ""))).map(([topic]) => topic);
  const topics = currentTopics.length ? currentTopics : (ANAPHORIC.test(String(currentText || ""))
    ? Object.entries(TOPIC_PATTERNS).filter(([, pattern]) => history.some((item) => pattern.test(String(item?.sanitizedText || "")))).map(([topic]) => topic)
    : []);
  const relevant = topics.length ? history.filter((item) => topics.some((topic) => TOPIC_PATTERNS[topic].test(String(item?.sanitizedText || "")))) : [];
  const selected = [...relevant.slice(-Math.min(4, maxMessages)), ...recent];
  const unique = [...new Map(selected.map((item) => [`${item.occurredAt || ""}:${item.direction}:${item.sanitizedText}`, item])).values()]
    .sort((a, b) => time(a.occurredAt) - time(b.occurredAt));
  while (unique.length > maxMessages) {
    const removable = unique.findIndex((item) => !relevant.includes(item));
    unique.splice(removable >= 0 ? removable : 0, 1);
  }
  let chars = 0;
  return unique.filter((item) => {
    const text = String(item?.sanitizedText || "").slice(0, 1200);
    if (!text || chars + text.length > maxChars) return false;
    chars += text.length; return true;
  }).map((item) => ({ ...item, sanitizedText: String(item.sanitizedText).slice(0, 1200) }));
}
...
export function buildRealShadowConversationTurns({ messages = [], conversations = [], mediaInterpretations = [], mediaRetrievals = [], env = process.env, now = Date.now(), gapMs = REAL_SHADOW_TURN_GAP_MS, settleMs = REAL_SHADOW_TURN_SETTLE_MS, mediaSettleMaxMs = REAL_SHADOW_MEDIA_SETTLE_MAX_MS }) {
  ...
  for (const [conversationId, items] of grouped) {
    ...
    const priorContext = (currentText) => selectRelevantPriorContext(history, currentText);
    for (const message of [...items].sort((a, b) => time(a.occurred_at) - time(b.occurred_at) || String(a.id).localeCompare(String(b.id)))) {
      if (message.direction === "inbound") {
        ...
        current ||= { conversationId, messages: [], priorContextMessages: priorContext(message.sanitized_text) };
