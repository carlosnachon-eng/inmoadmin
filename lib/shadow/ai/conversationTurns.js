...
export function buildRealShadowConversationTurns({ messages = [], conversations = [], env = process.env, now = Date.now(), gapMs = REAL_SHADOW_TURN_GAP_MS, settleMs = REAL_SHADOW_TURN_SETTLE_MS }) {
  ...
  const close = (current, reason, humanResponseId = null) => {
    ...
    const attachments = current.messages.flatMap((item) => Array.isArray(item.attachment_metadata) ? item.attachment_metadata : [])
      .map((item) => ({ type: String(item?.type || "file"), mimeType: item?.mimeType ? String(item.mimeType) : null }))
      .slice(0, 10);
    turns.push({
      ...
      attachmentContext: { present: attachments.length > 0, interpreted: false, items: attachments },
    });
  };
