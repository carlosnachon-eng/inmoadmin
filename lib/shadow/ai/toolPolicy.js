...
const serviceTypeFromMessage = (message) => {
  const value = String(message || "");
  if (/\b(?:cfe|luz|electricidad|el[eé]ctrico)\b/i.test(value)) return "luz";
  if (/\bagua\b/i.test(value)) return "agua";
  if (/\bgas\b/i.test(value)) return "gas_recarga";
  if (/\binternet\b/i.test(value)) return "internet";
  return null;
};

export function deriveRequiredTools({ intent, secondaryIntents = [], message = "", resolvedOperationalContext = null, resolvedContext = [], availableIdentifiers = {}, metadata = {}, toolResults = [] }) {
  const ids = buildResolvedOperationalContext({ metadata, persistedContext: resolvedOperationalContext || availableIdentifiers, resolvedEntities: resolvedContext, toolResults });
  ...
  rule(Boolean(ids.respondContactId), "resolve_contact_identity", { respondContactId: ids.respondContactId }, "policy:confirmed_contact_identity_required");
  ...
  const serviceType = ids.serviceType || serviceTypeFromMessage(message);
  const serviceArgs = ids.serviceId
    ? { serviceId: ids.serviceId }
    : { propertyId: ids.propertyId, serviceType, ...(ids.period ? { period: ids.period } : {}) };
  rule(intents.has("servicio") && patterns.servicio.test(message) && !isConversationalClose({ sanitizedText: message, providerMetadata: metadata }), "get_service_period_status", serviceArgs, "policy:servicio_contexto_suficiente");
  ...
  rule(Boolean(ids.workCenterContextKey), "get_work_center_case", { contextKey: ids.workCenterContextKey }, "policy:work_center_contexto_explicito");
