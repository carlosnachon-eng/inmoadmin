...
export function validateShadowAiDecision(value) {
  ...
  for (const [field, max] of [["summary",500],["contextAssessment",1000],["proposedAction",500]]) if (typeof value[field] !== "string" || value[field].length > max) throw new Error("invalid_structured_output");
  if (!Array.isArray(value.factualClaims) || value.factualClaims.length > 20 || value.factualClaims.some((claim) => !claim || typeof claim !== "object" || Array.isArray(claim) || Object.keys(claim).some((key) => !["factType","value","evidenceIds"].includes(key)) || !CRITICAL_FACT_TYPES.includes(claim.factType) || !["string","number","boolean"].includes(typeof claim.value) || !isStringArray(claim.evidenceIds, 5, 180))) throw new Error("invalid_structured_output");
  const parts = value.conversationalResponseParts;
  if (!parts || typeof parts !== "object" || Array.isArray(parts) || Object.keys(parts).some((key) => !["acknowledgement","verifiedFactReferences","clarificationQuestion","escalationMessage"].includes(key)) || !isStringArray(parts.verifiedFactReferences,20,180)) throw new Error("invalid_structured_output");
  for (const [field,max] of [["acknowledgement",300],["clarificationQuestion",400],["escalationMessage",400]]) if (parts[field] !== null && (typeof parts[field] !== "string" || parts[field].length > max)) throw new Error("invalid_structured_output");
  if (!["none","implied","explicit"].includes(value.executionCommitment)) throw new Error("invalid_structured_output");
