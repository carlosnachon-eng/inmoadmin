...
const isStringArray = (value, max) => Array.isArray(value) && value.length <= max && value.every((item) => typeof item === "string");
export function validateShadowAiDecision(value) {
  ...
  if (!isStringArray(value.secondaryIntents, 5) || value.secondaryIntents.some((x) => !SHADOW_AI_INTENTS.includes(x))) throw new Error("invalid_structured_output");
  for (const [field, max] of [["entitiesMentioned",10],["informationNeeded",8],["proposedToolCalls",10],["safetyFlags",10]]) if (!isStringArray(value[field], max)) throw new Error("invalid_structured_output");
  for (const field of ["summary","contextAssessment","proposedAction","proposedResponse"]) if (typeof value[field] !== "string") throw new Error("invalid_structured_output");
  ...
  if (value.escalationReason !== null && typeof value.escalationReason !== "string") throw new Error("invalid_structured_output");
  return value;
}
