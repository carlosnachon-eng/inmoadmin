...
// Shared by input redaction and residual/output verification: a label followed
// by ordinary prose is not an ID. Require an explicit marker or a digit within
// the same token (never a later word). Keep the existing minimum token length.
...
const RULES = Object.freeze([
  ...
  ["folio", labeledIdentifier("folio|referencia|operaci[oó]n|autorizaci[oó]n|rastreo|ticket", "giu"), "$1 [FOLIO]"],
...
const RESIDUAL = Object.freeze([
  ...
  ["labeled_identifier", labeledIdentifier(String.raw`folio|referencia|operaci[oó]n|autorizaci[oó]n|rastreo|cuenta|clabe|tarjeta|token|api\s*key`, "iu")],
