...
const criticalFreeText = /\b(?:pagad[oa]|pendiente|vencid[oa]|activ[oa]|nuev[oa]|urgente|cerrad[oa]|abiert[oa]|disponible|prestada|en resguardo)\b|(?:\$|monto\s+(?:de\s+)?|debes?\s+\$?)[\d,.]+|\b\d{4}-\d{2}(?:-\d{2})?\b/i;
...
function analyzeCriticalFreeText(decision, ledger) {
  ...
  for (const sentence of sentences) {
    const assertsReceiptRecorded = /\b(?:tenemos|est[aá]|aparece|qued[oó])\b[^.?!]{0,45}\b(?:registrad[oa]|recibid[oa])\b[^.?!]{0,35}\bcomprobante\b|\btenemos\s+registrado\s+(?:tu|el)\s+comprobante\b/i.test(sentence.text);
    const hasReceiptEvidence = ledger.some((entry) => Object.hasOwn(entry.facts || {}, "hasReceipt"));
    if (assertsReceiptRecorded && !hasReceiptEvidence) continue;
...
const factPhrase = Object.freeze({
  ...
  "key.status": (v) => `La llave aparece ${v === "en_resguardo" ? "en resguardo" : v === "prestada" ? "prestada" : `con estado ${String(v).replaceAll("_", " ")}`}.`,
...
export function groundAndRenderDecision(decision, tools = []) {
  ...
  const verifiedFacts = blocked ? [] : uniqueClaims
    .filter((claim) => !(claim.factType === "key.inCustody" && uniqueClaims.some((other) => other.factType === "key.status" && ["en_resguardo","prestada"].includes(other.value))))
    .map((claim) => factPhrase[claim.factType]?.(claim.value)).filter(Boolean);
