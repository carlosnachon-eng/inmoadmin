...
import { classifyAnthropicFailure, DEFAULT_SHADOW_AI_MODEL, shadowAiOutputMode } from "./anthropic.js";
...
import { actorAwarePriorConversation, plannableShadowToolCalls } from "./actorRoleGuards.js";
import { invokeShadowPhase3A, invokeShadowPhase3ARepair } from "./phase3AGateway.js";
import {
  finalizeShadowAiDecision,
...
export function createShadowAiInputSnapshot(envelope) {
  ...
  for (const item of prior) {
    ...
    priorConversation.push(actorAwarePriorConversation([{ direction: item.direction, sanitizedText }])[0]);
...
export async function terminateAwaitingShadowAiRun(admin, runId, { reason, timeout = false, now = Date.now() } = {}) {
  const endedAt = new Date(now).toISOString();
  const { data: run, error: loadError } = await admin.from("shadow_ai_runs")
    .select("id,telemetry_json")
    .eq("id", runId).eq("execution_state", "awaiting_model_round").maybeSingle();
  if (loadError) throw loadError;
  if (!run) return { status: "already_running", runId };
  const telemetry = run.telemetry_json && typeof run.telemetry_json === "object" ? structuredClone(run.telemetry_json) : {};
  telemetry.inter_round_terminal = { reason: String(reason || "continuation_blocked").slice(0, 80), at: endedAt };
  const state = timeout ? "timeout" : "blocked";
  const { data, error } = await admin.from("shadow_ai_runs").update({
    status: timeout ? "timeout" : "completed", execution_state: state,
    completed_at: endedAt, state_updated_at: endedAt,
    error_sanitized: String(reason || "continuation_blocked").slice(0, 80), telemetry_json: telemetry,
  }).eq("id", runId).eq("execution_state", "awaiting_model_round").select("id").maybeSingle();
  if (error) throw error;
  return data ? { status: state, state, runId, reason } : { status: "already_running", runId };
}
...
export async function executeShadowAiStateStep(admin, runId, expectedState, options = {}) {
  ...
  try {
    ...
    const roundStartedMs = clock.now();
    const previousRound = telemetry.rounds.at(-1);
    if (previousRound && !previousRound.next_round_started_at) {
      previousRound.next_round_started_at = new Date(roundStartedMs).toISOString();
      previousRound.inter_round_delay_ms = Math.max(0, roundStartedMs - Date.parse(previousRound.round_completed_at || previousRound.next_round_started_at));
    }
    ...
    const attempt = await executeAnthropicAttemptPolicy({
      call: ({ signal, onPhase }) => invokeShadowPhase3A({ envelope, deterministic, toolResults: previousTools, round: round - 1,
        systemPrompt, toolGuide, modelCall: options.modelCall, modelOptions: { signal, onPhase, env, outputMode },
      }),
      ...
      validation = await validateWithSingleRepair(modelResult.text, {
        repair: outputMode === "text_json_local" ? async (invalidText) => {
          ...
          const repairAttempt = await executeAnthropicAttemptPolicy({
            call: ({ signal, onPhase }) => invokeShadowPhase3ARepair(invalidText, { repairModelCall: options.repairModelCall, modelOptions: { signal, onPhase, env } }),
            clock, deadlineMs, round, telemetry,
            attemptTimeoutMs: Number(env.SHADOW_AI_ANTHROPIC_ATTEMPT_TIMEOUT_MS || SHADOW_AI_LIMITS.autoRealAnthropicAttemptTimeoutMs),
            minimumRetryBudgetMs: Number(env.SHADOW_AI_MINIMUM_RETRY_BUDGET_MS || SHADOW_AI_LIMITS.minimumAnthropicRetryBudgetMs),
    ...
    const proposedToolCount = decision.proposedToolCalls.length;
    decision.proposedToolCalls = plannableShadowToolCalls(decision);
    if (decision.proposedToolCalls.length < proposedToolCount) {
      decision.safetyFlags = [...new Set([...(decision.safetyFlags || []), "invalid_tool_arguments_plan_blocked"])];
    }
    ...
    const roundRecord = { round_number: round, request_id: modelResult.id || null, status: "completed", round_started_at: new Date(roundStartedMs).toISOString(), round_completed_at: new Date(clock.now()).toISOString(), next_round_started_at: null, inter_round_delay_ms: null, remaining_deadline_ms_at_round_start: Math.max(0, deadlineMs - roundStartedMs), anthropic_duration_ms: anthropicDurationMs, output_state: "complete", policy_required_tools: policy.requiredNowTools, expected_after_clarification_tools: policy.expectedAfterClarificationTools, proposed_tool_calls: plan.uniqueCalls.map(({ name, args, reason, source }) => ({ name, args, reason, source })) };
    ...
    let conversationActionResult = null;
    if (finalDecision.operational_resolution && !operationalInput) {
      try {
        ...
        conversationActionResult = actionResult;
    ...
    if (finalDecision.operational_resolution && !operationalInput && (isAutoRealInputMode(options.inputMode) || isAutoRealInputMode(run.telemetry_json?.input_mode))) {
      try {
        const r1Result = await maybeExecuteAdministrativeWorkR1({
          ...
          context: policyContext, conversationAction: conversationActionResult?.action || null,
          humanResponseId: run.telemetry_json?.human_response_id || null, env,
        });
