...
const safeTool = ({ name, args, reason, result, ok, error, durationMs, round }) => ({ name, args, reason, result, ok, error: error || null, durationMs, round });
...
export async function executeShadowAiStateStep(admin, runId, expectedState, options = {}) {
  ...
  const envelope = envelopeFromMessage(message || {}); const guard = shadowAiGuard(envelope, env);
  if (!guard.allowed) {
  ...
  try {
    ...
    const plan = nextRoundPlan(run, requestedShadowAiCalls(decision), previousTools);
    ...
    for (const call of plan.uniqueCalls.slice(0, SHADOW_AI_LIMITS.maxToolsPerRound)) {
      ...
      telemetry.tools.push({ round_number: round, name: item.name, tool_duration_ms: item.durationMs, succeeded: item.ok, error: item.error });
    }
    ...
    const roundRecord = { round_number: round, request_id: modelResult.id || null, status: "completed", anthropic_duration_ms: anthropicDurationMs, output_state: "complete", proposed_tool_calls: plan.uniqueCalls.map(({ name, args, reason }) => ({ name, args, reason })) };
    ...
    roundState.rounds ||= []; roundState.rounds.push({ roundNumber: round, decision, proposedToolCalls: roundRecord.proposed_tool_calls, providerRequestId: modelResult.id || null });
...
export async function startShadowAiStateMachine(admin, { messageId, envelope }, options = {}) {
  ...
  const model = env.SHADOW_AI_MODEL || DEFAULT_SHADOW_AI_MODEL; const key = shadowAiIdempotencyKey(messageId, model);
