...
export async function executeShadowAiStateStep(admin, runId, expectedState, options = {}) {
  ...
  const { data: message, error: messageError } = run.message_id
    ? await admin.from("shadow_messages").select("id,provider,direction,sanitized_text,provider_metadata,external_message_id,occurred_at").eq("id", run.message_id).maybeSingle()
...
export async function continueShadowAiStateMachine(admin, runId, options = {}) {
  const { data: run, error } = await admin.from("shadow_ai_runs").select("id,status,execution_state,message_id,operational_event_id,current_round,max_rounds").eq("id", runId).maybeSingle(); if (error) throw error;
