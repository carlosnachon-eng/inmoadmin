...
export async function executeShadowAiStateStep(admin, runId, expectedState, options = {}) {
  ...
  const { data: message, error: messageError } = run.message_id
    ? await admin.from("shadow_messages").select("id,conversation_id,provider,direction,sanitized_text,provider_metadata,external_message_id,occurred_at").eq("id", run.message_id).maybeSingle()
  ...
  const inputEnvelope = operationalInput?.envelope || envelopeFromMessage(message || {});
...
export async function startShadowAiStateMachine(admin, { messageId, envelope }, options = {}) {
  ...
  const model = env.SHADOW_AI_MODEL || DEFAULT_SHADOW_AI_MODEL; const campaignId = options.campaignId || null; const promptVersion = options.promptVersion || SHADOW_AI_PROMPT_VERSION; const key = shadowAiIdempotencyKey(messageId, model, campaignId, promptVersion);
