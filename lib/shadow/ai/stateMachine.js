...
export async function executeShadowAiStateStep(admin, runId, expectedState, options = {}) {
  ...
  try {
    ...
    let parsed;
    try {
      parsed = JSON.parse(modelResult.text);
    } catch {
      const parseError = new Error("invalid_structured_output:json_parse_error");
      parseError.diagnosticCode = "json_parse_error";
      parseError.outputStage = "json_parsing";
      parseError.providerResult = { id: modelResult.id || null, usage: modelResult.usage || null, durationMs: anthropicDurationMs };
      throw parseError;
    }
    let decision;
    try {
      decision = validateShadowAiDecision(parsed);
    } catch (validationError) {
      validationError.providerResult = { id: modelResult.id || null, usage: modelResult.usage || null, durationMs: anthropicDurationMs };
    ...
    if (!operationalInput && (options.inputMode === "auto_real_shadow" || run.telemetry_json?.input_mode === "auto_real_shadow")) {
    ...
    if (finalDecision.operational_resolution && !operationalInput && (options.inputMode === "auto_real_shadow" || run.telemetry_json?.input_mode === "auto_real_shadow")) {
      try {
    ...
    telemetry.anthropic_requests.push({
      ...
      status: "failed",
      anthropic_duration_ms: receivedInvalidOutput ? error.providerResult.durationMs : clock.now() - anthropicStartedMs,
      ...
      error: error?.timeoutStage || (receivedInvalidOutput ? "invalid_structured_output" : "anthropic_request_error"),
    });
...
export async function startOperationalShadowAiStateMachine(admin, operationalEvent, options = {}) {
  ...
  const { data: run, error: insertError } = await admin.from("shadow_ai_runs").insert({
    ...
    started_at: now, state_updated_at: now, idempotency_key: key, attempt_number: 1,
    round_state_json: { resolvedOperationalContext, rounds: [] },
    telemetry_json: { input_kind: "operational_event", operational_event_id: input.inputId, schema_version: SHADOW_AI_OUTPUT_SCHEMA_VERSION, prompt_version: SHADOW_AI_PROMPT_VERSION, http_steps: [], anthropic_requests: [], tools: [], rounds: [] },
  }).select("id").single();
  if (insertError?.code === "23505") return { status: "running", runId: null };
  if (insertError) throw insertError;
  return executeShadowAiStateStep(admin, run.id, "created", options);
...
export async function continueShadowAiStateMachine(admin, runId, options = {}) {
  const { data: run, error } = await admin.from("shadow_ai_runs").select("id,status,execution_state,message_id,operational_event_id,current_round,max_rounds,prompt_version,model,campaign_id").eq("id", runId).maybeSingle(); if (error) throw error;
