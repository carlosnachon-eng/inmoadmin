...
export function createShadowAiInputSnapshot(envelope) {
  ...
  return {
    ...
    providerMetadata: {
      channelId: String(envelope?.providerMetadata?.channelId || ""), priorConversation,
      attachmentContext: {
        ...
        items: Array.isArray(envelope?.providerMetadata?.attachmentContext?.items)
          ? envelope.providerMetadata.attachmentContext.items.slice(0, 10).map((item) => ({ type: String(item?.type || "file"), mimeType: item?.mimeType ? String(item.mimeType) : null }))
          : [],
      },
      conversationTurn: {
