...
import { classifyAnthropicFailure, createAnthropicShadowResponse, DEFAULT_SHADOW_AI_MODEL } from "./anthropic.js";
...
export async function executeShadowAiStateStep(admin, runId, expectedState, options = {}) {
  ...
  try {
    ...
    const attempt = await executeAnthropicAttemptPolicy({
      call: ({ signal, onPhase }) => (options.modelCall || createAnthropicShadowResponse)(messages, { signal, onPhase, env }),
    ...
    const modelResult = attempt.result; const anthropicDurationMs = attempt.record.latency_ms;
    let parsed;
    try {
