...
export async function executeShadowAiStateStep(admin, runId, expectedState, options = {}) {
  ...
  try {
    ...
    const policy = deriveRequiredTools({ intent: decision.intent, secondaryIntents: decision.secondaryIntents, message: envelope.sanitizedText, resolvedContext: decision.resolvedEntities, metadata: envelope.providerMetadata, toolResults: previousTools });
