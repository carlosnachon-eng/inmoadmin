...
export function realShadowEnvelope(message, conversation, env = process.env) {
  return {
    ...
    providerMetadata: { ...(message.provider_metadata || {}), channelId: String(conversation.channel), ...(env.SHADOW_IDENTITY_BRIDGE_ENABLED === "true" ? { respondContactId: String(conversation.respond_contact_id || "") } : {}) },
  };
}
...
export async function loadRealShadowMessage(admin, messageId) {
  ...
  const { data: conversation, error: conversationError } = message
    ? await admin.from("shadow_conversations").select("id,provider,channel,respond_contact_id").eq("id", message.conversation_id).maybeSingle()
