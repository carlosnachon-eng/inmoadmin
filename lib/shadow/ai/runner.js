...
const shadowCommitmentPatterns = /\b(?:(?:voy|vamos) a (?:registrar|enviar|programar|solicitar|realizar)|proceder(?:é|emos)|para que podamos (?:registrar|enviar|programar|solicitar|realizar)|para (?:ayudarte a )?procesar(?:lo|la)?)\b/i;
  ...
  try {
    for (let round = 0; round < SHADOW_AI_LIMITS.maxToolRounds; round += 1) {
      ...
      try {
        ...
        telemetry.anthropic_requests.push({ round: rounds, anthropic_request_started_at: new Date(anthropicStartedMs).toISOString(), anthropic_first_response_ms: null, anthropic_duration_ms: clock.now() - anthropicStartedMs, succeeded: true });
      } catch (error) {
        telemetry.anthropic_requests.push({ round: rounds, anthropic_request_started_at: new Date(anthropicStartedMs).toISOString(), anthropic_first_response_ms: null, anthropic_duration_ms: clock.now() - anthropicStartedMs, succeeded: false, error: error?.timeoutStage || "anthropic_request_error" });
        throw error;
      }
      ...
      if (!calls.length) { telemetry.rounds.push({ round: rounds, round_duration_ms: clock.now() - roundStartedMs }); activeRoundStartedMs = null; break; }
      ...
      telemetry.rounds.push({ round: rounds, round_duration_ms: clock.now() - roundStartedMs }); activeRoundStartedMs = null;
    }
    if (!decision) throw new Error("empty_model_output");
    decision = reconcileIntentTaxonomy(reconcileEvidence(decision, tools), envelope);
    if (shadowCommitmentPatterns.test(decision.proposedResponse)) {
      ...
      decision.proposedResponse = decision.intent === "devolucion_deposito"
        ...
        : "Necesito que una persona del equipo revise esta solicitud antes de confirmar cualquier acción.";
    }
    ...
    if (rounds && activeRoundStartedMs !== null && !telemetry.rounds.some((entry) => entry.round === rounds)) telemetry.rounds.push({ round: rounds, round_duration_ms: completedMs - activeRoundStartedMs });
