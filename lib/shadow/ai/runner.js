...
const shadowCommitmentPatterns = /\b(?:(?:voy|vamos) a (?:registrar|enviar|programar|solicitar|realizar)|proceder(?:é|emos)|para que podamos (?:registrar|enviar|programar|solicitar|realizar))\b/i;
...
export async function runShadowAi(admin, { messageId, envelope, deterministic }, options = {}) {
  ...
  try {
    ...
    if (shadowCommitmentPatterns.test(decision.proposedResponse)) {
      ...
      decision.proposedResponse = "Necesito que una persona del equipo revise esta solicitud antes de confirmar cualquier acción.";
    }
