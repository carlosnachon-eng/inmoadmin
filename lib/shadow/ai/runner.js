...
import { groundAndRenderDecision } from "./grounding.js";
import { combinePolicyAndModelTools, deriveRequiredTools } from "./toolPolicy.js";
import { applyActorRoleGuards, plannableShadowToolCalls } from "./actorRoleGuards.js";
import { buildAllowlistedShadowAiContext, decodeModelDecisionReferences, invokeShadowPhase3A } from "./phase3AGateway.js";
...
export const shadowAiIdempotencyKey = (messageId, model, campaignId = null, promptVersion = SHADOW_AI_PROMPT_VERSION) => crypto.createHash("sha256").update(`${messageId}:${promptVersion}:${model}${campaignId ? `:qa-campaign:${campaignId}` : ""}`).digest("hex");
const MAX_ATTEMPTS = 3;
const retryableStatus = (status) => ["error", "timeout"].includes(status);

export class ShadowAiStageTimeoutError extends Error {
  constructor(stage, timeoutClass = null) {
    super(stage);
    this.name = "ShadowAiStageTimeoutError";
    this.timeoutStage = stage;
    this.timeoutClass = timeoutClass || (stage === "global_run_timeout" ? "local_deadline_exceeded" : stage);
  }
}

export function durableRunDeadline(run) {
  const value = Date.parse(String(run?.deadline_at || ""));
  if (!Number.isFinite(value)) throw new ShadowAiStageTimeoutError("global_run_timeout", "local_deadline_exceeded");
  return value;
}
...
export function minimalShadowAiContext(envelope, deterministic, toolResults, round) {
  return buildAllowlistedShadowAiContext({ envelope, deterministic, toolResults, round, message: envelope.sanitizedText });
}

export function requestedShadowAiCalls(decision) {
  return plannableShadowToolCalls(decision).slice(0, SHADOW_AI_LIMITS.maxToolsPerRound).map((entry) => ({ name: entry.tool, args: entry.arguments, reason: entry.reason }));
}

const metadataId = Object.freeze({ respondContactId: "respondContactId", propertyId: "propertyId", contractId: "contractId", paymentId: "paymentId", serviceId: "serviceId", ticketId: "ticketId", contextKey: "workCenterContextKey", keyId: "keyId", ownerPaymentId: "ownerPaymentId", recordId: "recordId", unitId: "unitId" });
...
function reconcileEscalationTaxonomy(decision, envelope, tools = []) {
  ...
  const resolvedMaintenanceFollowup = decision.intent === "mantenimiento"
    && /\bel t[eé]cnico no lleg(?:o|ó)(?=\s|[.,;:!?]|$)/i.test(envelope.sanitizedText)
    && tools.some((tool) => tool.ok && tool.name === "get_maintenance_ticket_summary" && tool.result?.length);
  if (resolvedMaintenanceFollowup) {
    decision.requiresHuman = false;
    decision.escalationReason = null;
    decision.proposedAction = "Informar el estado verificado del mantenimiento y solicitar una sola precisión útil, sin prometer reprogramación.";
    decision.conversationalResponseParts.escalationMessage = null;
  }
...
function reconcileKeyResponse(decision, envelope) {
  if (decision.intent !== "llaves") return decision;
  if (decision.executionCommitment !== "none") return decision;
  const returned = /(?:devolv(?:i|í|ieron)|entregu(?:e|é|aron))/i.test(envelope.sanitizedText) && /\bllaves?\b/i.test(envelope.sanitizedText);
  decision.requiresHuman = true;
  decision.escalationReason ||= "El acceso físico y los cambios de custodia requieren revisión humana.";
  decision.executionCommitment = "none";
  decision.conversationalResponseParts = returned
    ? { acknowledgement: "Gracias por informar la devolución de las llaves.", verifiedFactReferences: decision.conversationalResponseParts?.verifiedFactReferences || [], clarificationQuestion: null, escalationMessage: "La recepción física y cualquier cambio de custodia requieren revisión del equipo de Administración." }
    : { acknowledgement: "Entiendo la solicitud de acceso mediante llaves.", verifiedFactReferences: decision.conversationalResponseParts?.verifiedFactReferences || [], clarificationQuestion: "¿Me confirmas el motivo principal por el que necesitas las llaves?", escalationMessage: "La entrega requiere revisión y autorización del equipo de Administración." };
  return decision;
}
...
const descriptiveHumanReviewPattern = /\b(?:esto|este caso|la entrega)\s+(?:requiere|necesita|est[aá] sujeta a)\s+(?:revisi[oó]n|autorizaci[oó]n)(?:\s+(?:del equipo de administraci[oó]n|humana))?\b/i;
const neutralizableOperationalPromise = /\b(?:te (?:esperamos|avisamos|contactamos)|lo (?:revisamos|canalizamos|gestionamos))(?=\s|[.,;:!?]|$)/i;
...
export function finalizeShadowAiDecision(decision, envelope, tools) {
  ...
  decision = applyActorRoleGuards(decision, envelope, tools);
  ...
  let commitmentCandidate = `${decision.proposedAction} ${responsePartText(decision)}`;
  if (neutralizableOperationalPromise.test(commitmentCandidate)) {
    decision.safetyFlags = [...new Set([...decision.safetyFlags, "shadow_action_promise_neutralized"])];
    decision.executionCommitment = "none";
    decision.proposedAction = "Revisión humana sugerida; Shadow no ejecuta acciones.";
    replaceResponseParts(decision, "Esto requiere revisión del equipo de Administración.");
    commitmentCandidate = `${decision.proposedAction} ${responsePartText(decision)}`;
  }
  if (decision.executionCommitment !== "none" && descriptiveHumanReviewPattern.test(responsePartText(decision)) && !shadowCommitmentPatterns.test(commitmentCandidate)) {
    decision.executionCommitment = "none";
  }
  if (shadowCommitmentPatterns.test(commitmentCandidate)) {
  ...
  decision = reconcileKeyResponse(reconcileEscalationTaxonomy(decision, envelope, tools), envelope);
  if (envelope.providerMetadata?.attachmentContext?.present === true) {
    decision.safetyFlags = [...new Set([...decision.safetyFlags, "uninterpreted_attachment"])];
    if (!(tools || []).length) decision.factualClaims = [];
    const nonAttachmentText = String(envelope.sanitizedText || "").replace(/\[(?:IMAGEN|DOCUMENTO|AUDIO|VIDEO|STICKER|UBICACION|CONTACTO|ARCHIVO)\]/g, "").trim();
    const attachmentAssertion = /(?:\b(?:imagen|archivo|documento|audio|comprobante|adjunto)\b[^.?!]{0,100}\b(?:confirma|demuestra|muestra|contiene|indica|corresponde|es v[aá]lid[oa]|pagad[oa])\b|\b(?:confirmo|verifiqu[eé])\b[^.?!]{0,100}\b(?:imagen|archivo|documento|audio|comprobante|adjunto)\b)/i.test(`${decision.proposedAction} ${responsePartText(decision)}`);
    if (!nonAttachmentText || attachmentAssertion) {
      decision.requiresHuman = true;
      decision.confidence = Math.min(decision.confidence, 0.5);
      decision.escalationReason ||= "El adjunto no fue interpretado y requiere contexto o revisión humana.";
      decision.conversationalResponseParts = {
        acknowledgement: "Recibí el archivo adjunto, pero no puedo verificar su contenido en modo Shadow.",
        verifiedFactReferences: [],
        clarificationQuestion: nonAttachmentText ? null : "¿Me indicas brevemente qué contiene o qué necesitas revisar?",
        escalationMessage: "El contenido del archivo requiere revisión humana.",
      };
    }
  }
...
export async function runShadowAi(admin, { messageId, envelope, deterministic }, options = {}) {
  ...
  try {
    for (let round = 0; round < SHADOW_AI_LIMITS.maxToolRounds; round += 1) {
      ...
      let modelResult;
      try {
        modelResult = await withShadowAiStageTimeout(() => invokeShadowPhase3A({ admin, envelope, deterministic, toolResults: tools, round,
          systemPrompt: SHADOW_AI_SYSTEM_PROMPT, toolGuide: SHADOW_AI_TOOL_GUIDE, modelCall: options.modelCall,
          modelOptions: { signal: controller.signal, env },
        }), { stage: "anthropic_request_timeout", stageTimeoutMs: anthropicTimeoutMs, globalDeadlineMs, clock, controller });
      ...
      decision = decodeModelDecisionReferences(validateShadowAiDecision(JSON.parse(modelResult.text)), modelResult);
      const policy = deriveRequiredTools({ intent: decision.intent, secondaryIntents: decision.secondaryIntents, message: envelope.sanitizedText, resolvedContext: decision.resolvedEntities, metadata: envelope.providerMetadata, toolResults: tools });
      const calls = combinePolicyAndModelTools(policy.requiredNowTools, requestedShadowAiCalls(decision)).filter(({ name }) => !tools.some((item) => item.ok && item.name === name));
      ...
      for (const call of calls) {
        ...
        try {
          ...
          tools.push({ name: call.name, args, reason: call.reason, source: call.source, result, ok: true, durationMs });
          telemetry.tools.push({ round: rounds, name: call.name, source: call.source, tool_duration_ms: durationMs, succeeded: true });
        } catch (error) {
          const durationMs = clock.now() - began;
          tools.push({ name: call.name, args: call.args || {}, reason: call.reason, source: call.source, result: [], ok: false, error: String(error?.message || "invalid_tool_call").slice(0,80), durationMs });
          telemetry.tools.push({ round: rounds, name: call.name, source: call.source, tool_duration_ms: durationMs, succeeded: false, error: error?.timeoutStage || String(error?.message || "invalid_tool_call").slice(0,80) });
    ...
    const { error: decisionError } = await admin.from("shadow_ai_decisions").insert({ ai_run_id: run.id, status: "completed", ...{
      ...
      decision_json: decision, tool_summary: tools.map(({ name, args, reason, source, result, ok, error, durationMs }) => ({ name, args, reason, source, resultCount: result.length, ok, error: error || null, durationMs })),
    } });
    if (decisionError) throw decisionError;
    const condominiumEvidence = tools.filter((tool) => tool.name === "resolve_contact_identity" && tool.result?.some((row) => row.identityDomain === "condominium"));
    await admin.from("shadow_ai_runs").update({ status: "completed", completed_at: completedAt, latency_ms: latencyMs, input_tokens: inputTokens, output_tokens: outputTokens, estimated_cost_usd: estimatedCostUsd, telemetry_json: telemetry,
      ...(condominiumEvidence.length ? { tool_results_json: condominiumEvidence } : {}) }).eq("id", run.id);
