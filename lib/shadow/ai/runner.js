...
export const shadowCommitmentPatterns = /\b(?:(?:voy|vamos) a (?:registrar|enviar|programar|solicitar|realizar)|proceder(?:é|emos)|para que podamos (?:registrar|enviar|programar|solicitar|realizar)|para (?:ayudarte a )?procesar(?:lo|la)?|(?:puedo|podr[eé]) (?:revisar|canalizar)|para proceder|vamos a gestionar|lo registrar[eé])(?=\s|[.,;:!?]|$)/i;
...
const runtimeClock = (options) => options.clock || { now: () => Date.now(), setTimeout, clearTimeout };
async function withStageTimeout(operation, { stage, stageTimeoutMs, globalDeadlineMs, clock, controller }) {
...
function reconcileIntentTaxonomy(decision, envelope) {
  const explicit = classifyExplicitShadowAiIntent(envelope.sanitizedText);
...
function reconcileEvidence(decision, tools) {
  ...
  const claimsExistingCase = /\b(?:ubicar|localizar) (?:tu|el) (?:reporte|ticket)\b/i.test(decision.proposedResponse);
  const unsupportedLanguage = (/\b(?:voy a revisar|ya revis[eé]|veo que|tenemos registrado)\b/i.test(decision.proposedResponse) || claimsExistingCase) && !hasErpEvidence;
  if (unsupported || unsupportedLanguage) {
    decision.requiresHuman = true;
    decision.safetyFlags = [...new Set([...decision.safetyFlags, "unsupported_erp_fact"])];
    if (unsupportedLanguage) decision.proposedResponse = decision.intent === "servicio"
      ? "Entiendo. No pude identificar con certeza la propiedad a la que te refieres. ¿Me confirmas cuál es para revisar lo del agua?"
      : "Necesito confirmar el contexto en el sistema antes de darte una respuesta precisa. ¿Puedes indicar la referencia exacta del inmueble?";
  }
...
export async function runShadowAi(admin, { messageId, envelope, deterministic }, options = {}) {
  ...
  const clock = runtimeClock(options); const runStartedMs = clock.now();
  ...
  const telemetry = { anthropic_requests: [], tools: [], rounds: [], total_run_duration_ms: null, timeout_stage: null };
  try {
    for (let round = 0; round < SHADOW_AI_LIMITS.maxToolRounds; round += 1) {
      ...
      let modelResult;
      try {
        modelResult = await withStageTimeout(() => (options.modelCall || createAnthropicShadowResponse)([
        ...
        telemetry.anthropic_requests.push({ request_number: rounds, round_number: rounds, round: rounds, anthropic_request_started_at: new Date(anthropicStartedMs).toISOString(), anthropic_first_response_ms: null, anthropic_duration_ms: clock.now() - anthropicStartedMs, succeeded: true });
      } catch (error) {
        telemetry.anthropic_requests.push({ request_number: rounds, round_number: rounds, round: rounds, anthropic_request_started_at: new Date(anthropicStartedMs).toISOString(), anthropic_first_response_ms: null, anthropic_duration_ms: clock.now() - anthropicStartedMs, succeeded: false, error: error?.timeoutStage || "anthropic_request_error" });
        throw error;
      }
      ...
      for (const call of calls) {
        ...
        try {
          ...
          const result = await withStageTimeout(() => (options.executeTool || executeShadowReadOnlyTool)(admin, call.name, args), { stage: "tool_timeout", stageTimeoutMs: toolTimeoutMs, globalDeadlineMs, clock, controller: null });
    ...
    if (decision.intent === "juridico_conflicto" && unsafeLegalActionPattern.test(decision.proposedAction)) {
      ...
      decision.proposedResponse = "Entiendo. Para que el equipo correspondiente pueda revisar tu caso, ¿me indicas brevemente cuál es el motivo principal de tu inconformidad?";
    }
    if (shadowCommitmentPatterns.test(decision.proposedResponse)) {
      decision.requiresHuman = true;
      decision.safetyFlags = [...new Set([...decision.safetyFlags, "shadow_action_promise_blocked"])];
      decision.proposedResponse = decision.intent === "devolucion_deposito"
        ? "Entiendo. Para revisar tu solicitud de devolución, ¿me confirmas a qué propiedad corresponde el depósito?"
        : decision.intent === "propietario_liquidacion"
          ? "¿Me confirmas a qué propiedad corresponde la liquidación?"
          : decision.intent === "contrato"
            ? "¿Me confirmas qué inmueble corresponde al contrato que deseas renovar?"
            : decision.intent === "llaves"
              ? "¿Me confirmas de qué inmueble necesitas las llaves?"
    ...
    await admin.from("shadow_ai_runs").update({ status, completed_at: completedAt, latency_ms: latencyMs, error_sanitized: timeoutStage || sanitizeError(error), telemetry_json: telemetry }).eq("id", run.id);
    return { status, runId: run.id, error: timeoutStage || sanitizeError(error), timeoutStage, telemetry, providerError: error?.providerError || null, latencyMs };
  }
}
