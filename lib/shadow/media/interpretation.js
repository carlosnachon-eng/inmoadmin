...
const explicitCurrency = (currency, evidenceText) => {
  const value = String(currency || "").trim().toUpperCase();
  const evidence = String(evidenceText || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase();
  if (!value) return null;
  const patterns = {
    MXN: /(?:\bMXN\b|\bPESOS?(?:\s+MEXICANOS?)?\b)/,
    USD: /(?:\bUSD\b|\bUS\$|\bDOLARES?(?:\s+ESTADOUNIDENSES?)?\b)/,
    EUR: /(?:\bEUR\b|\bEUROS?\b)/,
  };
  return patterns[value]?.test(evidence) ? value : null;
};
...
export function validateAndSanitizeMediaInterpretation(value,{model=DEFAULT_SHADOW_AI_MODEL,now=new Date()}={}) {
  ...
  return {
    ...
    extracted_fields:{
      amount,currency:explicitCurrency(fields.currency, `${value.summary} ${fields.reference || ""}`),date:sanitizeMediaInterpretationText(nullableString(fields.date),20)||null,
