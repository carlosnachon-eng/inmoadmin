...
function requestOnce(url, records, { request = https.request, connectTimeoutMs = 3_000 } = {}) {
  return new Promise((resolve,reject)=>{
    const req=request({protocol:"https:",hostname:url.hostname,port:url.port||443,path:`${url.pathname}${url.search}`,method:"GET",servername:url.hostname,headers:{accept:"application/octet-stream","user-agent":"inmoadmin-shadow-media/1"},lookup:(_host,_opts,cb)=>cb(null,records[0].address,records[0].family),timeout:connectTimeoutMs},resolve);
