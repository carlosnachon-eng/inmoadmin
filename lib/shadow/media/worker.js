...
import { mediaNetworkErrorStage, normalizeMediaNetworkErrorCode } from "./errors.js";
import { interpretValidatedShadowImage } from "./interpretation.js";
...
export async function processOneMediaRetrieval(admin,{privateKeyPem=process.env.SHADOW_MEDIA_RETRIEVAL_PRIVATE_KEY,download=secureDownload,interpret=interpretValidatedShadowImage,env=process.env,now=new Date(),workerId=crypto.randomUUID()}={}){
  ...
  try{
    ...
    let interpretation;
    try{interpretation=await interpret(admin,{claim,buffer:downloaded.buffer,validated,env,now});}
    catch{interpretation={status:"failed",error:"interpretation_internal_error"};}
    ...
    return{id:claim.id,status:"completed",result,interpretation:{status:interpretation?.status||"off",error:interpretation?.error||null}};
  }catch(error){
    const code=normalizeMediaNetworkErrorCode(error);const errorStage=mediaNetworkErrorStage(code);const retryable=error?.retryable===true&&claim.attempts<4&&new Date(claim.expires_at)>new Date();
    ...
    const {data:failed,error:updateError}=await admin.rpc("fail_shadow_media_retrieval",{p_queue_id:claim.id,p_worker_id:workerId,p_error_code:code,p_error_stage:errorStage,p_retry_at:next,p_terminal_status:terminalStatus});if(updateError)throw updateError;if(failed!==true)throw Object.assign(new Error("media_claim_lost"),{code:"media_claim_lost"});
    return{id:claim.id,status:retryable?"pending":(new Date(claim.expires_at)<=new Date()?"expired":"rejected"),error:code,errorStage};
  }finally{
