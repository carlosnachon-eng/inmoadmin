...
export const shadowContextTools = Object.freeze({
  ...
  find_active_contracts: (db, value) => { const args = typeof value === "string" ? { contractId: value } : value; return bounded(
    (args.contractId
      ? db.from("contracts").select("id,property_id,status").eq("id", args.contractId)
      : db.from("contracts").select("id,property_id,status").eq("property_id", args.propertyId)).in("status", ["activo", "active"]),
