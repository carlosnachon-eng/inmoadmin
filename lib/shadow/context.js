...
export const SHADOW_TOOL_ARGUMENT_SCHEMAS = Object.freeze({
  resolve_contact_identity: { type: "object", additionalProperties: false, required: ["respondContactId"], properties: { respondContactId: reference, propertyId: uuid, propertyReference: reference, effectiveAt: reference } },
...
export const shadowContextTools = Object.freeze({
  resolve_contact_identity: async (db, respondContactId, context = {}) => {
    const identity = await resolveConfirmedContactIdentity(db, respondContactId, context);
    ...
    return [
      { entityType: "contact_identity", internalId: identity.clientContextKey, canonicalContactId: identity.canonicalContactId, linkId: identity.linkId, status: "confirmed", resolved: true, identityConfirmed: identity.identityConfirmed, linkSource: identity.linkSource, roles: identity.roles, ambiguousPropertyContext: identity.ambiguousPropertyContext, ambiguousRoleContext: identity.ambiguousRoleContext, relationshipCurrent: identity.relationshipCurrent, evidenceLevel: identity.evidenceLevel, evidenceSources: identity.evidenceSources, failClosedReason: identity.failClosedReason },
...
export async function executeShadowReadOnlyTool(admin, name, args) {
  ...
  if (name === "resolve_contact_identity") return shadowContextTools[name](admin, valid.respondContactId, { propertyId: valid.propertyId, propertyReference: valid.propertyReference, effectiveAt: valid.effectiveAt });
