...
export const SHADOW_TOOL_ARGUMENT_SCHEMAS = Object.freeze(Object.fromEntries(READ_ONLY_SHADOW_TOOLS.map((name) => [name, {
  type: "object", additionalProperties: false, required: ["query"], properties: { query: { type: "string", minLength: 1, maxLength: 120 } },
}])));
...
export async function executeShadowReadOnlyTool(admin, name, args) {
  ...
  if (!args || typeof args !== "object" || Array.isArray(args) || Object.keys(args).some((key) => key !== "query")) throw new Error("invalid_tool_arguments");
  const query = String(args.query || "").trim().slice(0, 120);
  if (!query) throw new Error("invalid_tool_arguments");
  return shadowContextTools[name](admin, query, { byReference: name === "find_properties" && !/^[0-9a-f-]{36}$/i.test(query) });
}
