...
import { condominiumIdentityToolRows } from "./condominiumIdentity.js";
import { ADMIN_WORK_R0_TOOLS, executeAdministrativeWorkR0 } from "../operaciones/durableAdministrativeWork.js";
...
const SERVICE_TYPE_ALIASES = Object.freeze({
  ...
  gas: "gas_recarga", gas_recarga: "gas_recarga", "gas lp": "gas_recarga",
  cfe: "luz", luz: "luz", electricity: "luz", electricidad: "luz",
  internet: "internet", mantenimiento: "mantenimiento",
});
const normalizeServiceType = (value) => {
  const normalized = String(value || "").trim().toLowerCase();
  const result = SERVICE_TYPE_ALIASES[normalized];
  if (!result) throw new Error("unsupported_service_type");
  return result;
};
...
const serviceLookup = {
  ...
  properties: { serviceId: uuid, propertyId: uuid, serviceType: reference, period: reference },
  oneOf: [
    { required: ["serviceId"] },
    { required: ["propertyId", "serviceType"] },
  ],
};
...
export const SHADOW_TOOL_ARGUMENT_SCHEMAS = Object.freeze({
  resolve_contact_identity: { type: "object", additionalProperties: false, required: ["respondContactId"], properties: { respondContactId: reference } },
  ...
  get_service_period_status: serviceLookup,
  ...
  list_administrative_work: { type: "object", additionalProperties: false, allowEmpty: true, properties: { clientIdentityId: uuid, contractId: uuid, propertyId: uuid, domain: reference, status: reference } },
  get_administrative_work: { type: "object", additionalProperties: false, required: ["workItemId"], properties: { workItemId: uuid } },
  find_administrative_work_by_context: { type: "object", additionalProperties: false, properties: { clientIdentityId: uuid, contractId: uuid, propertyId: uuid, sourceType: reference, sourceId: reference }, oneOf: ["clientIdentityId", "contractId", "propertyId", "sourceId"].map((key) => ({ required: [key] })) },
  find_possible_duplicate_work: { type: "object", additionalProperties: false, properties: { clientIdentityId: uuid, contractId: uuid, propertyId: uuid, domain: reference }, oneOf: ["clientIdentityId", "contractId", "propertyId"].map((key) => ({ required: [key] })) },
  get_administrative_work_history: { type: "object", additionalProperties: false, required: ["workItemId"], properties: { workItemId: uuid } },
  get_administrative_evidence_summary: { type: "object", additionalProperties: false, required: ["workItemId"], properties: { workItemId: uuid } },
  get_pending_approvals: { type: "object", additionalProperties: false, allowEmpty: true, properties: { workItemId: uuid } },
});
...
export function validateShadowToolArguments(name, args) {
  ...
  if ((!keys.length && !schema.allowEmpty) || keys.some((key) => !allowed.includes(key))) throw new Error("invalid_tool_arguments");
...
export const shadowContextTools = Object.freeze({
  resolve_contact_identity: async (db, respondContactId) => {
    const identity = await resolveConfirmedContactIdentity(db, respondContactId);
    if (identity.identityDomain === "condominium") return condominiumIdentityToolRows(identity, respondContactId);
    if (!identity.resolved) return [{ entityType: "contact_identity", internalId: respondContactId, status: identity.reason, resolved: false, conflicts: identity.reason === "identity_conflict" }];
    return [
      { entityType: "contact_identity", internalId: identity.clientContextKey, linkId: identity.linkId, status: "confirmed", resolved: true, linkSource: identity.linkSource, roles: identity.roles, ambiguousPropertyContext: identity.ambiguousPropertyContext },
      ...identity.contracts.map((row) => ({ entityType: "contract", internalId: row.id, propertyId: row.propertyId, status: row.status, active: row.active, method: "confirmed_identity_link", confidence: 100, reasonCode: "confirmed_identity_contract" })),
      ...identity.properties.map((row) => ({ entityType: "property", internalId: row.id, method: "confirmed_identity_link", confidence: 100, reasonCode: "confirmed_identity_property" })),
    ];
  },
  ...
  get_service_period_status: async (db, value) => {
    const args = typeof value === "string" ? { serviceId: value } : value;
    const mapRow = (serviceId) => (x) => ({
      entityType: "service", internalId: serviceId, paymentId: x.id,
      evidenceId: `service:${serviceId}:control:${x.id}`,
      label: `Servicio ${x.periodo || serviceId}`, status: x.status, period: x.periodo,
      amount: numericOrUndefined(x.monto), hasReceipt: Boolean(x.comprobante_url),
      method: args.serviceId ? "explicit_link" : "property_service_type_link", confidence: 100,
      reasonCode: args.serviceId ? "service_period_match" : "service_period_property_type_match",
      href: `/propiedades?serviceId=${encodeURIComponent(serviceId)}${x.periodo ? `&period=${encodeURIComponent(x.periodo)}&paymentId=${encodeURIComponent(x.id)}` : ""}`,
    });
    if (args.serviceId) return bounded(
      db.from("pagos_servicios").select("id,servicio_id,periodo,status,monto,comprobante_url").eq("servicio_id", args.serviceId).order("periodo", { ascending: false }),
      mapRow(args.serviceId),
    );

    const normalizedType = normalizeServiceType(args.serviceType);
    const { data: properties, error: propertyError } = await db.from("properties").select("id,name").eq("id", args.propertyId).limit(2);
    if (propertyError) throw propertyError;
    if ((properties || []).length !== 1) throw new Error("ambiguous_service_property_context");
    const propertyName = properties[0].name;
    const { data: services, error: serviceError } = await db.from("servicios_inmueble")
      .select("id,property_name,tipo,aplica").eq("property_name", propertyName).eq("tipo", normalizedType).eq("aplica", true).limit(2);
    if (serviceError) throw serviceError;
    if ((services || []).length !== 1) throw new Error((services || []).length ? "ambiguous_service_context" : "service_not_found");
    let query = db.from("pagos_servicios").select("id,servicio_id,periodo,status,monto,comprobante_url")
      .eq("property_name", propertyName).eq("tipo", normalizedType).order("periodo", { ascending: false });
    if (args.period) query = query.eq("periodo", args.period);
    const { data: payments, error: paymentError } = await query.limit(MAX_RESULTS);
    if (paymentError) throw paymentError;
    if ((payments || []).some((row) => row.servicio_id && row.servicio_id !== services[0].id)) {
      throw new Error("contradictory_service_period_context");
    }
    return (payments || []).map(mapRow(services[0].id));
  },
  get_maintenance_ticket_summary: (db, value) => { const args = typeof value === "string" ? { ticketId: value } : value; return bounded(
    args.ticketId
      ? db.from("maintenance_tickets").select("id,property_id,status,priority,category,assigned_to,created_at,updated_at").eq("id", args.ticketId)
      : db.from("maintenance_tickets").select("id,property_id,status,priority,category,assigned_to,created_at,updated_at").eq("property_id", args.propertyId).order("created_at", { ascending: false }),
    (x) => ({ entityType: "maintenance_ticket", internalId: x.id, label: `Mantenimiento ${x.status || x.id}`, status: x.status, priority: x.priority, category: x.category || null, responsibleId: x.assigned_to || null, createdAt: x.created_at || null, lastActionAt: x.updated_at || x.created_at || null, pending: !["cerrado","cancelado","terminado"].includes(String(x.status || "").toLowerCase()), method: "explicit_link", confidence: 100, reasonCode: "maintenance_ticket_match", href: `/mantenimiento?ticketId=${encodeURIComponent(x.id)}` }),
  ); },
  get_work_center_case: (db, contextKey) => bounded(
    db.from("administrative_case_controls").select("context_key,resolution_status,corrected_bucket,corrected_priority,responsible_profile_id,requires_authorization,updated_at").eq("context_key", contextKey),
    (x) => ({ entityType: "work_center_case", internalId: x.context_key, label: `Caso operativo ${x.resolution_status}`, status: x.resolution_status, priority: x.corrected_priority, bucket: x.corrected_bucket, responsibleId: x.responsible_profile_id || null, requiresAuthorization: Boolean(x.requires_authorization), lastActionAt: x.updated_at || null, method: "explicit_link", confidence: 100, reasonCode: "work_center_case_match", href: `/mi-trabajo-administrativo?contextKey=${encodeURIComponent(x.context_key)}` }),
  ),
...
export async function executeShadowReadOnlyTool(admin, name, args) {
  ...
  if (ADMIN_WORK_R0_TOOLS.includes(name)) return executeAdministrativeWorkR0(admin, name, valid);
  if (name === "resolve_contact_identity") return shadowContextTools[name](admin, valid.respondContactId);
  ...
  if (["find_active_contracts", "get_payment_summary", "get_service_period_status", "get_maintenance_ticket_summary"].includes(name)) return shadowContextTools[name](admin, valid);
