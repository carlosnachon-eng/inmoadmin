...
const SAFE_IDENTITY_SOURCES = new Set(["exact_phone_unique", "manual_admin_review", "erp_structured_link", "condominium_owner_admin_review"]);
...
function identityAttribution(run, cohort) {
  ...
  if (resolved.length !== 1) {
    const condoEvidence = identities.length === 1 && identities[0].identityDomain === "condominium" && identities[0].linkSource === "condominium_owner_admin_review";
    return {
      attribution: "unattributed",
      identityState: "unresolved",
      canonicalIdentityRef: null,
      resolutionSource: condoEvidence ? "condominium_owner_admin_review" : null,
      ...(condoEvidence ? { identityDomain: "condominium", unitResolved: false, unitRef: null, condominiumRef: null } : {}),
  ...
  if (identity.identityDomain === "condominium" && identity.linkSource === "condominium_owner_admin_review") {
    const units = [...new Map(rows.filter((r) => r.entityType === "condominium_unit" && r.method === "confirmed_identity_link" && r.active === true && r.internalId && r.condominiumId)
      .map((r) => [r.internalId, r])).values()];
    const unitResolved = units.length === 1 && identity.ambiguousUnitContext === false;
    return { attribution: "attributed", identityState: identity.status, canonicalIdentityRef: opaqueRef("canonical_identity", identity.internalId),
      resolutionSource: identity.linkSource, identityDomain: "condominium", exactPhone7of7Ref: null, inExactPhone7of7: false,
      propertyResolved: false, unitResolved, relationshipResolved: units.length > 0,
      unitRef: unitResolved ? opaqueRef("unit", units[0].internalId) : null,
      condominiumRef: unitResolved ? opaqueRef("condominium", units[0].condominiumId) : null,
      unitCount: units.length, attributionReason: unitResolved ? null : "insufficient_unit_context" };
  }
