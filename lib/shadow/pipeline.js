...
export async function processSyntheticShadowFixture(admin, [id, text, metadata]) {
  ...
  return processShadowEnvelope(admin, envelope, { fixtureId: id });
}

export async function processShadowEnvelope(admin, envelope, extra = {}) {
  const classification = classifyShadowMessage(envelope);
  const result = await ingestShadowEnvelope(admin, envelope);
  if (result?.status !== "accepted") return { ...result, ...extra, classification, context: null };
  if (envelope.direction !== "inbound") {
    return { ...result, ...extra, classification, context: null };
  }
  ...
  return { ...result, ...extra, classification, context };
}
