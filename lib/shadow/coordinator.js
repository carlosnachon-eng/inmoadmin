...
export const SHADOW_PROVIDERS = new Set(["synthetic", "respond", "respond_admin"]);
export const SHADOW_DIRECTIONS = new Set([
  "inbound", "outbound", "outbound_human", "outbound_ai_inmoadmin", "outbound_respond_ai", "outbound_unknown",
]);
...
export function validateShadowEnvelope(input) {
  ...
  const direction = clean(input.direction, 32).toLowerCase();
  ...
  const providerMetadata = safeObject(input.providerMetadata, [
    ...
    "workCenterContextKey", "salesRelevant", "syntheticScenario",
    "eventType", "messageType", "senderSource", "echoGatePending", "contentDisposition",
    "resolvedOrigin", "originResolution",
    "hasUninterpretedAttachments", "attachmentTypes", "respondContactId", "sourceChannelId", "assigneeId",
    "teamId", "teamInboxId", "workflowId", "lifecycleId", "routingReason", "routingEventType", "routedAt",
  ]);
  const attachmentMetadata = Array.isArray(input.attachmentMetadata)
    ? input.attachmentMetadata.slice(0, 5).map((item) => safeObject(item, ["type", "mimeType", "fileName", "size", "caption", "referenceHash"])).filter((item) => Object.keys(item).length)
    : [];
  ...
  return {
    ...
    rejectionReason: sanitized.rejected ? clean(input.rejectionReason, 80) || "sanitization_failure" : null,
    attachmentMetadata,
    providerMetadata,
...
export function classifyShadowMessage(envelope) {
  if (envelope.direction === "outbound_ai_inmoadmin") {
    return {
      administrativeLikelihood: "unknown",
      reasonCodes: ["inmoadmin_sender_echo"],
      intent: "no_determinado",
      requiresHuman: false,
    };
  }
  if (envelope.direction === "outbound_respond_ai") {
    return {
      administrativeLikelihood: "unknown",
      reasonCodes: ["respond_ai_echo"],
      intent: "no_determinado",
      requiresHuman: false,
    };
  }
  if (envelope.direction === "outbound_unknown") {
    return {
      administrativeLikelihood: "unknown",
      reasonCodes: ["unknown_outbound_origin"],
      intent: "no_determinado",
      requiresHuman: true,
    };
  }
  if (envelope.direction === "outbound_human") {
    return {
      administrativeLikelihood: "unknown",
...
export async function ingestShadowEnvelope(admin, rawEnvelope) {
  ...
  const { respondContactId, ...persistedProviderMetadata } = envelope.providerMetadata || {};
  const persistedEnvelope = { ...envelope, providerMetadata: persistedProviderMetadata };
  const { data, error } = await admin.rpc("ingest_shadow_message", {
    p_envelope: persistedEnvelope, p_classification: classification,
  });
  if (error) throw error;
  if (data?.conversationId && envelope.provider === "respond_admin" && envelope.channel === "544519" && respondContactId) {
    const { error: contactError } = await admin.from("shadow_conversations")
      .update({ respond_contact_id: respondContactId })
      .eq("id", data.conversationId).eq("provider", "respond_admin").eq("channel", "544519");
    if (contactError) throw contactError;
  }
  if (data?.status === "rejected" && data?.eventId && envelope.rejectionReason) {
    const { error: reasonError } = await admin
      .from("shadow_ingestion_events")
      .update({ error_code: envelope.rejectionReason })
      .eq("id", data.eventId)
      .eq("status", "rejected");
