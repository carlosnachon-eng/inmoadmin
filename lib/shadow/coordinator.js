...
export const SHADOW_PROVIDERS = new Set(["synthetic", "respond"]);
...
export function validateShadowEnvelope(input) {
  ...
  const providerMetadata = safeObject(input.providerMetadata, [
    ...
    "contractId", "workCenterContextKey", "salesRelevant", "syntheticScenario",
  ]);
  const attachmentMetadata = Array.isArray(input.attachmentMetadata)
    ? input.attachmentMetadata.slice(0, 5).map((item) => safeObject(item, ["type", "mimeType", "fileName", "size"])).filter((item) => Object.keys(item).length)
    : [];
...
const RULES = [
  ...
  ["reportar_mantenimiento", /(?:fuga|sale agua|no funciona|se descompuso|reparar)/i],
  ...
  ["contrato", /cancelar.*contrato|contrato.*(?:cancelar|vence)/i],
...
export function classifyShadowMessage(envelope) {
  ...
  const matches = RULES.filter(([, regex]) => regex.test(text)).map(([intent]) => intent);
  ...
  return { administrativeLikelihood: likelihood, reasonCodes, intent, requiresHuman: likelihood !== "high" || intent === "multintencion" };
}
...
export async function ingestShadowEnvelope(admin, rawEnvelope) {
  const envelope = validateShadowEnvelope(rawEnvelope);
