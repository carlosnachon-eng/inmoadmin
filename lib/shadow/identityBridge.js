import { CONDOMINIUM_RESPOND_SOURCE, resolveApprovedCondominiumIdentity } from "./condominiumIdentity.js";
...
export async function resolveConfirmedContactIdentity(admin, respondContactId, { audit = true } = {}) {
  ...
  const link = links[0];
  if (link.link_source === CONDOMINIUM_RESPOND_SOURCE) return resolveApprovedCondominiumIdentity(admin, link);
...
export async function reviewIdentityLink(admin, { linkId, action, actorProfileId }) {
  ...
  if (current.link_source === CONDOMINIUM_RESPOND_SOURCE) throw new Error("condominium_review_required");
