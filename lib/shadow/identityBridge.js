...
export async function generateIdentityCandidates(admin, { respondContactId, normalizedPhone }) {
  ...
  for (const row of candidateRows || []) {
    if (!row.inmoadmin_client_id) continue;
    const item = matches.get(row.inmoadmin_client_id) || { clientId: row.inmoadmin_client_id, contracts: [], properties: [] };
    if (row.contract_id) item.contracts.push(row.contract_id);
    if (row.property_id) item.properties.push(row.property_id);
    matches.set(row.inmoadmin_client_id, item);
  }
  ...
  for (const item of candidates) {
    const { data: existing, error: existingError } = await admin.from("respond_identity_links").select("id").eq("respond_contact_id", contactId).eq("inmoadmin_client_id", item.clientId).in("link_status", ["candidate", "confirmed", "conflict"]).maybeSingle();
    if (existingError) throw existingError;
    let link = existing;
    if (!link) {
      const { data: inserted, error } = await admin.from("respond_identity_links").insert({
        respond_contact_id: contactId, inmoadmin_client_id: item.clientId, link_status: status,
...
export async function resolveConfirmedContactIdentity(admin, respondContactId, { audit = true } = {}) {
  ...
  const { data: links, error } = await admin.from("respond_identity_links")
    .select("id,inmoadmin_client_id,link_status,link_source,confidence,confirmed_at")
  ...
  const [contractsResult, propertiesResult] = await Promise.all([
    admin.from("contracts").select("id,property_id,status,start_date,end_date").eq("tenant_id", link.inmoadmin_client_id),
    admin.from("properties").select("id,status").eq("owner_id", link.inmoadmin_client_id),
  ]);
  if (contractsResult.error) throw contractsResult.error;
  if (propertiesResult.error) throw propertiesResult.error;
  const contracts = (contractsResult.data || []).map((row) => ({ id: row.id, propertyId: row.property_id, status: row.status, active: activeRelation(row), startDate: row.start_date, endDate: row.end_date }));
  ...
  if (audit) await admin.from("respond_identity_audit").insert({ link_id: link.id, respond_contact_id: contactId, event_type: "resolved", context_ids: { clientId: link.inmoadmin_client_id, contractIds: contracts.map((x) => x.id).slice(0, 10), propertyIds: propertyIds.slice(0, 10) } });
  return {
    resolved: true, linkId: link.id, linkSource: link.link_source, linkStatus: link.link_status,
    confidence: Number(link.confidence), clientContextKey: link.inmoadmin_client_id, roles,
