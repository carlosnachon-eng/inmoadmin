...
export async function resolveRespondOutgoingOrigin(admin, envelope) {
  ...
  try {
    ...
    const resolvedOrigin = envelope.direction === "outbound_human" ? "human"
      : envelope.direction === "outbound_respond_ai" ? "respond_ai" : "unknown";
    return {
      ...envelope,
      providerMetadata: { ...envelope.providerMetadata, resolvedOrigin, originResolution: "respond_sender_source", echoGatePending: envelope.direction !== "outbound_human" },
    };
  } catch {
    return {
      ...envelope,
  ...
  try {
    const result = await Promise.race([
      (async () => {
        ...
        return processEnvelope(admin, envelope);
      })(),
