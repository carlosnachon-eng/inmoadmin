...
export async function generarContratoPromocion(data) {
  ...
  const firmaTable = () => new Table({
    ...
    rows: [new TableRow({ children: [
      new TableCell({ borders: noBorders, width: { size: 4680, type: WidthType.DXA }, children: [
        ...
        new Paragraph({ alignment: AlignmentType.CENTER, children: [bold(data.nombre_propietario)] }),
        new Paragraph({ alignment: AlignmentType.CENTER, children: [normal('EL PROPIETARIO')] }),
      ]}),
      new TableCell({ borders: noBorders, width: { size: 4680, type: WidthType.DXA }, children: [
        new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: '_____________________________________________', size: 20, font: 'Arial' })] }),
  ...
  const d = {
    ...
    monto_renta_letra:     data.monto_renta_letra || '',
    fecha_inicio:          fmtFechaDia(data.fecha_inicio || data.fecha_firma),
