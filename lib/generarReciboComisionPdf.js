...
export function generarReciboComisionPdf({ cierre, pagos = [], emitidoPor = null }) {
  ...
  const propiedad = safeText(cierre.propiedad) || "inmueble";
  ...
  doc.setFontSize(11);
  doc.setTextColor(dark);
  const body = `Emporio Inmobiliario hace constar que recibió la cantidad de ${fmt(recibido)} por concepto de comisión inmobiliaria correspondiente a la ${operacion} de ${propiedad}.`;
  doc.text(doc.splitTextToSize(body, pageW - margin * 2), margin, 378);

  doc.setFillColor("#FFFFFF");
  doc.setDrawColor("#E5E7EB");
  doc.roundedRect(margin, 426, pageW - margin * 2, 132, 10, 10, "D");
  drawLabelValue(doc, "Propiedad", propiedad, margin + 16, 452, 310);
  drawLabelValue(doc, "Operación", String(cierre.operacion || "RENTA").toUpperCase(), pageW - 180, 452, 130);
  drawLabelValue(doc, "Fecha cierre", cierre.fecha_cierre || "—", margin + 16, 508, 160);
  drawLabelValue(doc, "Asesor", cierre.vendedor || "—", 220, 508, 160);
  drawLabelValue(doc, "Monto comisión", fmt(cierre.comision), 390, 508, 150);
