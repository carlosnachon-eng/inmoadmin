...
const CONCEPTOS_NO_BI = new Set(['gasto_juridico', 'historico_validado', 'no_bi']);
...
export function clasificarMovimientoCajaPoliza({ movimiento }) {
  ...
  if (esNoBi) {
    return {
      ...
      estado_financiero: movimiento?.concepto === 'historico_validado' ? 'Histórico validado' : 'Fuera de BI',
      diferencia: 0,
      estado: 'conciliado',
      causa_probable: movimiento?.concepto === 'historico_validado'
        ? 'Cobro histórico validado manualmente. No requiere expediente porque corresponde a operación anterior al sistema.'
        : 'Movimiento clasificado fuera del BI financiero de póliza.',
