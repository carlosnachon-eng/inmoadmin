...
const STATUS_PAGO_COMPLETO = new Set(['pagado', 'cobrado', 'liquidado', 'completo']);
...
function montoMovimiento(mov) {
  return redondearMoneda(mov?.amount || mov?.monto || 0);
}

function montoCercano(a, b) {
...
function movimientoPerteneceATicket(mov, ticket) {
  ...
  const categoria = mov.category || mov.concepto;
  const monto = montoMovimiento(mov);
  const fechaMov = mov.date || mov.fecha || mov.created_at;
  const fechaTicket = ticket.updated_at || ticket.created_at;
  const dias = diasEntre(fechaTicket, fechaMov);
  const fechaCompatible = dias === null || dias <= 21;
  const propertyCompatible = property && text.includes(property);
  const esCobro = (mov.type || mov.tipo) === 'entrada' && CATEGORIAS_COBRO.has(categoria);
  const esCosto = (mov.type || mov.tipo) === 'salida' && CATEGORIAS_COSTO.has(categoria);

  if (propertyCompatible && fechaCompatible && esCobro) {
    if (montoCercano(monto, ticket.charged_amount) || montoCercano(monto, ticket.advance_amount)) return true;
  }

  if (propertyCompatible && fechaCompatible && esCosto && montoCercano(monto, ticket.provider_cost)) {
    return true;
  }
...
export function clasificarTicketMantenimiento({ ticket, quotes = [], cashMovements = [], properties = [], contracts = [] }) {
  ...
  const statusPago = normalizar(ticket.status_pago);
  const statusPagoCompleto = STATUS_PAGO_COMPLETO.has(statusPago);
  ...
  const evidenciaLiquidacion = descontadoLiquidacion || Boolean(ticket.fecha_cobro_propietario) || Boolean(ticket.recibo_cobro_id);
  const cobradoLiquidacion = descontadoLiquidacion ? montoCobrable : 0;
  const cobradoOperativo = statusPagoCompleto || evidenciaLiquidacion ? montoCobrable : 0;
  const cobradoAnticipoOperativo = anticipoPagado ? Math.min(anticipoRegistrado, montoCobrable) : 0;
  const cobradoTrazable = redondearMoneda(Math.max(cobradoCaja, cobradoLiquidacion, cobradoOperativo, cobradoAnticipoOperativo));
  ...
  const tieneEvidenciaCobro = cobradoCaja > 0 || evidenciaLiquidacion || statusPagoCompleto || (anticipoPagado && anticipoRegistrado > 0);
  ...
  return {
    ...
    evidencia: {
      ticket: {
        ...
        status_pago: ticket.status_pago || '—',
        advance_paid: Boolean(ticket.advance_paid),
        advance_amount: anticipoRegistrado,
      ...
      liquidacion: {
        ...
        evidencia_liquidacion: evidenciaLiquidacion,
        status_pago_completo: statusPagoCompleto,
        fecha_cobro_propietario: ticket.fecha_cobro_propietario || null,
