...
export function waitingResponseStats({ opportunities = [], snapshots = [], profileId = null }) {
  ...
  snapshots
    .filter((snap) => (!profileId || snap.mapped_profile_id === profileId) && snap.respond_unanswered_since)
    .forEach((snap) => {
  ...
  opportunities
    .filter((opp) => (!profileId || opp.asesor_id === profileId) && opp.respond_unanswered_since)
    .forEach((opp) => {
