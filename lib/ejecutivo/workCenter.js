...
export function serverAppEnvironment() {
  const explicit = String(process.env.APP_ENV || process.env.SUPABASE_ENVIRONMENT || "").toLowerCase();
  const vercel = String(process.env.VERCEL_ENV || "").toLowerCase();
  if (vercel === "production") return "production";
  if (explicit === "production" || explicit === "prod") return "production";
  if (explicit === "preview" || explicit === "dev" || explicit === "development" || explicit === "test") return "preview";
  return "preview";
}

export function publicAppEnvironment() {
  const explicit = String(process.env.NEXT_PUBLIC_APP_ENV || "").toLowerCase();
  if (explicit === "production" || explicit === "prod") return "production";
  return "preview";
}
...
export function isFase2AEnabledServer() {
  const value = String(process.env.FASE_2A_ENABLED || "").toLowerCase();
  if (isProductionApp()) return value === "true";
  return value !== "false";
}

export function isFase2AEnabledPublic() {
  const value = String(process.env.NEXT_PUBLIC_FASE_2A_ENABLED || "").toLowerCase();
  if (isPublicProductionApp()) return value === "true";
  return value !== "false";
}
...
function legacyCitaConfirmacion(cita) {
...
function respondInboxLink(contactId) {
...
export function buildWorkItems({ profileId, opportunities = [], snapshots = [], citas = [], seguimientos = [], profilesById = new Map() }) {
  ...
  citas
    ...
    .forEach((cita) => {
      items.push({
        confirmationStatus: legacyCitaConfirmacion(cita),
        id: `cita-${cita.id}`,
        type: "cita",
        source: "cita",
        types: ["cita"],
        severity: legacyCitaConfirmacion(cita) === "pendiente_confirmar" ? "alta" : "normal",
        title: "Cita de hoy",
        ...
        citaConfirmacionEstado: legacyCitaConfirmacion(cita),
        citaNotas: cita.notas || "",
        ownerName: safeName(profilesById.get(cita.asesor_id)),
        channel: "Cita",
        stage: legacyCitaConfirmacion(cita),
        nextAction: legacyCitaConfirmacion(cita) === "pendiente_confirmar" ? "Confirmar asistencia" : "Registrar resultado",
        dueAt: cita.fecha_hora,
        risk: legacyCitaConfirmacion(cita) === "pendiente_confirmar" ? "normal" : "bajo",
        conversationStatus: "",
      });
    });
...
export function calculateSummary({ opportunities = [], snapshots = [], citas = [], profileId = null }) {
  ...
  const appointmentStats = scopedCitas.reduce((acc, cita) => {
    const status = legacyCitaConfirmacion(cita);
    acc.scheduled += 1;
