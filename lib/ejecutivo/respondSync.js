...
export function respondMessageSignals(messages, existingSnapshot = null) {
  ...
  for (const message of messages || []) {
    ...
    if (message.traffic !== "outgoing") continue;
...
export function buildRespondSnapshot({
  ...
  const existingProfile = existingSnapshot?.mapped_profile_id
    ? (profiles || []).find((profile) => profile.id === existingSnapshot.mapped_profile_id)
    : null;
  const sameAssignee = Boolean(
    assignee?.id
    && existingSnapshot?.respond_assignee_id
    && String(assignee.id) === String(existingSnapshot.respond_assignee_id)
  );
  if (!mapped.profile && sameAssignee && existingProfile) {
    mapped = {
      profile: existingProfile,
      status: existingSnapshot.mapping_status || "matched",
      method: "preserved_same_assignee",
    };
  }
  ...
  const active = true;
  const deactivatedAt = relevance.salesRelevant && active && !blocked
    ? null
...
export function shouldPersistRespondSnapshot(snapshot, existingSnapshot) {
  return Boolean(existingSnapshot || snapshot?.sales_relevant);
}
...
export async function readRespondMessages(respondContactId, maxPages = RESPOND_MAX_MESSAGE_PAGES_PER_CONTACT) {
  ...
  while (nextPath && pagesRead < maxPages) {
    ...
    messages.push(...respondItems(body));
    nextPath = respondPaginationNext(body);
    pagesRead += 1;
    const signals = respondMessageSignals(messages);
    if (signals.lastInbound && signals.lastOutbound && messages.length >= RESPOND_MESSAGE_PAGE_LIMIT) break;
  }
