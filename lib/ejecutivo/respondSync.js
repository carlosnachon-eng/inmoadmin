...
export function respondMessageSignals(messages, existingSnapshot = null) {
  ...
  const unansweredSince = latestMessage
    ? (latestMessage.direction === "incoming" ? latestMessage.timestamp : null)
    : (lastInbound && (!lastOutbound || lastInbound > lastOutbound) ? lastInbound : null);
  return {
    lastInbound,
    lastOutbound,
    lastHumanOutbound,
