...
const estatusActivoRecibo = (estatus) => ['activo', 'solicitud_recibida'].includes(String(estatus || '').toLowerCase());

const formatMoneyShort = (value) => new Intl.NumberFormat('es-MX', {
  style: 'currency',
  currency: 'MXN',
...
const dividirSeguro = (numerador, denominador) => {
  ...
  if (!den) return 0;
  return toNumber(numerador) / den;
};

function construirLecturaContraMeta({
  year,
  corte,
  metaAnual,
  totalProyectado,
  ...
  const meta = roundMoney(metaAnual);
  const proyectado = roundMoney(totalProyectado);
  const faltanteMeta = roundMoney(Math.max(0, meta - proyectado));
  const cumplimiento = meta > 0 ? Math.round(dividirSeguro(proyectado, meta) * 1000) / 10 : null;
  const diasRestantes = Math.max(0, diasAnio - diasTranscurridos);
  const mesesRestantes = Math.max(1, Math.ceil(diasRestantes / 30.4375));
  const semanasRestantes = Math.max(1, Math.ceil(diasRestantes / 7));
  const ritmoMensualNecesario = faltanteMeta > 0 ? roundMoney(faltanteMeta / mesesRestantes) : 0;
  const ritmoSemanalNecesario = faltanteMeta > 0 ? roundMoney(faltanteMeta / semanasRestantes) : 0;
  const ritmoDiarioNecesario = faltanteMeta > 0 && diasRestantes > 0 ? roundMoney(faltanteMeta / diasRestantes) : 0;
  const operacionesEquivalentes = ticketPromedio > 0 && faltanteMeta > 0 ? Math.ceil(faltanteMeta / ticketPromedio) : 0;

  let estado = 'meta_por_definir';
  let estadoLabel = 'Meta por definir';
  let estadoColor = 'gray';
  if (meta > 0 && cumplimiento >= 100) {
    estado = 'en_ruta';
    estadoLabel = 'En ruta';
    estadoColor = 'green';
  } else if (meta > 0 && cumplimiento >= 85) {
    estado = 'riesgo_moderado';
    estadoLabel = 'Riesgo moderado';
    estadoColor = 'yellow';
  } else if (meta > 0) {
    estado = 'fuera_de_meta';
    estadoLabel = 'Fuera de meta';
    estadoColor = 'red';
  }

  const lectura = meta > 0
    ? proyectado >= meta
      ? `Al ritmo actual, Emporio superaría la meta anual por ${formatMoneyShort(proyectado - meta)}.`
      : `Al ritmo actual, Emporio cerraría ${formatMoneyShort(faltanteMeta)} por debajo de la meta anual.`
    : 'Todavía falta definir una meta anual confiable; por ahora el sistema muestra el rumbo real observado.';

  const recomendacion = meta > 0
    ? faltanteMeta > 0
      ? operacionesEquivalentes > 0
        ? `Para alcanzar la meta será necesario generar aproximadamente ${formatMoneyShort(ritmoMensualNecesario)} mensuales durante el resto del año, equivalente a ${operacionesEquivalentes} operación${operacionesEquivalentes === 1 ? '' : 'es'} adicionales del ticket promedio.`
        : `Para alcanzar la meta será necesario generar aproximadamente ${formatMoneyShort(ritmoMensualNecesario)} mensuales durante el resto del año.`
      : 'Mantener el ritmo actual. El foco debe estar en proteger la calidad de las operaciones y no forzar crecimiento innecesario.'
    : 'Definir una meta anual base para que el Centro pueda interpretar si el ritmo actual es suficiente o requiere intervención.';
  ...
  return {
    ...
    meta_anual: meta,
    meta_fuente: meta > 0 ? `Superar generado ${year - 1}` : 'Meta pendiente',
    ...
    ritmo_mensual_necesario: ritmoMensualNecesario,
    ritmo_semanal_necesario: ritmoSemanalNecesario,
    ritmo_diario_necesario: ritmoDiarioNecesario,
    ticket_promedio: roundMoney(ticketPromedio),
    operaciones_equivalentes: operacionesEquivalentes,
    lectura,
    recomendacion,
    corte,
    confirmado,
...
export function construirProyeccionGeneradoTotal({
  ...
  const ticketPromedioBase = [...comprometidoItems.map((item) => item.monto), confirmado].filter((monto) => toNumber(monto) > 0);
  const ticketPromedio = ticketPromedioBase.length
    ? roundMoney(ticketPromedioBase.reduce((acc, monto) => acc + toNumber(monto), 0) / ticketPromedioBase.length)
    : 0;
  const lecturaMeta = construirLecturaContraMeta({
    year,
    corte,
    metaAnual,
    totalProyectado,
  ...
  return {
    ...
    lectura: totalProyectado > 0
      ? `Si el año sigue caminando como viene, Emporio podría generar ${formatMoneyShort(totalProyectado)} en ${year}.`
