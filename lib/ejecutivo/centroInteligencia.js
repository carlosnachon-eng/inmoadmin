...
export function construirDiagnosticoCaja({ periodo, cashMovements = [], polizaCaja = [], resultadoOperativo = 0 }) {
  ...
  const alertas = [];
  if (Math.abs(diferenciaPorExplicar) > 0) {
    alertas.push('La diferencia no es necesariamente error: compara resultado operativo devengado contra flujo de caja real del periodo.');
  }
  ...
  return {
    ...
    categorias,
    alertas,
    nota: 'Diagnóstico de lectura: no modifica datos. Ayuda a explicar por qué el resultado operativo no coincide con el dinero disponible en caja.',
  };
}
