...
export async function generarPolizaJuridica(exp) {
  ...
  const filaCaratula = (etiqueta, valor) => new Paragraph({
    children: [bold(etiqueta + ' ', 20), normal(valor, 20)],
  ...
  const children = [
    ...
    // CARÁTULA
    // ══════════════════════════════════════════════════════
    centrado([bold('PÓLIZA JURÍDICA INMOBILIARIA', 28)], { spacing: { before: 0, after: 120 } }),

    // Datos de la póliza
    centrado([bold('DATOS DE LA PÓLIZA', 20)], { spacing: { before: 60, after: 40 } }),
    ...
    centrado([bold('DATOS DEL PROPIETARIO', 20)], { spacing: { before: 80, after: 40 } }),
    ...
    centrado([bold('DATOS DEL ARRENDATARIO', 20)], { spacing: { before: 80, after: 40 } }),
    ...
    // Datos del inmueble
    centrado([bold('DATOS DEL INMUEBLE ARRENDADO', 20)], { spacing: { before: 80, after: 40 } }),
    ...
    // Datos del contrato
    centrado([bold('DATOS DEL CONTRATO DE ARRENDAMIENTO', 20)], { spacing: { before: 80, after: 40 } }),
    filaCaratula('Fecha de firma:', d.fecha_firma),
    filaCaratula('Monto de renta mensual:', d.renta),

    // Observaciones
    centrado([bold('OBSERVACIONES IMPORTANTES', 20)], { spacing: { before: 80, after: 40 } }),
    para([
    ...
    divider(),
    centrado([bold('EMPORIO INMOBILIARIO', 20)], { spacing: { before: 40, after: 20 } }),
    centrado([italic('Tu respaldo jurídico al momento de rentar')], { spacing: { before: 0, after: 0 } }),
