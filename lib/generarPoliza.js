...
export async function generarPolizaJuridica(exp) {
  ...
  const normal  = (text, size = 20, font = FONT_CUERPO, color = COLOR_NEGRO) =>
    new TextRun({ text: String(text ?? ''), size, font, color })
  const italic  = (text, size = 18, font = FONT_CUERPO, color = COLOR_GRIS) =>
    new TextRun({ text: String(text ?? ''), italics: true, size, font, color })
  const rojo    = (text, size = 20) =>
  ...
  const seccionDatos = (titulo, filas) => [
    new Paragraph({ children: [new TextRun({ text: '', size: 16 })], spacing: { before: 30, after: 30 } }),
    new Paragraph({
      children: [rojo(titulo, 18)],
  ...
  const children = [
    ...
    new Paragraph({
      children: [
        normal('La presente póliza jurídica inmobiliaria ', 18),
        new TextRun({ text: 'no garantiza el cumplimiento del arrendatario ni la recuperación de pagos', bold: true, size: 18, font: FONT_CUERPO }),
        normal(', y se limita a la ', 18),
        new TextRun({ text: 'gestión jurídica preventiva, extrajudicial y, en su caso, judicial', bold: true, size: 18, font: FONT_CUERPO }),
        normal(', para la recuperación del inmueble y el deslinde de responsabilidades del propietario.', 18),
      ],
    ...
    ...espacio(1),
    divider(),
    centrado([bold('EMPORIO INMOBILIARIO', 18, FONT_TITULO, COLOR_ROJO)], { spacing: { before: 60, after: 20 } }),
    centrado([italic('Tu respaldo jurídico al momento de rentar', 18)], { spacing: { before: 0, after: 0 } }),
  ...
  const doc = new Document({
    styles: {
      default: {
        document: { run: { font: FONT_CUERPO, size: 20 } },
      },
    },
    sections: [{
      properties: {
        page: {
          ...
          margin: { top: 1800, right: 1200, bottom: 2160, left: 1200 },
        },
      },
      children,
    }],
