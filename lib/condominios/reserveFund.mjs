...
export function reserveFundEvidencePath({ condominioId, unidadId, contributionId, mimeType }) {
  const extension = reserveFundEvidenceExtension(mimeType);
  if (![condominioId, unidadId, contributionId].every(isReserveFundUuid) || !extension) return null;
  return `${condominioId}/${unidadId}/${contributionId}.${extension}`;
}
...
export function validateReserveFundCreateInput(payload = {}) {
  ...
  if (![payload.condominioId, payload.unidadId, payload.idempotencyKey].every(isReserveFundUuid)) return "INVALID_SCOPE";
...
export function reserveFundContributionPublicShape(row = {}) {
  return {
    id: row.id,
