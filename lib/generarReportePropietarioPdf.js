...
function rangoRapido(tipo, fechaInicioPromocion) {
  const hoy = new Date();
  const inicioPromo = fechaInicioPromocion ? new Date(fechaInicioPromocion) : null;

  if (tipo === "mes_actual") {
    const inicioMes = new Date(hoy.getFullYear(), hoy.getMonth(), 1);
    // Nunca mostrar actividad de antes de que la propiedad existiera
    // publicada — si la promoción empezó después del 1 del mes, usamos esa
    // fecha como inicio en vez del 1 del mes calendario.
    const inicio = inicioPromo && inicioPromo > inicioMes ? inicioPromo : inicioMes;
  ...
  if (tipo === "desde_publicacion") {
    const inicio = inicioPromo || new Date(hoy.getFullYear(), hoy.getMonth(), 1);
    return { desde: inicio.toISOString().split("T")[0], hasta: hoy.toISOString().split("T")[0] };
  }
...
export default function ReportePropietario() {
  ...
  const elegirPropiedad = async (p) => {
    ...
    // Recalcula "Este mes" considerando la fecha real de inicio de
    // promoción de esta propiedad — evita mostrar "01 jun" en una
    // propiedad que apenas se publicó hace unos días.
    const r = rangoRapido("mes_actual", prop?.fecha_inicio_promocion);
    setDesde(r.desde);
    setHasta(r.hasta);
  };

  const aplicarRango = (tipo) => {
    const r = rangoRapido(tipo, propietario?.fecha_inicio_promocion);
    setDesde(r.desde);
    setHasta(r.hasta);
  };
        ...
        {propiedadSel && (
          <>
            ...
            <div style={{ background: "#fff", borderRadius: 12, padding: 16, marginBottom: 16, border: "1px solid #f0f0f0", display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              ...
              {propietario?.fecha_inicio_promocion && (
                <button onClick={() => aplicarRango("desde_publicacion")} style={{ background: "#f3f4f6", border: "none", borderRadius: 8, padding: "8px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Desde que se publicó</button>
              )}
