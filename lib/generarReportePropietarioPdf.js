...
export async function generarReportePropietarioPdf({ propiedad, propietario, periodo, datos, asesorNombre }) {
  ...
  // ── Resumen ejecutivo: un número protagonista + frase con criterio ──────
  // En vez de 4 cajas iguales compitiendo, el dato más representativo del
  // periodo (visitas a la ficha) se vuelve grande y editorial; el resto de
  // datos pasan a ser soporte secundario más abajo.
  ensureSpace(80);
  const numeroProtagonista = String(datos.visitas.length);
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(CARBON);
  doc.text("VISITAS A LA FICHA", numeroX, y + 58);
  ...
  const vistasRedes = [
    propiedad.vistas_tiktok > 0 ? `${propiedad.vistas_tiktok.toLocaleString("es-MX")} vistas en TikTok` : null,
    propiedad.vistas_instagram > 0 ? `${propiedad.vistas_instagram.toLocaleString("es-MX")} en Instagram` : null,
    propiedad.vistas_facebook > 0 ? `${propiedad.vistas_facebook.toLocaleString("es-MX")} en Facebook` : null,
  ].filter(Boolean);
  if (vistasRedes.length > 0) bullet(vistasRedes.join("  ·  "));
  y += 10;
