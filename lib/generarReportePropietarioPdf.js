...
doc.text(kpiPrincipal.etiqueta, numeroX, y + 58);
  ...
  if (propiedad.status === "reserved") {
    tituloSeccion("Estado comercial");
    const desde = propiedad.apartado_fecha ? fmtFechaCorta(propiedad.apartado_fecha) : "fecha no registrada";
    const hasta = propiedad.apartado_vigencia_hasta
      ? fmtFechaCorta(`${propiedad.apartado_vigencia_hasta}T12:00:00`)
      : "fecha no registrada";
    const notaReserva = `La propiedad se encuentra reservada por un prospecto calificado desde ${desde}. El apartado está vigente hasta ${hasta}, sujeto a firma de contrato/promesa.`;
    const lineasReserva = doc.splitTextToSize(notaReserva, contentWidth);
    doc.setFont("Helvetica", "normal");
    doc.setFontSize(10);
    doc.setTextColor(CARBON);
    doc.text(lineasReserva, marginL, y);
    y += lineasReserva.length * 14 + 18;
  }
...
} else if (propiedad.status === "reserved") {
  bullet("La promoción como propiedad disponible se encuentra pausada durante la vigencia del apartado.");
} else {
  bullet("La propiedad no se encuentra en promoción activa durante este periodo.");
}
y += 10;
