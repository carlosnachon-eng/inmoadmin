...
export async function generarReportePropietarioPdf({ propiedad, propietario, periodo, datos }) {
  ...
  if (datos.comparativaAnterior) {
    ...
    for (const [label, actual, anterior] of comparaciones) {
      ...
      doc.setFontSize(9.5);
      doc.setTextColor(GRIS_MEDIO);
      doc.text(`${label}: ${actual} (antes ${anterior})`, margin, y);
      doc.setFont("helvetica", "bold");
  ...
  if (datos.citas.length > 0) {
    ...
    for (const linea of resumenCitas) {
      ensureSpace(13);
  ...
  doc.setFontSize(11);
  doc.setTextColor(GRIS_OSCURO);
  doc.text("Canales de promoción", margin, y);
  y += 16;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9.5);
  doc.setTextColor(GRIS_MEDIO);
  doc.text("•  Publicada en el sitio web de Emporio Inmobiliario", margin, y);
  y += 14;
  if (propiedad.en_marketplace) {
    ensureSpace(14);
    doc.text("•  Publicada en Facebook Marketplace", margin, y);
    y += 14;
  }
  const vistasRedes = [
  ...
  if (vistasRedes.length > 0) {
    ensureSpace(14);
    doc.text(`•  ${vistasRedes.join("   ·   ")}`, margin, y);
    y += 14;
  }
  y += 8;
