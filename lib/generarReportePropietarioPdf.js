...
export async function generarReportePropietarioPdf({ propiedad, propietario, periodo, datos }) {
  ...
  tarjetas.forEach(([valor, label], i) => {
    ...
    doc.setFillColor(datos.solicitudes.length > 0 && i === 2 ? VERDE_BG : GRIS_BG);
    doc.roundedRect(bx, y, colW - 8, boxH, 6, 6, "F");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(20);
    doc.setTextColor(i === 2 && datos.solicitudes.length > 0 ? VERDE_TXT : GRIS_OSCURO);
  ...
  if (datos.visitas.length === 0 && datos.envios.length === 0 && datos.solicitudes.length === 0) {
    ensureSpace(20);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    doc.setTextColor(GRIS_MEDIO);
