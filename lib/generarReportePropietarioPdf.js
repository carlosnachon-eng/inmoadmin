...
// Normaliza nombres de personas que pueden venir en MAYÚSCULAS o minúsculas
// desde la base de datos (captura manual del equipo), a Formato Título —
// "EDSON GARCIA PERALTA" -> "Edson Garcia Peralta". Las partículas comunes
// de apellidos compuestos en español se mantienen en minúscula.
...
export async function generarReportePropietarioPdf({ propiedad, propietario, periodo, datos, asesorNombre }) {
  ...
  doc.setFontSize(11);
  doc.setTextColor(GRIS_OSCURO);
  doc.text(`Hola ${formatearNombre(propietario?.nombre_propietario)},`, margin, y);
  y += 18;
  doc.setFontSize(9.5);
  ...
  // El título puede ser largo (las propiedades suelen tener títulos
  // descriptivos de una línea completa); se calcula cuántas líneas
  // necesita ANTES de dibujar la franja, para que la franja crezca y
  // nunca se pierda texto fuera de ella.
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12.5);
  const tituloLineas = doc.splitTextToSize(propiedad.titulo || "Propiedad", contentWidth - 24).slice(0, 2);
  const direccionTexto = [propiedad.direccion, propiedad.colonia, propiedad.ciudad].filter(Boolean).join(", ");
  const alturaTextoTitulo = tituloLineas.length * 14;
  const franjaHFinal = alturaTextoTitulo + 28; // espacio para el título + la línea de dirección

  if (doc.GState) {
    doc.saveGraphicsState();
    doc.setGState(new doc.GState({ opacity: 0.55 }));
    doc.setFillColor(0, 0, 0);
    doc.rect(margin, y + bannerH - franjaHFinal, contentWidth, franjaHFinal, "F");
  ...
  let tituloY = y + bannerH - franjaHFinal + 16;
  for (const linea of tituloLineas) {
    doc.text(linea, margin + 12, tituloY);
    tituloY += 14;
  }
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor("#e5e7eb");
  doc.text(direccionTexto, margin + 12, tituloY + 4);
