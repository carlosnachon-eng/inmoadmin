...
// Genera contrato de arrendamiento en .docx
// Soporta: habitacional | amueblado | comercial
// Soporta: persona física | persona moral
...
function numeroALetra(n) {
  ...
  function miles(num) {
    if (num===0) return ''; if (num<1000) return menorMil(num)
    const m=Math.floor(num/1000), r=num%1000
    let res=m===1?'MIL':menorMil(m)+' MIL'
    if (r>0) res+=' '+menorMil(r); return res.trim()
  }
  function millones(num) {
    if (num===0) return 'CERO'; if (num<1000000) return miles(num)
    const m=Math.floor(num/1000000), r=num%1000000
    let res=m===1?'UN MILLÓN':menorMil(m)+' MILLONES'
    if (r>0) res+=' '+miles(r); return res.trim()
  }
...
export async function generarContratoArrendamiento(exp) {
  ...
  // ── Tipo de contrato y arrendatario ─────────────────────────
  const tipo = exp.tipo_contrato || 'habitacional' // habitacional | amueblado | comercial
  const esMoral = exp.tipo_arrendatario === 'moral'
  const esAmueblado = tipo === 'amueblado'
  const esComercial = tipo === 'comercial'
  const giroComercial = exp.giro_comercial || '[Describir el giro exacto]'
  ...
  const para = (children, opts = {}) => new Paragraph({
    ...
    spacing: { before: 80, after: 80 }, alignment: AlignmentType.JUSTIFIED, ...opts,
  })
  const centrado = (children, opts = {}) => new Paragraph({
    children: Array.isArray(children) ? children : [children],
    spacing: { before: 80, after: 80 }, alignment: AlignmentType.CENTER, ...opts,
  })
  ...
  const d = {
    ...
    nombre_arrendatario:        esMoral ? (exp.razon_social_arrendatario || exp.nombre_arrendatario || '') : (exp.nombre_arrendatario || ''),
    razon_social:               exp.razon_social_arrendatario || '',
    representante:              exp.representante_legal_arrendatario || '',
    ...
    renta_letra:                exp.renta_mensual_letra || numeroALetra(Number(exp.renta_mensual)),
    deposito:                   Number(exp.deposito_garantia) || 0,
    deposito_letra:             exp.deposito_garantia_letra || numeroALetra(Number(exp.deposito_garantia)),
  ...
  // ── Título según tipo ─────────────────────────────────────
  const tituloContrato = esComercial
    ? 'CONTRATO DE ARRENDAMIENTO COMERCIAL'
    : esAmueblado
      ? 'CONTRATO DE ARRENDAMIENTO DE INMUEBLE AMUEBLADO'
      : 'CONTRATO DE ARRENDAMIENTO'

  // ── Declaración 1.2 ───────────────────────────────────────
  const decl12 = esComercial
    ? [para([bold('1.2. '), normal('Que es legítimo propietario del inmueble ubicado en: '), bold(d.direccion), normal('.')])]
    : esAmueblado
      ? [
          para([bold('1.2. '), normal('Que es legítimo propietario de la Casa Habitación ubicada en: '), bold(d.direccion), normal('.')]),
          para([normal('Así mismo declara que es propietario de los bienes que amueblan la vivienda. Dichos bienes muebles se señalan en el inventario, firmado por las partes, que se adjunta al presente documento, como parte integrante del contrato.')]),
        ]
      : [para([bold('1.2. '), normal('Que es legítimo propietario de la Casa Habitación ubicada en: '), bold(d.direccion), normal('.')])]

  // ── Declaración 1.3 ───────────────────────────────────────
  const decl13 = esComercial
    ? para([bold('1.3. '), normal('Que su deseo celebrar el presente contrato de Arrendamiento, a efecto de que "EL ARRENDATARIO" tome en arrendamiento el inmueble citado en el punto número 1.2 párrafo que antecede, el cual será utilizado exclusivamente para el giro comercial de: '), bold(giroComercial), normal('.')])
  ...
  // ── Declaración 2.1 ───────────────────────────────────────
  const decl21 = esMoral
    ? para([bold('2.1. '), normal('Que es persona moral de nacionalidad mexicana, denominada '), bold(d.razon_social),
        normal(', con domicilio en: '), bold(d.domicilio_arrendatario),
        normal(', con RFC: '), bold(d.rfc_arrendatario),
        normal(', representada en este acto por su representante legal '), bold(d.representante),
        normal(', con número de contacto '), bold(d.telefono_arrendatario),
        normal(', con correo electrónico: '), bold(d.correo_arrendatario),
        normal(' y que cuenta con la suficiente capacidad jurídica para celebrar el presente contrato.'),
      ])
  ...
  // ── Declaración 2.7 (solo amueblado) ─────────────────────
  const decl27 = esAmueblado
    ? [para([bold('2.7. '), normal('"EL ARRENDATARIO" declara que el inmueble y los muebles que se encuentran en él, están en perfecto estado de uso y conservación, y acepta el valor que se le asigna a los bienes muebles en el inventario.')])]
    : []

  // ── Segunda cláusula ─────────────────────────────────────
  const clausulaSegunda = esAmueblado
    ? clausula('SEGUNDA. CALIDAD DEL BIEN INMUEBLE.',
        para([normal('Por medio de la presente cláusula "EL ARRENDADOR" manifiesta que el bien inmueble y los muebles que se encuentran en él, ambos objeto del presente contrato se encuentra en perfecto estado de uso y conservación, y desde este momento "EL ARRENDATARIO" acepta el valor que se le asigna a los bienes muebles en el inventario.')]),
      )
    : clausula('SEGUNDA. CALIDAD DEL BIEN INMUEBLE.',
  ...
  // ── Cuarta cláusula (forma de pago) ──────────────────────
  const pagoCuarta = d.forma_pago?.toLowerCase().includes('transferencia') && d.clabe
    ? [normal(' mediante transferencia electrónica a la CLABE '), bold(d.clabe),
       normal(' perteneciente a la institución bancaria '), bold(d.banco || ''), normal(' a nombre del "ARRENDADOR"')]
    : [normal(' mediante '), bold(d.forma_pago)]

  // ── Texto cláusula 11 ─────────────────────────────────────
  const uso11 = esComercial
    ? `El bien inmueble será destinado única y exclusivamente para el giro comercial de: ${giroComercial}, del cual podrá usar y disponer en forma ordenada y pacífica, quedando prohibido expresamente el uso del mismo contrario a la ley, la moral y las buenas costumbres.`
    : 'El bien inmueble será destinado única y exclusivamente para USO HABITACIONAL, del cual podrá usar y disponer en forma ordenada y pacífica, quedando prohibido expresamente el uso del mismo contrario a la ley, la moral y las buenas costumbres.'

  // ── Obligaciones del arrendador (cláusula 8) ─────────────
  const obligArrendador = [
    esAmueblado
      ? para([normal('- Entregar a "EL ARRENDATARIO" el bien objeto de este contrato, los muebles, todos sus accesorios y servidumbres en condiciones óptimas para el uso pactado;')])
    ...
    esAmueblado
      ? para([normal('- Responder por los daños que se causen al inmueble en cuanto a desperfectos estructurales, así como los vicios ocultos (instalaciones eléctricas, hidráulicas, etc.) y los muebles los cuales deberán notificársele por escrito a más tardar treinta días naturales;')])
    ...
    ...(!esComercial ? [para([normal('- Entregar reglamentos ya sea de forma impresa o digital según sea el caso;')])] : []),
    ...(!esComercial ? [para([normal(
  ...
  const obligArrendatario = [
    ...
    esComercial
      ? para([normal('- No usar o disponer el inmueble para un giro comercial distinto al pactado entre las partes.')])
    ...
    esAmueblado
      ? para([normal('- Devolver el inmueble y los muebles en el existentes a "EL ARRENDADOR" en las mismas condiciones en que lo recibió originalmente.')])
      : para([normal('- Devolver el inmueble a "EL ARRENDADOR" en las mismas condiciones en que lo recibió originalmente.')]),
    ...(!esComercial ? [para([normal('- Respetar el Reglamento Interior de Condominios si hubiere; por lo que en caso de ser acreedor a una multa por el incumplimiento del mismo deberá hacerse responsable.')])] : []),
  ]

  // ── Cláusula de mascotas (15) ─────────────────────────────
  const clausulaMascotas = !esComercial
    ? clausula('DÉCIMA QUINTA. SOBRE LA TENENCIA DE ANIMALES.',
        ...
        ...(d.mascotas === 'si' || d.mascotas === 'condicionado' ? [
          ...
          para([normal('Como dueño de la mascota, "EL ARRENDATARIO" es estrictamente responsable de toda cantidad incurrida por cualquier daño que la mascota cause, ya sea agrediendo a una(s) persona(s) y/o a la propiedad.')]),
        ] : []),
      )
    : []

  // ── Cláusula 15 comercial (exclusión de gastos) ──────────
  const clausula15 = esComercial
    ? clausula('DÉCIMA QUINTA. DE LA EXCLUSIÓN DE GASTOS.',
        para([normal('"LAS PARTES" acuerdan que en el pago de la renta no incluye servicios, como agua potable, energía eléctrica, gas, teléfono, internet, alarma o cualquier otro servicio que se contrate con terceros, serán por cuenta exclusiva de "EL ARRENDATARIO", quien deberá justificar que se encuentra al corriente en dichos pagos, cada vez que sea requerido por "EL ARRENDADOR", comprometiéndose a mostrar los originales de los recibos de pago debidamente pagados, a "EL ARRENDADOR".')]),
      )
    : clausula('DÉCIMA QUINTA. SOBRE LA TENENCIA DE ANIMALES.',
        ...clausulaMascotas.slice(1)
      )

  // ── Cláusula 16 (exclusión gastos / derecho del tanto) ───
  const clausula16Titulo = esComercial ? 'DÉCIMA SEXTA. DERECHO DEL TANTO Y TRANSMISIÓN DEL INMUEBLE.' : 'DÉCIMA SEXTA. DE LA EXCLUSIÓN DE GASTOS.'
  const clausula16Contenido = esComercial
    ? [
  ...
  // ── Cláusula 17 ──────────────────────────────────────────
  const clausula17Titulo = esComercial ? 'DÉCIMA SÉPTIMA. DEL JUICIO ORAL SUMARISIMO.' : 'DÉCIMA SÉPTIMA. DERECHO DEL TANTO Y TRANSMISIÓN DEL INMUEBLE.'
  const clausula17Contenido = esComercial
  ...
  // ── Numeración de cláusulas 18-25 según tipo ─────────────
  // Habitacional/Amueblado: 18=JuicioOral, 19=FueraMayor, 20=Robo, 21=Privacidad, 22=Rescisión, 23=Notificaciones, 24=MediosElect, 25=Tribunales
  // Comercial:              18=CasoFortuito, 19=Robo, 20=Licencias, 21=Privacidad, 22=Rescisión, 23=Notificaciones, 24=MediosElect, 25=Tribunales

  const clausulasFinales = esComercial ? [
    ...clausula('DÉCIMA OCTAVA. CASO FORTUITO O CAUSAS DE FUERZA MAYOR.',
    ...
    ...clausula('DÉCIMA NOVENA. DE LA RESPONSABILIDAD DE EL ARRENDATARIO POR ROBO.',
    ...
    ...clausula('VIGÉSIMA. DE LAS LICENCIAS Y PERMISOS DE FUNCIONAMIENTO.',
      para([normal('"EL ARRENDATARIO", se obliga a tramitar y obtener, por su cuenta y riesgo, todas las licencias, permisos, autorizaciones, y/o avisos de funcionamiento necesarios ante las autoridades municipales, estatales y federales para el giro comercial de: '), bold(giroComercial), normal('.')]),
      para([normal('"EL ARRENDATARIO" asume total responsabilidad por cualquier sanción, multa o clausura que las autoridades competentes impongan debido a la falta de documentos o al incumplimiento de las normativas aplicables, liberando a "EL ARRENDADOR" de cualquier responsabilidad civil, administrativa o penal.')]),
      para([normal('"EL ARRENDADOR" se obliga a entregar el inmueble con el uso de suelo permitido para actividades comerciales y a facilitar al arrendatario la documentación de propiedad necesaria para dichos trámites.')]),
      para([normal('"EL ARRENDATARIO" deberá entregar al arrendador copia de las licencias obtenidas al inicio y en cada refrendo anual.')]),
    ),
  ...
  // ── Anexo B inventario muebles (solo amueblado) ──────────
  const anexoB = esAmueblado ? [
    pageBreak(),
    centrado([bold(`ANEXO "B" — INVENTARIO DE BIENES MUEBLES`)]),
    centrado([bold(`INMUEBLE: ${d.direccion}`)]),
    centrado([bold('LAS PARTES FIRMAN DE CONFORMIDAD CON EL INVENTARIO ADJUNTO.')]),
    espacio(),
    new Table({
      ...
      rows: [
        new TableRow({ children: [
          new TableCell({ shading: { fill: 'D5E8F0', type: ShadingType.CLEAR }, margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [bold('DESCRIPCIÓN DEL BIEN')] })] }),
          new TableCell({ shading: { fill: 'D5E8F0', type: ShadingType.CLEAR }, margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [bold('CANTIDAD')] })] }),
          new TableCell({ shading: { fill: 'D5E8F0', type: ShadingType.CLEAR }, margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [bold('ESTADO')] })] }),
          new TableCell({ shading: { fill: 'D5E8F0', type: ShadingType.CLEAR }, margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [bold('OBSERVACIONES')] })] }),
        ]}),
        ...Array.from({ length: 12 }, () => new TableRow({ children: [
          new TableCell({ margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [normal('')] })] }),
          new TableCell({ margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [normal('')] })] }),
          new TableCell({ margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [normal('')] })] }),
          new TableCell({ margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [normal('')] })] }),
        ]}))
  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        // Título
        para([
          bold(`${tituloContrato} QUE CELEBRAN POR UNA PARTE EL C. `), bold(d.nombre_arrendador),
          bold(' A QUIEN EN LO SUCESIVO SE LE DENOMINARÁ '), bold('"EL ARRENDADOR"'),
          bold(', Y POR OTRA PARTE '), bold(esMoral ? d.razon_social : d.nombre_arrendatario),
        ...
        ...clausula('CUARTA. RENTA MENSUAL Y FORMA DE PAGO.',
          para([
            normal('"LAS PARTES" acuerdan que "EL ARRENDATARIO" pagará '),
            ...pagoCuarta,
            normal(' a "EL ARRENDADOR" o a quien en su derecho la represente, la cantidad de '),
        ...
        ...clausula('NOVENA. RESCISIÓN DE CONTRATO Y DESOCUPACIÓN DEL INMUEBLE.',
          ...
          ...(!esComercial ? [para([normal('- Incurrir de manera reiterada al reglamento del clúster y/o fraccionamiento;')])] : []),
        ...
        ...clausula(esComercial ? 'DÉCIMA QUINTA. DE LA EXCLUSIÓN DE GASTOS.' : 'DÉCIMA QUINTA. SOBRE LA TENENCIA DE ANIMALES.',
          ...(esComercial
            ? [para([normal('"LAS PARTES" acuerdan que en el pago de la renta no incluye servicios, como agua potable, energía eléctrica, gas, teléfono, internet, alarma o cualquier otro servicio que se contrate con terceros, serán por cuenta exclusiva de "EL ARRENDATARIO", quien deberá justificar que se encuentra al corriente en dichos pagos, cada vez que sea requerido por "EL ARRENDADOR", comprometiéndose a mostrar los originales de los recibos de pago debidamente pagados, a "EL ARRENDADOR".')])]
            : clausulaMascotas.slice(1)
          ),
        ),
        ...clausula(clausula16Titulo, ...clausula16Contenido),
        ...clausula(clausula17Titulo, ...clausula17Contenido),
        ...clausulasFinales,

        espacio(), espacio(),
        para([normal('Enteradas las partes previa lectura del presente instrumento, el cual consta de '), normal(esComercial ? 'ocho' : 'nueve'), normal(' hojas útiles y pleno conocimiento de su contenido y alcance legal que causa el presente contrato, lo firman por triplicado en su totalidad en la ciudad de '), bold(d.ciudad), normal(' el día '), bold(d.ff), normal('.')]),
        espacio(), espacio(), espacio(),
        firmaTable({ nombre: esMoral ? d.razon_social : d.nombre_arrendatario, rol: '"EL ARRENDATARIO"' }, { nombre: d.nombre_arrendador, rol: '"EL ARRENDADOR"' }),
        ...
        // Anexo B muebles (solo amueblado)
        ...anexoB,
      ]
    }]
  })
  ...
  a.href = url
  a.download = `Contrato_${tipo}_${(esMoral ? d.razon_social : d.nombre_arrendatario).replace(/\s+/g, '_')}_${exp.fecha_firma || 'borrador'}.docx`
  a.click()
