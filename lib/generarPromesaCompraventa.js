...
export async function generarPromesaCompraventa(data) {
  ...
  // ── Género ──────────────────────────────────────────────────────────────
  const sexoV = data.curp_vendedor && data.curp_vendedor.length >= 11 ? (data.curp_vendedor[10].toUpperCase() === "M" ? "mujer" : "hombre") : "hombre"
  const sexoC = data.curp_comprador && data.curp_comprador.length >= 11 ? (data.curp_comprador[10].toUpperCase() === "M" ? "mujer" : "hombre") : "hombre"
