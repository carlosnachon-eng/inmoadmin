...
export async function generarPromesaCompraventa(data) {
  ...
  const SIZE_SEC   = 20   // 10pt para encabezados de sección

  const bold   = (text, size = SIZE_BODY, font = FONT_BODY) => new TextRun({ text: String(text ?? ''), bold: true,   size, font })
  const normal = (text, size = SIZE_BODY, font = FONT_BODY) => new TextRun({ text: String(text ?? ''),                size, font })
  const italic = (text, size = SIZE_BODY, font = FONT_BODY) => new TextRun({ text: String(text ?? ''), italics: true, size, font })
  ...
  const espacio = () => new Paragraph({
    children: [new TextRun({ text: '', size: SIZE_SMALL })],
    spacing: { before: 40, after: 40 },
  })

  const espGrande = () => new Paragraph({
    children: [new TextRun({ text: '', size: SIZE_SMALL })],
    spacing: { before: 100, after: 100 },
  })

  // Encabezado de sección con línea horizontal
  const seccion = (texto) => new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { before: 280, after: 200 },
    border: {
      bottom: { style: BorderStyle.SINGLE, size: 6, color: '1a1a1a', space: 6 },
    },
  ...
  // ── Género por CURP (posición 10) ───────────────────────────────────────
  const sexoV = data.curp_vendedor && data.curp_vendedor.length >= 11
    ? (data.curp_vendedor[10].toUpperCase() === 'M' ? 'mujer' : 'hombre') : 'hombre'
  const sexoC = data.curp_comprador && data.curp_comprador.length >= 11
    ? (data.curp_comprador[10].toUpperCase() === 'M' ? 'mujer' : 'hombre') : 'hombre'
  ...
  const firmaTable = () => new Table({
    ...
    rows: [new TableRow({ children: [
      new TableCell({ borders: noBorders, width: { size: 4680, type: WidthType.DXA }, children: [
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 60 }, children: [bold('"EL PROMITENTE VENDEDOR"', SIZE_SMALL)] }),
        espacio(), espacio(), espacio(), espacio(),
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { before: 0, after: 80 },
          border: { top: { style: BorderStyle.SINGLE, size: 4, color: '1a1a1a', space: 4 } },
          children: [new TextRun({ text: '', size: SIZE_SMALL })],
        }),
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 40 }, children: [bold(d.nombre_vendedor.toUpperCase(), SIZE_SMALL)] }),
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 0 }, children: [normal('Promitente Vendedor', SIZE_SMALL)] }),
      ]}),
      new TableCell({ borders: noBorders, width: { size: 4680, type: WidthType.DXA }, children: [
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 60 }, children: [bold('"EL PROMITENTE COMPRADOR"', SIZE_SMALL)] }),
        espacio(), espacio(), espacio(), espacio(),
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { before: 0, after: 80 },
        ...
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 40 }, children: [bold(d.nombre_comprador.toUpperCase(), SIZE_SMALL)] }),
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 0 }, children: [normal('Promitente Comprador', SIZE_SMALL)] }),
      ]}),
    ]})],
  })
  ...
  const parrafosGravamen = () => {
    if (d.gravamen) {
      return [
        ...
        espacio(),
        para([normal('Asimismo, las partes acuerdan que la liberación del gravamen será condición indispensable para la firma de la escritura definitiva, por lo que en ningún caso podrá formalizarse la compraventa sin que el inmueble se encuentre libre de gravámenes.')]),
        espacio(),
      ]
    }
    return [
    ...
    sections: [{
      properties: {
        page: {
          ...
          margin: { top: 1800, right: 1620, bottom: 1800, left: 1620 },
        },
      },
      children: [
        ...
        new Paragraph({
          ...
          border: {
            top: { style: BorderStyle.SINGLE, size: 8, color: '1a1a1a', space: 8 },
          },
        ...
        new Paragraph({
          ...
          spacing: { before: 100, after: 100 },
          children: [new TextRun({
            text: 'CONTRATO DE PROMESA DE COMPRA-VENTA',
            bold: true,
            size: 28,
            font: FONT_TITLE,
            characterSpacing: 80,
        ...
        new Paragraph({
          ...
          spacing: { before: 0, after: 60 },
          border: {
            bottom: { style: BorderStyle.SINGLE, size: 8, color: '1a1a1a', space: 8 },
          },
          children: [new TextRun({ text: '', size: SIZE_BODY })],
        }),
        espGrande(),
        ...
        para([
          ...
          normal(`${senorV} `),
          bold(d.nombre_vendedor.toUpperCase()),
          normal(`, declara ser ${mexicanoV} por nacimiento`),
        ...
        // ── ANTECEDENTES ─────────────────────────────────────────────────
        seccion('A N T E C E D E N T E S'),
        espacio(),
        ...
        para([
          ...
          d.notaria               ? normal(`DE LA ${d.notaria.toUpperCase()}, `) : normal(''),
        ...
        new Paragraph({
          ...
          children: [new TextRun({ text: d.direccion_inmueble.toUpperCase(), bold: true, size: SIZE_BODY, font: FONT_BODY })],
        }),
        espacio(),
