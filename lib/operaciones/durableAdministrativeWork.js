...
export const ADMIN_WORK_R1_HARD_CAP = 20;
...
export function administrativeWorkR1NotBefore(env = process.env) {
  const raw = String(env.SHADOW_ADMIN_WORK_R1_NOT_BEFORE || "").trim();
  if (!raw) return null;
  const parsed = new Date(raw);
  return Number.isNaN(parsed.getTime()) ? null : parsed.toISOString();
}

export function assertAdministrativeWorkR1Window(sourceOccurredAt, env = process.env) {
  if (!administrativeWorkR1Enabled(env)) { const error = new Error("admin_work_r1_disabled"); error.statusCode = 409; throw error; }
  const notBefore = administrativeWorkR1NotBefore(env);
  if (!notBefore) { const error = new Error("admin_work_r1_not_before_invalid"); error.statusCode = 409; throw error; }
  const source = new Date(sourceOccurredAt);
  if (!sourceOccurredAt || Number.isNaN(source.getTime())) { const error = new Error("admin_work_r1_source_time_invalid"); error.statusCode = 409; throw error; }
  if (source.getTime() < new Date(notBefore).getTime()) { const error = new Error("admin_work_r1_before_cutoff"); error.statusCode = 409; throw error; }
  if (source.getTime() > Date.now() + 5 * 60_000) { const error = new Error("admin_work_r1_source_time_invalid"); error.statusCode = 409; throw error; }
  return { notBefore, sourceOccurredAt: source.toISOString(), hardCap: ADMIN_WORK_R1_HARD_CAP };
}
...
export async function executeAdministrativeWorkR1(admin, { action, input, idempotencyKey, actorType = "admin", actorProfileId, sourceOccurredAt }, env = process.env) {
  const window = assertAdministrativeWorkR1Window(sourceOccurredAt, env);
  ...
  const { data, error } = await admin.rpc("execute_administrative_work_r1_supervised", {
    p_action: action, p_input: valid, p_idempotency_key: key,
    p_actor_type: aiActor ? "ai" : "human", p_actor_profile_id: aiActor ? null : actor,
    p_source_occurred_at: window.sourceOccurredAt, p_not_before: window.notBefore, p_hard_cap: window.hardCap,
  });
