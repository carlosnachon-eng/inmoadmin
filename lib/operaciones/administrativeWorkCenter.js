...
function workItem({
  ...
  return {
    ...
    presentationCategory: presentationCategory || (missingFields.length ? "data_quality" : "operational"),
  };
}
...
const shiftDateMonths = (value, months) => {
  ...
  return `${shifted.getUTCFullYear()}-${String(shifted.getUTCMonth() + 1).padStart(2, "0")}-${String(Math.min(date, lastDay)).padStart(2, "0")}`;
};

export function resolveServiceContractRelation(service, contracts = [], today = "") {
  const byId = new Map((contracts || []).filter((row) => row?.id).map((row) => [String(row.id), row]));
  if (service?.contract_id) {
    const contract = byId.get(String(service.contract_id));
    if (!contract) return { relation: "missing", contract: null };
    const active = normalize(contract.status) === "activo" && (!contract.end_date || contract.end_date >= today);
    return { relation: active ? "active" : "ended", contract };
  }

  const propertyName = normalize(service?.property_name);
  if (!propertyName) return { relation: "missing", contract: null };
  const matches = (contracts || []).filter((row) => normalize(row.property_name) === propertyName);
  const active = matches.filter((row) => normalize(row.status) === "activo" && (!row.end_date || row.end_date >= today));
  if (active.length === 1) return { relation: "legacy_match", contract: active[0] };
  if (active.length > 1) return { relation: "ambiguous", contract: null };
  if (matches.length === 1) return { relation: "ended", contract: matches[0] };
  if (matches.length > 1) return { relation: "ambiguous", contract: null };
  return { relation: "missing", contract: null };
}

function serviceItems(services, payments, contracts, properties, today) {
  const currentPeriod = today.slice(0, 7);
  const paymentByService = new Map();
  const paymentHistoryByService = new Map();
  (payments || []).forEach((row) => {
    const legacyKey = `${normalize(row.property_name)}:${normalize(row.tipo)}`;
    const serviceKey = row.servicio_id ? `id:${row.servicio_id}` : `legacy:${legacyKey}`;
    const history = paymentHistoryByService.get(serviceKey) || [];
    history.push(row);
    paymentHistoryByService.set(serviceKey, history);
    if (row.periodo !== currentPeriod) return;
    const key = `${serviceKey}:${row.periodo}`;
    const list = paymentByService.get(key) || [];
    list.push(row);
    paymentByService.set(key, list);
  });

  const propertyByName = new Map((properties || []).filter((row) => row?.name).map((row) => [normalize(row.name), row]));
  ...
  return (services || []).flatMap((service) => {
    ...
    const relation = resolveServiceContractRelation(service, contracts, today);
    const property = propertyByName.get(normalize(service.property_name)) || null;
    const legacyKey = `${normalize(service.property_name)}:${type}`;
    const serviceKey = service.id && (payments || []).some((row) => String(row.servicio_id || "") === String(service.id))
      ? `id:${service.id}`
      : `legacy:${legacyKey}`;
    const rows = paymentByService.get(`${serviceKey}:${currentPeriod}`) || [];
    const row = rows.slice().sort((a, b) => String(b.created_at || "").localeCompare(String(a.created_at || "")))[0];
    const history = (paymentHistoryByService.get(serviceKey) || [])
      .filter((payment) => /^\d{4}-\d{2}$/.test(String(payment.periodo || "")))
      .sort((a, b) => String(b.periodo).localeCompare(String(a.periodo)) || String(b.created_at || "").localeCompare(String(a.created_at || "")));
    const latestAnchor = history[0] || null;
    const cadenceMonths = frequency === "bimestral" ? 2 : frequency === "anual" ? 12 : null;
    const expectedPeriod = cadenceMonths && latestAnchor ? addMonthsToPeriod(latestAnchor.periodo, cadenceMonths) : null;
    const anchoredDueDate = cadenceMonths && latestAnchor?.fecha_limite ? shiftDateMonths(latestAnchor.fecha_limite, cadenceMonths) : null;
    const effectiveRow = row || (expectedPeriod === currentPeriod && anchoredDueDate ? {
      id: null, periodo: expectedPeriod, fecha_limite: anchoredDueDate, status: "pendiente", created_at: latestAnchor.created_at,
    } : null);
    const status = normalize(effectiveRow?.status);
    const hasReceipt = row?.hasReceipt === true;
    const daysUntil = daysBetween(today, effectiveRow?.fecha_limite);
    let ruleKey;
    let bucket = "para_hoy";
    let priority = "P1";
    let title;
    ...
    if (!row && ["bimestral", "anual"].includes(frequency) && (!expectedPeriod || !anchoredDueDate)) missing.push("periodicidad_sin_ancla");
    if (["missing", "ambiguous"].includes(relation.relation)) missing.push(`contract_relation.${relation.relation}`);

    if (relation.relation === "ended") {
      ruleKey = "servicio_historico_revisar";
      bucket = "requiere_autorizacion";
      priority = "P2";
      title = "Servicio histórico por revisar";
      reason = "El servicio está ligado a un contrato terminado y no debe tratarse como deuda actual.";
      action = "Revisar manualmente la configuración antes de archivarla o asociarla a otro contrato.";
    } else if (missing.length || (row && row.property_name !== service.property_name)) {
    ...
    } else if (effectiveRow && daysUntil !== null && daysUntil < 0 && status !== "pagado") {
    ...
    } else if (effectiveRow && daysUntil !== null && daysUntil <= 7 && status !== "pagado") {
    ...
    } else if (frequency === "mensual" && !row) {
      ruleKey = "servicio_periodo_sin_control";
      bucket = "para_hoy";
      title = "Control mensual no generado";
    ...
    return [workItem({
      ...
      dueAt: datePart(effectiveRow?.fecha_limite),
      lastActivityAt: row?.created_at || latestAnchor?.created_at || service.created_at || null,
      href: `/propiedades?serviceId=${encodeURIComponent(service.id)}&period=${encodeURIComponent(currentPeriod)}${row?.id ? `&paymentId=${encodeURIComponent(row.id)}` : ""}`,
      metadata: {
        propertyId: property?.id || null,
        contractId: relation.contract?.id || null,
        serviceId: service.id,
        servicePaymentId: row?.id || null,
        propertyLabel: service.property_name || null,
        type, frequency, payer: payer || null, period: currentPeriod,
        expectedPeriod, anchoredDueDate, status: status || null, hasReceipt,
        paidByEmporio: Boolean(row?.gasto_id), contractRelation: relation.relation,
      },
      missingFields: missing,
      presentationCategory: relation.relation === "ended" ? "historical_review" : missing.length ? "data_quality" : "operational",
    })];
  });
}
...
function keyItems(rows, now) {
  return (rows || []).flatMap((row) => {
    ...
    return [workItem({
      ...
      dueAt: row.fecha_prestamo, lastActivityAt: row.fecha_prestamo, href: `/checador?tab=llaves&keyId=${encodeURIComponent(row.id)}`,
      metadata: { keyId: row.id, keyNumber: row.numero || null, propertyName: row.propiedad || null, holderRole: row.portador_nombre ? "portador" : null, daysOut },
...
function liquidationItems(sources, today) {
  ...
  return owners.flatMap((ownerEmail) => {
    ...
    if (result.balance > 0) items.push(workItem({
      ...
      responsibleArea: "Administración", dueAt: `${period}-10`, href: `/liquidaciones?ownerId=${encodeURIComponent(ownerSourceId)}&period=${encodeURIComponent(period)}`,
      metadata: { ownerId: ownerSourceId, period, totalRent: result.totalRent, totalCommission: result.totalRetainableCommission, totalCommissionAccrued: result.totalCommissionAccrued, totalExpenses: result.totalExpenses, totalMaintenance: result.totalDiscountedMaintenance + result.totalPriorMaintenance, totalPaid: result.totalPaid, balance: result.balance, requiresFinancialAuthorization: true },
    }));
    result.priorPendingMaintenance.forEach((ticket) => items.push(workItem({
      ...
      href: `/mantenimiento?ticketId=${encodeURIComponent(ticket.id)}`, metadata: { ticketId: ticket.id, period, balance: maintenanceOwnerBalance(ticket), requiresFinancialAuthorization: true },
    })));
    return items;
  });
}
...
function ownerReceiptItems(rows) {
  return (rows || []).flatMap((row) => {
    ...
    return [workItem({
      ...
      href: `/liquidaciones?receiptId=${encodeURIComponent(row.id)}`, metadata: { receiptId: row.id, paymentMethod: method, hasReceipt: row.hasReceipt === true, requiresFinancialAuthorization: true },
    })];
  });
}
...
function condominiumCollectionItems(condominios, unidades, cuotas, today) {
  ...
  return (unidades || []).flatMap((unit) => {
    ...
    return [workItem({
      ...
      href: `/condominio/${condominiumId}?unitId=${encodeURIComponent(unitId)}&period=${encodeURIComponent(currentPeriod)}${receiptFees[0]?.id || overdueFees[0]?.id ? `&feeId=${encodeURIComponent(receiptFees[0]?.id || overdueFees[0]?.id)}` : ""}`,
      metadata: {
        condominiumId,
        unitId,
        feeId: receiptFees[0]?.id || overdueFees[0]?.id || null,
...
function paymentItems(rows, today) {
  return (rows || []).flatMap((row) => {
    ...
    if (status === "en_revision" && row.hasReceipt) return [workItem({
      ...
      href: `/cobranza?paymentId=${encodeURIComponent(row.id)}`, metadata: { paymentId: row.id, contractId: row.contract_id || null, propertyLabel: row.property_name || null, status, hasReceipt: true },
    })];
    ...
    return [workItem({
      ...
      href: `/cobranza?paymentId=${encodeURIComponent(row.id)}`,
      metadata: { paymentId: row.id, contractId: row.contract_id || null, propertyLabel: row.property_name || null, status, daysUntil, overdueDays },
    })];
  });
}
...
function contractItems(rows, today) {
  return (rows || []).flatMap((row) => {
    ...
    return [workItem({
      ...
      lastActivityAt: row.created_at || null,
      href: `/contratos?contractId=${encodeURIComponent(row.id)}`,
      metadata: { contractId: row.id, propertyLabel: row.property_name || null, status: normalize(row.status), daysUntil, overdueDays },
    })];
  });
}
...
function maintenanceItems(rows, quotes, now) {
  ...
  return (rows || []).flatMap((row) => {
    ...
    return [workItem({
      ...
      href: `/mantenimiento?ticketId=${encodeURIComponent(row.id)}`,
      metadata: {
        ticketId: row.id,
        propertyLabel: row.property_name || null,
        status,
...
function quoteItems(rows, now) {
  return (rows || []).flatMap((row) => {
    ...
    return [workItem({
      ...
      waitingOn,
      lastActivityAt: row.updated_at || row.created_at || null,
      href: `/mantenimiento?ticketId=${encodeURIComponent(row.ticket_id || "")}&quoteId=${encodeURIComponent(row.id)}`,
      metadata: {
        quoteId: row.id,
...
function signatureAppointmentItems(rows, firmas, today) {
  ...
  return (rows || []).flatMap((row) => {
    ...
    return [workItem({
      ...
      dueAt,
      lastActivityAt: row.created_at || null,
      href: `/firmas/${row.firma_id}?appointmentId=${encodeURIComponent(row.id)}`,
      metadata: { signatureId: row.firma_id, appointmentId: row.id, type: normalize(row.tipo), daysUntil },
    })];
  });
}
...
function policyItems(rows, today) {
  return (rows || []).flatMap((row) => {
    ...
    return [workItem({
      ...
      lastActivityAt: row.updated_at || row.created_at || null,
      href: `/poliza?expedienteId=${encodeURIComponent(row.id)}`,
      metadata: {
        policyCaseId: row.id,
        status: normalize(row.status),
...
export function dedupeSignatureContext(items = []) {
  const signatureById = new Map(items
    .filter((item) => item?.sourceType === "firmas" && item.metadata?.signatureId)
    .map((item) => [String(item.metadata.signatureId), item]));

  const overdueAppointmentBySignature = new Map();
  items
    .filter((item) => item?.sourceType === "firmas_citas" && item.bucket === "vencido" && item.metadata?.signatureId)
    .slice()
    .sort((a, b) => {
      const dueA = a.dueAt || "";
      const dueB = b.dueAt || "";
      if (dueA !== dueB) return dueB.localeCompare(dueA);
      return String(a.metadata?.appointmentId || a.contextKey)
        .localeCompare(String(b.metadata?.appointmentId || b.contextKey));
    })
    .forEach((item) => {
      const signatureId = String(item.metadata.signatureId);
      if (!overdueAppointmentBySignature.has(signatureId)) {
        overdueAppointmentBySignature.set(signatureId, item);
      }
    });

  return items.flatMap((item) => {
    if (item?.sourceType === "firmas" && item.metadata?.signatureId) {
      const appointment = overdueAppointmentBySignature.get(String(item.metadata.signatureId));
      if (!appointment) return [item];
      const appointmentId = appointment.metadata.appointmentId;
      return [{
        ...item,
        href: `/firmas/${item.metadata.signatureId}?appointmentId=${encodeURIComponent(appointmentId)}`,
        metadata: {
          ...item.metadata,
          appointmentContext: {
            appointmentId,
    ...
    if (item?.sourceType !== "firmas_citas" || !item.metadata?.signatureId) return [item];
    const primary = signatureById.get(String(item.metadata.signatureId));
    if (!primary || item.bucket !== "vencido") return [item];
    return [];
  });
}
...
export function buildAdministrativeWorkCenter(sources = {}, options = {}) {
  ...
  const deterministicItems = sortAndDedupeAdministrativeItems(dedupeSignatureContext([
    ...paymentItems(sources.payments, today),
    ...serviceItems(sources.servicios_inmueble, sources.pagos_servicios, sources.contracts, sources.properties, today),
  ...
  const summary = ADMINISTRATIVE_BUCKETS.reduce((acc, bucket) => {
    acc[bucket] = items.filter((item) => item.bucket === bucket && item.presentationCategory !== "data_quality" && item.presentationCategory !== "historical_review").length;
    return acc;
  }, { total: items.length });
  summary.data_quality = items.filter((item) => item.presentationCategory === "data_quality").length;
  summary.historical_review = items.filter((item) => item.presentationCategory === "historical_review").length;
