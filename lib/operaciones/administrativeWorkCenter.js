...
function liquidationItems(sources, today) {
  ...
  return owners.flatMap((ownerEmail) => {
    ...
    if (result.balance > 0) items.push(workItem({
      ...
      metadata: { period, totalRent: result.totalRent, totalCommission: result.totalCommission, totalExpenses: result.totalExpenses, totalMaintenance: result.totalDiscountedMaintenance + result.totalPriorMaintenance, totalPaid: result.totalPaid, balance: result.balance, requiresFinancialAuthorization: true },
    }));
...
export function applyAdministrativeCaseControls(items = [], controls = []) {
  ...
  }).filter((item) => item.supervision.status !== "resolved");
}
