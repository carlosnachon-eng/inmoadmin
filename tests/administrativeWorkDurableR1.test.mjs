...
test("RPC es única frontera mutativa y conserva actor", async () => { let payload; const db = { rpc: async (name, args) => { payload = { name, args }; return { data: { work_item_id: id }, error: null }; } }; await executeAdministrativeWorkR1(db, { action: "mark_information_received", input: { workItemId: id }, idempotencyKey: "fixture:key:0002", actorProfileId: id }, { SHADOW_ADMIN_WORK_R1_ENABLED: "true" }); assert.equal(payload.name, "execute_administrative_work_r1"); assert.equal(payload.args.p_actor_type, "human"); });
