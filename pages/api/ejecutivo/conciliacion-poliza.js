...
import { construirConciliacionPoliza } from '../../../lib/ejecutivo/conciliacionPoliza';
import { cargarPolizaConciliacion } from '../../../lib/ejecutivo/dataLoaders';
...
async function autenticarDireccion(req) {
  ...
  if (!autorizado) {
    return { error: 'No tienes permiso para consultar conciliación de póliza', status: 403 };
  }
...
export default async function handler(req, res) {
  ...
  try {
    const datos = await cargarPolizaConciliacion(supabase, metrics);
    const conciliacion = construirConciliacionPoliza(datos);
    ...
    return res.status(500).json({
      ok: false,
      error: error.message || 'Error al construir conciliación de póliza',
      _performance: metrics.summary(),
    });
  }
}
