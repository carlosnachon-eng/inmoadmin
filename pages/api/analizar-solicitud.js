...
export default async function handler(req, res) {
  ...
  const { data: sol, error: solError } = await supabase
    ...
    .select('nombre_completo, razon_social, tipo_ingresos, ingresos_mensuales, ingresos_empresa, monto_renta_solicitada, doc_comprobante_ingresos_b64, doc_ingresos_b64_2, doc_ingresos_b64_3, doc_identificacion_b64, curp')
    .eq('id', solicitud_id)
    .single();
  ...
  // ── Verificar si hay documentos de ingresos ──
  const docsB64 = [
    sol.doc_comprobante_ingresos_b64,
    sol.doc_ingresos_b64_2,
    sol.doc_ingresos_b64_3,
  ].filter(Boolean);

  const tieneDocumentos = docsB64.length > 0;
  ...
  const curpValida = validacionCurp?.valido === true;
  const curpFallo  = validacionCurp && !curpValida;

  // ── REGLA 1: Sin documentos → siempre Pendiente, nunca Viable ──
  if (!tieneDocumentos) {
    ...
    return res.status(200).json({
      ...
      mensajeInterno: `Sin documentos adjuntos.${curpFallo ? ` CURP inválida: ${validacionCurp?.curp_status}` : ''}`,
      validacionCurp,
      detalles: {
        nombre, renta, multiplicador, ingresoRequerido,
        ingresoDetectado: null, ingresoEvaluar: null,
        analisisIA: { alertas, confianza: 'baja' },
        sin_documentos: true,
      },
    });
  }

  // ── REGLA 2: CURP inválida → máximo "Revisar", nunca Viable ──
  // (se aplica al final después del análisis de documentos)
...
Analiza este documento y responde SOLO en formato JSON:
{
  ...
  "origen_ingresos": "descripción de depósitos",
  ...
  "alertas": ["observaciones para el analista"],
  "resumen_analista": "párrafo breve con todo lo relevante",
  "confianza": "alta|media|baja"
}

REGLAS:
1. SIEMPRE calcula ingreso_mensual_total aunque los depósitos sean cuestionables.
...
4. AUTENTICIDAD: Sin QR ni cadena digital → alerta específica.
5. ALERTAS: Menciona montos, conceptos y patrones encontrados.
6. NO uses saldo promedio para calcular ingreso.
No incluyas texto fuera del JSON.`;
  ...
  try {
    ...
    if (validos.length > 0) {
      ...
      const ingresosVerificables = docsIngresos.map(r => r.ingreso_mensual_verificable || r.ingreso_mensual_total).filter(v => v && v > 0);
      const ingresosTotales      = docsIngresos.map(r => r.ingreso_mensual_total).filter(v => v && v > 0);
      ...
      if (docINE && docsIngresos.length > 0) {
        ...
        const palabrasINE  = normalizar(docINE.nombre_en_documentos || '').split(' ').filter(p => p.length > 2);
        const nombreEstado = normalizar(docsIngresos[0].nombre_en_documentos || '');
        if (palabrasINE.filter(p => nombreEstado.includes(p)).length < 2) {
          alertaCruceNombre = `Nombre en INE (${docINE.nombre_en_documentos}) no coincide con nombre en estados de cuenta (${docsIngresos[0].nombre_en_documentos})`;
        }
      }
      ...
      const todasAlertas         = [...new Set([...validos.flatMap(r => r.alertas || []), ...alertasAutenticidad, ...(alertaCruceNombre ? [alertaCruceNombre] : [])])].filter(Boolean);
  ...
  if (validacionCurp) {
    ...
    if (validacionCurp.resultado === 'no_match' || validacionCurp.resultado === 'curp_baja') {
  ...
  if (analisisIA?.revision_manual) {
    ...
    icono          = '⚠️';
    mensaje        = 'Hemos recibido tu solicitud. Nuestro equipo jurídico revisará tus documentos y te contactará en breve.';
    mensajeInterno = `CURP inválida en RENAPO: ${validacionCurp?.curp_status || 'no verificada'}. ${alertas.join('. ')}`.trim();

  } else if (!ingresoEvaluar || ingresoEvaluar === 0) {
    resultado      = alertas.length > 0 || !actividadLicita || !nombreCoincide ? 'revisar' : 'pendiente';
    color          = '#92400e';
    icono          = resultado === 'revisar' ? '⚠️' : '⏳';
    mensaje        = 'Hemos recibido tu solicitud. Nuestro equipo jurídico la revisará y te contactará en breve.';
    mensajeInterno = `No se pudo calcular el ingreso desde los documentos.${alertas.length > 0 ? ` Alertas: ${alertas.join('. ')}` : ''}`;
    ...
    icono          = '⚠️';
    mensaje        = 'Hemos recibido tu solicitud. Nuestro equipo jurídico revisará tus documentos y te contactará en breve.';
    mensajeInterno = `Nombre en documentos (${analisisIA?.nombre_en_documentos || '—'}) no coincide con el solicitante.`;
    ...
    icono          = '⚠️';
    mensaje        = 'Hemos recibido tu solicitud. Nuestro equipo jurídico revisará tus documentos y te contactará en breve.';
    mensajeInterno = `Actividad cuestionable: ${alertas.join('. ')}`;

  } else if (ingresoEvaluar >= ingresoRequerido) {
    // Solo Viable si CURP válida Y documentos OK Y nombre coincide Y actividad lícita
  ...
  return res.status(200).json({
    ...
    detalles: {
      ...
      tipoDocumento, analisisIA, errorIA,
    },
  });
}
