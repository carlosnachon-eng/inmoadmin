...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [messages, conversations, events, matches, evaluations, operationalEvents, aiRuns, aiDecisions, toolAudit, manualAuthorizations, conversationActions, adminOutboundMessages, adminOutboundCanaries] = await Promise.all([
    ...
    const canaries = (adminOutboundCanaries.data || []).map((item) => ({
      ...item,
      provider_message_ref: sanitizedProviderMessageRef(item.provider_message_id),
      provider_message_id: undefined,
    }));
