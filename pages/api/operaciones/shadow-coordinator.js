...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [messages, conversations, events, matches, evaluations, operationalEvents] = await Promise.all([
    ...
    const failure = [messages, conversations, events, matches, evaluations, operationalEvents].find((result) => result.error)?.error;
    ...
    return res.status(200).json({ ok: true, messages: enrichedMessages, operationalEvents: operationalEvents.data || [], conversations: conversations.data || [], matches: matches.data || [], evaluations: evaluations.data || [], metrics: counts, aiStatus: "not_executed" });
  } catch (error) {
