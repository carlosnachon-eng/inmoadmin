...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [messages, conversations, events, matches, evaluations, aiRuns, aiDecisions, toolAudit] = await Promise.all([
      ...
      admin.from("shadow_ai_runs").select("id,message_id,status,model,prompt_version,started_at,completed_at,latency_ms,input_tokens,output_tokens,estimated_cost_usd,error_sanitized,attempt_number,retry_of_run_id").order("created_at", { ascending: false }),
    ...
    const failure = [messages, conversations, events, matches, evaluations, aiRuns, aiDecisions, toolAudit].find((result) => result.error)?.error;
    ...
    return res.status(200).json({ ok: true, messages: enrichedMessages, conversations: conversations.data || [], matches: matches.data || [], evaluations: evaluations.data || [], aiRuns: aiRuns.data || [], aiDecisions: aiDecisions.data || [], toolAudit: toolAudit.data || [], metrics: counts, aiStatus: (aiRuns.data || []).some((x)=>x.status==="completed") ? "executed_qa" : "not_executed" });
  } catch (error) {
