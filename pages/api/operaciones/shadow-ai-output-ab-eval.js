...
import { runShadowOutputAbFixture, SHADOW_OUTPUT_AB_FIXTURES } from "../../../lib/shadow/ai/outputAbEvaluation.js";
...
export default async function handler(req,res){
  ...
  try{return res.status(200).json({ok:true,result:await runShadowOutputAbFixture(fixtureId)});}catch(error){return res.status(500).json({ok:false,error:String(error?.message||"evaluation_failed").slice(0,80)});}
}
