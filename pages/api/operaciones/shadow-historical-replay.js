...
return async function handler(req, res) {
  ...
  try {
    if (req.method === "GET") {
      ...
      const enriched = (cases || []).map((row) => ({ ...row, ...storedHistoricalReplayConversationResult(row), review: latestReview.get(row.id) || null }));
