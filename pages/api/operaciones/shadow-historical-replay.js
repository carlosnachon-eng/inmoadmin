  ...
  try {
    ...
    if (action === "execute_one") {
      ...
      try {
        ...
        await admin.from("shadow_historical_replay_cases").update({ status: "completed", operational_resolution: resolution, conversation_action: conversation.conversation_action, proposed_message: conversation.proposed_message, result_safe: { tools: result.tools, evidence: result.evidence, providerRequestRefs: result.providerRequestRefs }, message_safe: result.messageSafe, would_resolve_without_human: resolution.would_resolve_without_human, input_tokens: result.inputTokens, output_tokens: result.outputTokens, estimated_cost_usd: result.estimatedCostUsd, latency_ms: result.latencyMs, completed_at: new Date().toISOString() }).eq("id", id).eq("status", "running");
        return res.status(200).json({ ok: true, caseId: id, status: "completed" });
      } catch (executionError) {
        await admin.from("shadow_historical_replay_cases").update({ status: "error", error_code: String(executionError.message || "replay_error").replace(/[^a-z0-9_]/gi, "_").toLowerCase().slice(0, 80), completed_at: new Date().toISOString() }).eq("id", id).eq("status", "running");
