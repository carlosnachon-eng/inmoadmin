...
import { validateQaCampaignId } from "../../../lib/shadow/ai/qaOrchestrator";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const campaignId = validateQaCampaignId(req.body?.campaignId);
    const admin = getAdminSupabase();
    const { data: owned, error } = await admin.from("shadow_ai_runs").select("id,message_id,prompt_version,campaign_id").eq("id", runId).maybeSingle();
    ...
    if (!owned || owned.prompt_version !== SHADOW_AI_PROMPT_VERSION || owned.campaign_id !== campaignId || message?.provider !== "synthetic" || !message?.provider_metadata?.syntheticScenario) return res.status(403).json({ ok: false, error: "Run fuera de la campaña QA actual." });
