...
export default function NuevaFirma() {
  ...
  async function handleSubmit(e) {
    ...
    if (data.firma) router.push(`/firmas/${data.firma.id}`)
  ...
  return (
    <div style={{ maxWidth: '620px', margin: '2rem auto', padding: '0 1rem' }}>
      <h1 style={{ color: '#1a3c5e', fontSize: '1.3rem', marginBottom: '1.5rem' }}>
        Nuevo expediente de firma
      </h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={card}>
          ...
          {esCV && (
            <div style={{ marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <input type="checkbox" id="contado"
                checked={form.es_contado}
                onChange={e => set('es_contado', e.target.checked)} />
              <label htmlFor="contado" style={{ fontSize: '0.9rem', color: '#555' }}>
                Operacion de contado (omite credito, avaluo e INFONAVIT)
              </label>
            </div>
          )}
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div>
              ...
              <input value={form.nombre_comprador} onChange={e => set('nombre_comprador', e.target.value)}
                required style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Nombre del propietario</label>
              <input value={form.nombre_vendedor} onChange={e => set('nombre_vendedor', e.target.value)}
                required style={inputStyle} />
            </div>
          </div>
        </div>
        ...
        {/* Coordinacion */}
        <div style={card}>
          <h3 style={sectionTitle}>Coordinacion</h3>

          {esCV && (
            <>
              <label style={labelStyle}>Monto de apartado</label>
              <input type="number" value={form.monto_apartado}
                onChange={e => set('monto_apartado', e.target.value)}
                placeholder="10000" style={inputStyle} />
            </>
          )}

          <label style={labelStyle}>Forma de pago del propietario</label>
          ...
          <select value={form.modalidad_firma} onChange={e => set('modalidad_firma', e.target.value)}
            style={inputStyle}>
          ...
          <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap', marginTop: '0.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <input type="checkbox" id="propietario"
                checked={form.propietario_asiste}
                onChange={e => set('propietario_asiste', e.target.checked)} />
              <label htmlFor="propietario" style={{ fontSize: '0.9rem', color: '#555' }}>
                Propietario asiste a la firma
              </label>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <label htmlFor="urgente" style={{ fontSize: '0.9rem', color: '#555' }}>
                Urgente (3 dias)
              </label>
            </div>
          </div>
        ...
        <button type="submit" disabled={loading}
          style={{ width: '100%', padding: '1rem', background: '#1a3c5e',
...
const card = {
  ...
  boxShadow: '0 1px 6px rgba(0,0,0,0.08)', marginBottom: '1rem'
}
const sectionTitle = { fontSize: '0.95rem', color: '#1a3c5e', marginTop: 0, marginBottom: '1rem' }
const labelStyle = { display: 'block', fontSize: '0.82rem', marginBottom: '4px', color: '#555' }
const inputStyle = {
  width: '100%', padding: '0.6rem', marginBottom: '1rem',
  border: '1px solid #ddd', borderRadius: '6px', fontSize: '0.95rem', boxSizing: 'border-box'
}
