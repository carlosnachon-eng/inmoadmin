...
import { RESPONSABLE_LABELS, STATUS_COLORS } from '../../lib/firmasEtapas'

export default function DetalleFirema() {
  ...
  if (!firma) return <div style={{ padding: '2rem' }}>Expediente no encontrado</div>
    ...
    <div style={{ maxWidth: '780px', margin: '2rem auto', padding: '0 1rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem' }}>
          <div>
            <p style={{ color: '#c8a45a', fontSize: '0.8rem', margin: '0 0 4px' }}>
              {firma.folio} — {firma.tipo.toUpperCase()} {firma.es_contado ? '(Contado)' : ''}
            </p>
            <h1 style={{ color: '#fff', fontSize: '1.15rem', margin: 0 }}>{firma.titulo}</h1>
            {firma.direccion && (
              <p style={{ color: '#aac4de', fontSize: '0.85rem', margin: '4px 0 0' }}>{firma.direccion}</p>
            )}
          </div>
          <span style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '1.5rem' }}>
        ...
        <div>
          ...
          {etapas.map((etapa, i) => {
            const sc = STATUS_COLORS[etapa.status]
            ...
            return (
              <div key={etapa.id} style={{
                ...
                boxShadow: esActual ? '0 2px 8px rgba(26,60,94,0.1)' : 'none',
                ...
                <div style={{ padding: '0.9rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: 0, fontSize: '0.88rem', fontWeight: esActual ? 600 : 400,
                      color: etapa.status === 'no_aplica' ? '#bbb' : '#222' }}>
                      {etapa.nombre}
                    </p>
                ...
                {abierta && (
                  <div style={{ borderTop: '1px solid #eee', padding: '0.9rem 1rem', background: '#fafafa', borderRadius: '0 0 8px 8px' }}>
                    ...
                    <textarea value={notaEtapa} onChange={e => setNotaEtapa(e.target.value)}
                      rows={2} placeholder="Agrega observaciones si es necesario..."
                      style={{ width: '100%', padding: '0.5rem', border: '1px solid #ddd',
                        borderRadius: '6px', fontSize: '0.88rem', boxSizing: 'border-box', resize: 'none' }} />
                    <button onClick={() => completarEtapa(etapa)} disabled={avanzando}
                      style={{ marginTop: '8px', background: '#22c55e', color: '#fff',
                ...
                {etapa.notas && etapa.status === 'completada' && (
                  <div style={{ borderTop: '1px solid #eee', padding: '0.6rem 1rem',
                    fontSize: '0.8rem', color: '#666', background: '#fafafa', borderRadius: '0 0 8px 8px' }}>
                    Nota: {etapa.notas}
                  </div>
                )}
              </div>
        ...
        <div>
          ...
          <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #eee', padding: '1rem', marginBottom: '1rem' }}>
            ...
            <button onClick={enviarComentario}
              style={{ marginTop: '8px', background: '#1a3c5e', color: '#fff',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {comentarios.map(c => (
              <div key={c.id} style={{
                background: c.tipo === 'cambio_etapa' ? '#f0f7ff' : '#fff',
                border: '1px solid #eee', borderRadius: '8px', padding: '0.75rem'
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                  <span style={{ fontSize: '0.78rem', fontWeight: 600, color: '#1a3c5e' }}>
