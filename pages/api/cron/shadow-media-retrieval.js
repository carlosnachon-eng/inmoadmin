...
export const config={api:{responseLimit:"1mb"},maxDuration:120};
...
export default async function handler(req,res){
  ...
  try{const processed=await processOneMediaRetrieval(getAdminSupabase());return res.status(200).json({ok:true,claimed:processed?1:0,processed:processed?[{id:processed.id,status:processed.status,error:processed.error||null,interpretation:processed.interpretation?.status||null}]:[]});}
