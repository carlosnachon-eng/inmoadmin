...
import { processOneMediaRetrieval } from "../../../lib/shadow/media/worker";
...
export default async function handler(req,res){
  ...
  if(process.env.SHADOW_MEDIA_RETRIEVAL_ENABLED!=="true"||process.env.SHADOW_OUTBOUND_ENABLED==="true")return res.status(409).json({ok:false,error:"Media retrieval deshabilitado."});
  try{const processed=await processOneMediaRetrieval(getAdminSupabase());return res.status(200).json({ok:true,processed:processed?[{id:processed.id,status:processed.status,error:processed.error||null}]:[]});}
