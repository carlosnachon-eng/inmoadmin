...
export default function ShadowCoordinatorPage() {
  ...
  const [identityRunFilter, setIdentityRunFilter] = useState("all");
  ...
  const operateReplayAttempt = async (action, item) => {
    if (replayAttemptInFlight.current || historicalReplayBusy || profile?.active !== true || profile?.role_id !== "admin") return;
    if (!window.confirm(action === "prepare_retry"
      ? "Crear un nuevo intento pendiente, conservando íntegramente el error anterior. No se ejecutará el modelo. ¿Continuar?"
      : "Ejecutar una sola vez este intento de Historical Replay, sin envíos. ¿Continuar?")) return;
    replayAttemptInFlight.current = true; setHistoricalReplayBusy(true); setError("");
    try {
      const { data: authData, error: sessionError } = await supabase.auth.getSession();
      const current = authData?.session;
      if (sessionError || !current?.access_token || Number(current.expires_at || 0) <= Math.floor(Date.now()/1000)) throw new Error("session_required");
      const body = action === "prepare_retry"
        ? { action, caseId: item.case_id || item.id, parentAttemptId: item.case_id ? item.id : null, authorization: "explicit_admin_retry" }
        : { action: "execute_one", caseId: item.case_id, attemptId: item.id };
      const response = await fetch("/api/operaciones/shadow-historical-replay", { method:"POST", headers:{"Content-Type":"application/json",Authorization:`Bearer ${current.access_token}`}, body:JSON.stringify(body) });
      const json = await response.json();
      if (!response.ok) setError(json.error || "replay_attempt_failed");
      await loadHistoricalReplay();
    } catch {
      setError("No se pudo acreditar el resultado. Actualiza la vista antes de otra acción; no se reintentó automáticamente.");
    } finally { replayAttemptInFlight.current = false; setHistoricalReplayBusy(false); }
  };
  ...
  const runExactPhoneIdentityPreflight = async () => {
    ...
    try {
      ...
      const json = await response.json();
      if (!response.ok) return setError(json.error || "No se pudo ejecutar el preflight de identidad.");
      setIdentityPreflight(json);
    } catch {
      setError("No se pudo ejecutar el preflight de identidad.");
    } finally {
  ...
  const identityRuns = useMemo(() => (data?.runIdentityObservability || []).filter((item) => {
    if (identityRunFilter === "confirmed") return item.identityState === "confirmed";
    if (identityRunFilter === "7of7") return item.inExactPhone7of7;
    if (identityRunFilter === "unattributed") return item.attribution === "unattributed";
    return true;
  }), [data, identityRunFilter]);
  ...
  return <Layout view="coordinador_ia_sombra" profile={profile} onLogout={async () => { await supabase.auth.signOut(); window.location.href = "/"; }}>
    ...
    <main style={{ padding: 22 }}>
      {identityPreflightAuthorized && <CondominiumIdentityReview supabase={supabase} profile={profile} />}
      ...
      {authorized && <details style={{ ...card, marginBottom: 14 }} open><summary><strong>Observabilidad de identidad por run</strong></summary><p style={{color:brand.grayLight}}>Atribución derivada sólo de la resolución persistida que utilizó cada run. Sin evidencia inequívoca se muestra como no atribuido.</p><div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>{[["all","Todos"],["confirmed","Identidad confirmada"],["7of7","Cohorte 7/7"],["unattributed","No atribuidos"]].map(([value,label])=><button key={value} aria-pressed={identityRunFilter===value} onClick={()=>setIdentityRunFilter(value)}>{label}</button>)}</div>{!identityRuns.length?<p>No hay runs para este filtro.</p>:identityRuns.map((item)=><article key={item.runRef} style={{borderTop:`1px solid ${brand.border}`,padding:"10px 0"}}><strong>Run {item.runRef}</strong>{item.inExactPhone7of7&&<span> · 7/7 {item.exactPhone7of7Ref}</span>}<p style={{margin:"4px 0"}}>Identidad: {item.identityState} · fuente: {item.resolutionSource||"sin evidencia"} · <RunIdentityScope item={item} /> · relación: {item.relationshipResolved?"resuelta":"no resuelta"}</p><p style={{margin:"4px 0"}}>3B: {item.action3B||"sin acción"} · requiere humano: {item.requiresHuman==null?"sin dato":item.requiresHuman?"sí":"no"} · auto-send elegible: {item.autoSendEligible==null?"sin dato":item.autoSendEligible?"sí":"no"}</p><small>Blocker: {item.blocker||item.attributionReason||"ninguno"}</small></article>)}</details>}
      ...
      {identityPreflightAuthorized && <details style={{ ...card, marginBottom: 14 }}>
        <summary><strong>Preflight identidad exact_phone_unique</strong></summary>
      ...
      {authorized && <details style={{ ...card, marginBottom: 16 }}><summary><strong>Evaluación histórica 3B</strong></summary>
        ...
        <button disabled={historicalReplayBusy} onClick={()=>operateHistoricalReplay("preview")}>Previsualizar cohorte</button>{historicalReplayPreview&&<><p>{historicalReplayPreview.selected} elegibles · mantenimiento {historicalReplayPreview.counts?.maintenance||0} · pagos {historicalReplayPreview.counts?.payment||0} · pendientes {historicalReplayPreview.counts?.administrative_pending||0}</p>
          <p>Ventana de turnos: {historicalReplayPreview.sourceInfo?.occurredSince} → {historicalReplayPreview.sourceInfo?.asOf}. Conversaciones completas: {historicalReplayPreview.sourceInfo?.conversationsLoaded}. Excluidas por límite de contexto: {historicalReplayPreview.sourceInfo?.excluded?.context_read_limit||0}; por contexto de adjuntos: {historicalReplayPreview.sourceInfo?.excluded?.media_context_read_limit||0}. {historicalReplayPreview.sourceInfo?.conversationLimitReached?"Se alcanzó el tope de conversaciones recientes; no es un censo de todo el tráfico.":""}</p>
          {(historicalReplayPreview.cases||[]).map((item)=><label key={item.historicalTurnKey} style={{display:"block",padding:"3px 0"}}><input type="checkbox" checked={historicalReplayTurnKeys.includes(item.historicalTurnKey)} disabled={!historicalReplayTurnKeys.includes(item.historicalTurnKey)&&historicalReplayTurnKeys.length>=30} onChange={(event)=>setHistoricalReplayTurnKeys((current)=>event.target.checked?[...current,item.historicalTurnKey]:current.filter((key)=>key!==item.historicalTurnKey))}/> {item.domain} · {item.messageCount} mensaje(s) · {new Date(item.occurredAt).toLocaleDateString("es-MX")} · respuesta humana {item.humanResponseAvailable?"sí":"no"}</label>)}<button disabled={historicalReplayBusy||!historicalReplayTurnKeys.length} onClick={()=>operateHistoricalReplay("prepare",{turnKeys:historicalReplayTurnKeys,sourceSnapshot:historicalReplayPreview.sourceSnapshot})}>Preparar selección ({historicalReplayTurnKeys.length}/30)</button></>}
        <p>{historicalReplay?.metrics?.total||0} seleccionados · {historicalReplay?.metrics?.completed||0} evaluados · {Math.round(Number(historicalReplay?.metrics?.safeMessageRate||0)*100)}% mensajes seguros · {historicalReplay?.metrics?.autoSendEligible||0} con auto_send_eligible=true de 3B · {historicalReplay?.metrics?.eligibilityNotRecorded||0} sin elegibilidad 3B registrada</p>
        <p>Los contadores anteriores corresponden al intento original de cada caso. Los retries se muestran separados y no sustituyen su evidencia.</p>
        {!(historicalReplay?.cases||[]).length?<p>No hay una cohorte histórica preparada.</p>:(historicalReplay.cases||[]).flatMap((item)=>[item,...(item.attempts||[])]).map((item)=><article key={item.id} style={{borderTop:`1px solid ${brand.border}`,padding:"10px 0"}}>
          <p><strong>Intento {item.attempt_number||1}: {item.status}</strong> · caso {item.case_ref}{item.attempt_ref?<> · intento ref {item.attempt_ref} · anterior {item.parent_ref}</>:" · original (evidencia conservada)"}</p>
          {item.error_code&&<p>Error registrado: {item.error_code}</p>}
          {item.can_retry&&<button disabled={historicalReplayBusy} onClick={()=>operateReplayAttempt("prepare_retry",item)}>Crear nuevo intento (sin ejecutar)</button>}
          ...
          <p><strong>3B real:</strong> {item.conversation_action??"no registrado"} · requires_human: {item.requires_human==null?"no registrado":String(item.requires_human)} · auto_send_eligible: {item.auto_send_eligible==null?"no registrado":String(item.auto_send_eligible)} · blocker: {item.auto_send_eligible==null?"no registrado":item.blocked_reason??"ninguno"}. La valoración humana es independiente de estos valores.</p>
          ...
          <p><strong>Tokens:</strong> {item.input_tokens??"desconocido"}/{item.output_tokens??"desconocido"} · USD {item.estimated_cost_usd==null?"desconocido":Number(item.estimated_cost_usd).toFixed(4)} · {item.latency_ms||0} ms</p>
          <p><strong>Modelo acreditado:</strong> {item.provider_model_status==="reported"?(item.result_safe?.providerModels||[]).join(", "):item.provider_model_status==="partial"?"parcial; no acreditado en todas las rondas":"no acreditado"}</p>
          {item.provider_http&&<div aria-label="Diagnóstico HTTP sanitizado del proveedor">
