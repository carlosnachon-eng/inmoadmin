...
export default function ShadowCoordinatorPage() {
  ...
  const [diagnosticBusy, setDiagnosticBusy] = useState(false); const [diagnosticResults, setDiagnosticResults] = useState(null);
  ...
  const runStagedDiagnostic = async () => {
    setDiagnosticBusy(true); setError(""); const results = [];
    for (const stage of [1,2,3,4,5]) {
      const response = await fetch("/api/operaciones/anthropic-staged-diagnostic", { method:"POST", headers:{"Content-Type":"application/json",Authorization:`Bearer ${session.access_token}`}, body:JSON.stringify({stage}) });
      const json = await response.json(); results.push(json); if (!response.ok) break;
    }
    setDiagnosticResults(results); setDiagnosticBusy(false);
  };
  ...
  return <Layout view="coordinador_ia_sombra" profile={profile} onLogout={async () => { await supabase.auth.signOut(); window.location.href = "/"; }}>
    ...
    <main style={{ padding: 22 }}>
      ...
      {authorized && <details style={{ ...card, marginBottom: 14 }}><summary><strong>Diagnóstico Anthropic temporal 5/5</strong></summary><p>Contexto exclusivamente sintético. Cada escalón sólo puede reclamarse una vez en este entorno.</p><button disabled={diagnosticBusy||Boolean(diagnosticResults)} onClick={runStagedDiagnostic}>{diagnosticBusy?"Ejecutando…":"Ejecutar cinco escalones"}</button>{diagnosticResults&&<pre style={{overflow:"auto",maxHeight:320}}>{JSON.stringify(diagnosticResults,null,2)}</pre>}</details>}
