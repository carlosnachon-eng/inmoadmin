...
export default function ShadowCoordinatorPage() {
  ...
  const reconcileClient = async (action, candidate = null) => {
    ...
    const response = await fetch("/api/operaciones/client-reconciliation", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ action, ...(candidate ? { candidateId: candidate.id } : {}), ...(existingIdentityId ? { existingIdentityId } : {}) }) });
  ...
  return <Layout view="coordinador_ia_sombra" profile={profile} onLogout={async () => { await supabase.auth.signOut(); window.location.href = "/"; }}>
    ...
    <main style={{ padding: 22 }}>
      ...
      {authorized && clientReconciliation && <details style={{ ...card, marginBottom: 14 }}><summary><strong>Reconciliación de clientes activos</strong></summary><p style={{color:brand.grayLight}}>Sólo contratos activos y propietarios administrados. La confirmación es humana, auditable y no crea cuentas de acceso.</p><button disabled={clientReconciliationBusy} onClick={()=>reconcileClient("prepare")}>Preparar cohorte operativa</button>{(clientReconciliation.candidates || []).filter((item)=>item.candidate_status!=="confirmed").map((item)=><article key={item.id} style={{borderTop:`1px solid ${brand.border}`,padding:"10px 0"}}><strong>{item.role_kind === "tenant" ? "Inquilino" : "Propietario"}</strong> · {item.candidate_status.replaceAll("_"," ")}<p>{item.source_count} fuente(s) · {item.reason_code}</p><label>Unir a identidad existente (UUID opcional)<input aria-label={`Identidad existente ${item.id}`} value={existingClientIdentityIds[item.id] || ""} onChange={(event)=>setExistingClientIdentityIds((current)=>({...current,[item.id]:event.target.value}))} autoComplete="off" style={{display:"block",width:"100%",boxSizing:"border-box",padding:7,margin:"4px 0 8px"}}/></label><button disabled={clientReconciliationBusy} onClick={()=>reconcileClient("confirm",item)}>Confirmar</button><button disabled={clientReconciliationBusy} onClick={()=>reconcileClient("reject",item)} style={{marginLeft:8}}>Rechazar</button><button disabled={clientReconciliationBusy} onClick={()=>reconcileClient("conflict",item)} style={{marginLeft:8}}>Marcar conflicto</button><button disabled={clientReconciliationBusy} onClick={()=>reconcileClient("skip",item)} style={{marginLeft:8}}>Omitir</button></article>)}</details>}
