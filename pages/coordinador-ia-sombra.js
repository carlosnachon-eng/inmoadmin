...
export default function ShadowCoordinatorPage() {
  ...
  const [anthropicDiagnostic, setAnthropicDiagnostic] = useState(null); const [anthropicDiagnosticBusy, setAnthropicDiagnosticBusy] = useState(false);
  ...
  const runAnthropicDiagnostic = async () => {
    setAnthropicDiagnosticBusy(true);
    const response = await fetch("/api/operaciones/anthropic-connectivity-diagnostic", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: "{}" });
    setAnthropicDiagnostic(await response.json()); setAnthropicDiagnosticBusy(false);
  };
  ...
  return <Layout view="coordinador_ia_sombra" profile={profile} onLogout={async () => { await supabase.auth.signOut(); window.location.href = "/"; }}>
    ...
    <main style={{ padding: 22 }}>
      <div style={{ marginBottom: 16 }}><h1 style={{ margin: 0, color: brand.gray }}>🌒 Coordinador IA — Sombra</h1><p style={{ color: brand.grayLight }}>Sólo observación y evaluación. No envía mensajes ni modifica el ERP.</p><button disabled={anthropicDiagnosticBusy || Boolean(anthropicDiagnostic)} onClick={runAnthropicDiagnostic}>{anthropicDiagnosticBusy ? "Diagnosticando…" : "Diagnóstico Anthropic (una vez)"}</button>{anthropicDiagnostic && <pre>{JSON.stringify(anthropicDiagnostic, null, 2)}</pre>}</div>
