do $$ begin
  ...
  if has_function_privilege('anon','public.claim_shadow_admin_outbound(text)','EXECUTE') or has_function_privilege('authenticated','public.claim_shadow_admin_outbound(text)','EXECUTE') then raise exception 'claim must be server-side'; end if;
  if not has_function_privilege('service_role','public.claim_shadow_admin_outbound(text)','EXECUTE') then raise exception 'service role claim missing'; end if;
