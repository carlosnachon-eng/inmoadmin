-- Web -> WhatsApp -> Respond.io attribution for the property CTA.
-- Additive: no backfill and no changes to the canonical Respond.io queue/snapshots.
...
create table public.whatsapp_attributions (
  ...
  page_origin text not null default 'emporio_web' check (
    page_origin = 'emporio_web'
  ),
...
create index whatsapp_attributions_retention_idx
  on public.whatsapp_attributions (retention_expires_at, id);
...
create or replace function public.purge_whatsapp_attribution_retention()
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_deleted_attributions integer := 0;
begin
  perform pg_catalog.set_config('app.whatsapp_attribution_retention_purge', 'on', true);

  delete from public.whatsapp_attribution_events e
  where exists (
    select 1
    from public.whatsapp_attributions a
    where a.id = e.attribution_id
      and a.retention_expires_at <= now()
  );
  get diagnostics v_deleted_events = row_count;

  delete from public.whatsapp_attributions
  where retention_expires_at <= now();
  get diagnostics v_deleted_attributions = row_count;

  return jsonb_build_object(
    'deleted_attributions', v_deleted_attributions,
...
begin
  ...
  if p_property_id is null or not exists (
    ...
    where id = p_property_id
      and public_id = trim(p_property_public_id)
      and status in ('published', 'reserved')
  ) then
    raise exception 'property_context_not_found' using errcode = '22023';
  end if;
  ...
  with expired as (
    ...
    where status in ('clicked', 'message_observed')
      and expires_at <= now()
    returning id, expires_at
  )
  insert into public.whatsapp_attribution_events (
    attribution_id, event_type, occurred_at, metadata
  )
  select id, 'expired', expires_at, jsonb_build_object('outcome', 'retention_maintenance')
  from expired;

  perform public.purge_whatsapp_attribution_retention();
  ...
  perform pg_catalog.pg_advisory_xact_lock(20260814, 1);
  if (
    select count(*)
    from public.whatsapp_attributions
...
begin
  ...
  if v_row.status = 'contact_linked' then
    insert into public.whatsapp_attribution_events (
      ...
      v_row.id,
      case
        when v_row.respond_contact_id = p_respond_contact_id then 'replayed'
        else 'invalid'
      end,
      p_webhook_event_id, left(p_respond_contact_id, 128),
      nullif(left(coalesce(p_message_id, ''), 128), ''), v_occurred_at,
      jsonb_build_object(
        'outcome', case
...
begin
  if tg_op = 'DELETE'
     and current_setting('app.whatsapp_attribution_retention_purge', true) = 'on' then
    return old;
  end if;
...
revoke all on function public.purge_whatsapp_attribution_retention()
  from public, anon, authenticated;
...
grant execute on function public.purge_whatsapp_attribution_retention()
  to service_role;
