-- DEV ONLY: habilita el provider Respond/Admin sobre el esquema P0 existente.
...
begin
  if to_regclass('public.shadow_conversations') is null
     ...
     or to_regprocedure('public.ingest_shadow_message(jsonb,jsonb)') is null then
    raise exception 'P1 Respond/Admin requiere el bootstrap P0 Shadow';
  end if;
  if coalesce(obj_description('public.shadow_conversations'::regclass,'pg_class'),'')
    ...
    raise exception 'P1 Respond/Admin sólo puede modificar objetos P0 Shadow DEV';
  end if;
end $$;
...
alter table public.shadow_conversations add constraint shadow_conversations_provider_check
  check (provider in ('synthetic','respond','respond_admin','meta','bsp'));
...
alter table public.shadow_messages add constraint shadow_messages_direction_check
  check (direction in ('inbound','outbound','outbound_human'));
...
begin
  if coalesce(p_envelope->>'provider','') not in ('synthetic','respond','respond_admin') then raise exception 'Provider Shadow no permitido'; end if;
  ...
  select id into v_event from public.shadow_ingestion_events
...
comment on function public.ingest_shadow_message(jsonb,jsonb) is 'dev-bootstrap:202608190002:fase-2a-p1-respond-admin';
