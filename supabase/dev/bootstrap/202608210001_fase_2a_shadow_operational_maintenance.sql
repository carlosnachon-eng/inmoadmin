-- DEV-only: Fase 2A Operational Events / mantenimiento. Sin IA, seeds ni datos reales.
begin;

do $$ begin
  if current_database() is null or to_regclass('public.maintenance_tickets') is null
    ...
    raise exception 'Dependencias DEV incompletas';
  end if;
  if to_regclass('public.inmoadmin_operational_events') is not null
     or to_regclass('public.shadow_operational_events') is not null then
    raise exception 'Colisión: bootstrap Operational Events ya existe';
  end if;
end $$;
...
comment on column public.maintenance_tickets.maintenance_scope is 'dev-bootstrap:202608210001; managed_property|external_job; NULL sólo para filas legacy';
comment on column public.maintenance_tickets.external_job_reference is 'dev-bootstrap:202608210001; referencia segura no-PII para external_job';
...
comment on table public.inmoadmin_operational_events is 'dev-bootstrap:202608210001:fase-2a-shadow-operational-outbox';
...
comment on table public.shadow_operational_events is 'dev-bootstrap:202608210001:fase-2a-shadow-operational-events';
create index shadow_operational_events_ticket_idx on public.shadow_operational_events(ticket_id,occurred_at desc);

create or replace function public.shadow_operational_authorized_role() returns boolean language sql stable security definer set search_path=public as $$
...
create or replace function public.create_maintenance_ticket_with_event(p_ticket jsonb)
...
begin
  ...
  if not ((scope='managed_property' and prop is not null and ref is null) or (scope='external_job' and prop is null and ref is not null and length(ref)<=120)) then raise exception 'maintenance_scope inconsistente'; end if;
...
create or replace function public.approve_maintenance_quote_with_event(p_quote_id uuid)
returns jsonb language plpgsql security definer set search_path=public as $$
...
create or replace function public.process_operational_event(p_event_id uuid)
returns jsonb language plpgsql security definer set search_path=public as $$
