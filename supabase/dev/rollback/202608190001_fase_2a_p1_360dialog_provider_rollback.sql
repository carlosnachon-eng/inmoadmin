-- DEV ONLY. Restaura la allowlist P0 sin eliminar tablas ni datos.
begin;
do $$
begin
  if coalesce(obj_description('public.shadow_conversations'::regclass, 'pg_class'), '')
    ...
    raise exception 'Objeto Shadow no pertenece al bootstrap P0 DEV';
  end if;
end $$;
...
alter table public.shadow_conversations add constraint shadow_conversations_provider_check
  check (provider in ('synthetic','respond','meta','bsp')) not valid;

do $$ begin
  if exists(select 1 from public.shadow_conversations where provider='360dialog') then
    raise exception 'Rollback bloqueado: existen filas 360dialog; no se borrarán automáticamente';
  end if;
end $$;
alter table public.shadow_conversations validate constraint shadow_conversations_provider_check;
