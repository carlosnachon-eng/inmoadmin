...
export default function LegacyCondominoPortal({ allowedUnitIds = null }) {
  ...
  const loadData = async () => {
    ...
    let unitQuery = supabase
      ...
      .eq("activo", true);
    if (Array.isArray(allowedUnitIds)) unitQuery = unitQuery.in("id", allowedUnitIds);
    const { data: unidades } = await unitQuery.limit(1);
